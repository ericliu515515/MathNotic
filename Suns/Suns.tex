...
(iv)$\implies $(i): This follows from the fact that \customref{THpp1}{$p$-groups are nilpotent} and that nilpotency is closed under taking finite direct product. 
\end{proof}
\section{Old Numbers}
\begin{abstract}
...
\begin{abstract}
This section deals with the very basic properties of direct limit, especially in $\mathbf{Ring}$ and $\mathbf{Mod}_A$. 
\end{abstract}
\begin{theorem}
...
\begin{theorem}
\label{THpdl}
\textbf{(Basic properties of direct limit)}  Let $A$ be a ring, $I$ a directed set, and $(M_i,\phi)$ a directed system of $A$-modules. Then 
\begin{enumerate}[label=(\roman*)]
...
\end{enumerate}
Let $(A_i, \psi)$ be a directed system of ring. Then 
\begin{enumerate}[label=(\roman*)]
  \item $\directlimit A_i =0 \implies A_i=0$ for some $i$.  
  \item If $A_i$ are integral domain, then  $\directlimit A_i$ is an integral domain. 
  \item We have a natural $\Z$-module isomorphism: 
    \begin{align*}
    \directlimit \operatorname{Nil}(A_i)\cong   \operatorname{Nil} \left(\directlimit A_i \right), \quad \pfi _i(a_i) \mapsto \psi_i (a_i) 
    \end{align*}
   where the canonical morphism $\pfi$ of directed system of $\operatorname{Nil}(A_i)$ is obtained by restricting $\psi$. 
\end{enumerate}
\end{theorem}
\begin{proof}
The statements for $M_i$ all follow from the construction, so we only prove the statement for $A_i$.\\

(i): Fix $i$. Because  $\psi_i$ maps $1_{A_i}$ to $0$, we know there exists some $j \geq i$ such that $\psi_{ij}$ maps $1_{A_i}$ to $0$. Then because ring homomorphism preserve multiplicative identity, we see $A_j=0$.\\


(ii): Let  $\psi_i(a_i)\psi_j(a_j)=0$. Then for all $k\geq i,j$, we have $\psi_k(\psi_{ik}(a_i)\psi_{jk}(a_j))=0$. It then follows from the premise that $A_k$ is an integral domain that either $\psi_{ik}(a_i)$ or  $\psi_{jk}(a_j)$ is $0$, which implies that either  $\psi_i(a_i)=\psi_k \circ \psi_{ik}(a_i)$ or $\psi_j\circ \psi_{jk}(a_j)$ is $0$. \\

(iii): The reason we may restrict $\psi$ follows from the obvious fact that ring homomorphisms preserve nilpotents. The proposed homomorphism is clearly well-defined and have image lying inside $\operatorname{Nil}\left(\directlimit A_i \right)$. To see $\operatorname{Nil}\left(\directlimit A_i \right)\leq \directlimit \operatorname{Nil}(A_i)$, just observe that if $\psi_i(a_i)^n=0$, then $\psi_i(a_i^n)=0$, and therefore for some $j\geq i$, we have $\psi_{ij}(a_i)^n=0$, which implies $\psi_{ij}(a_i)\in \operatorname{Nil}(A_j)$, and therefore $\psi_i(a_i)$ is the image of $\pfi _{ij}(a_i)\in \operatorname{Nil}(A_j)$. 
\end{proof}
\begin{theorem}
\label{THdls}
\textbf{(Module sums are direct limits)} Let $M_i$ be a collection of submodules of $M$ indexed by $I$ that satisfies 
\begin{center}
   \begin{minipage}{0.9\linewidth}  
       \centering
       For all $i,j \in I$, there exists some $k \in I$ that makes $M_i + M_j \leq M_k$. 
   \end{minipage}
\end{center}
then $M_i$ and  $M \quotient M_i$ naturally forms a directed system whose direct limit can be realized as  
\begin{align*}
\directlimit M_i = \sum M_i  = \bigcup  M_i\quad \text{ and }\quad \directlimit (M \quotient M_i)= M \quotient \sum M_i
\end{align*}
...
Routine. 
\end{proof}
Let $A$ be a ring and $I$ be a directed set. Then we can define a covariant functor $D: \operatorname{Func}(I,\mathbf{Mod}_A)\rightarrow \mathbf{Mod}_A$ by 
\begin{align*}
D\left(M_i, \mu \right) \triangleq \directlimit M_i \quad \text{ and }\quad D(\Phi)\triangleq \phi
\end{align*}
where $\phi:\directlimit M_i \rightarrow \left(\directlimit N_i,\nu   \right)$ is the unique $A$-module homomorphism such that $\phi \circ \mu _i = \nu _i \circ \phi_i$.  
\begin{equiv_def}
\label{EDsm}
\textbf{(Direct limit is an exact functor)} Let $A$ be a ring and $I$ be a directed set. The covariant functor $\directlimit : \operatorname{Func}(I, \mathbf{Mod}_A)\rightarrow \mathbf{Mod}_A$ is exact. 
\end{equiv_def}
\begin{proof}
Routine. 
\end{proof}
...
\begin{theorem}
\label{THgCH}
\textbf{(Generalized Cayley-Hamilton theorem)} Let $M$ be a finitely generated $A$-module, $\mathfrak{a}\trianglelefteq A$, and $\pfi$ an $A$-module endomorphism of  $M$ such that $\pfi (M)\leq \mathfrak{a}M$. Then for some $n\geq 1$, $\pfi $ satisfies an equation of the form 
\begin{align*}
...
\begin{enumerate}[label=(\roman*)]
  \item Let $(A,\mathfrak{m},k)$  be a local ring and $M$ be an  $A$-module.  If $\set{x_i+\mathfrak{m}M: 1 \leq i \leq n}$ forms a basis for the $k$-vector space $M \quotient \mathfrak{m}M$, then $M= (x_1,\dots ,x_n)$.  
...
\end{align*}
In particular, for all $M_0 \leq M$ and $N_0 \leq N$, given $x\in M_0$ and $y \in N_0$ that makes $x \otimes  y =0$ in $M_0 \otimes  N_0$, the module homomorphism from $M_0 \otimes  N_0$ to $M \otimes  N$ tell us that $x \otimes  y$ must also be $0$ in  $M \otimes  N$. The converse however is not true. That is, given $x \in M_0$ and $y \in N_0$ with $x \otimes  y=0$ in $M \otimes  N$, we don't always have $x \otimes  y =0 $ in $M_0 \otimes  N_0$. \\

In short, if an expression is zero in a tensor products of submodules, then it must be zero in the parent tensor product, but an expression being zero in the parent tensor product can still be nonzero in the tensor product of submodules. 
...
\end{align*}
\end{Example}
However, we do have the following existence result. 
\begin{theorem}
\label{THfgz}
...
\begin{theorem}
\label{THtfg}
\textbf{(Criteria for finite generation under extension and restriction of scalars)} Let $f:A \rightarrow B$ be a ring homomorphism, $M$ an $A$-module and  $N$ a  $B$-module. Then 
\begin{enumerate}[label=(\roman*)]
...
Because for generic $f':M' \rightarrow M''$ and $g':N' \rightarrow N''$, we clearly have:  
\begin{align*}
  (f' \circ f) \otimes  (g' \circ g) =(f' \otimes  g') \circ (f \otimes  g)
\end{align*}
For all $(A,B)$-bimodule $N$, the tensor product $- \otimes_A N$ forms a clearly additive \emph{covariant} functor from $\mathbf{Mod}_A$ to $\mathbf{Mod}_{(A,B)}$ with the action 
\begin{align}
\label{EQtpm}
f\mapsto f \otimes  \id_N
\end{align}
\begin{theorem}
\label{THbpt}
\textbf{(Basic property of tensor products)} Let $M_1, \dots ,M_n$ and $N$ be $A$-modules. Then  
\begin{enumerate}[label=(\roman*)]
...
\end{align*}
called the \textbf{tensor-hom adjunction} that maps $f$ to $f'$, where  $f'(m)(n)\triangleq f(m \otimes n)$. In particular, they are both naturally bijective to the set of bilinear maps from  $M\times N$ to $P$.  
\item The covariant functor  $- \otimes_A N$ is \emph{right exact}. 
\end{enumerate}
\end{theorem}
\begin{proof}
(ii) is routine. We now prove (i). Let $E\leq M_1 \otimes  \cdots \otimes  M_n$ be the submodule generated by basic elements with quotient map $\pi  : M_1 \otimes  \cdots \otimes  M_n \twoheadrightarrow E$. We are required to prove that $\pi=0$. This follows from noting that both $\pi$ and $0: M_1 \otimes  \cdots \otimes  M_n \rightarrow (M_1 \otimes  \cdots \otimes  M_n)\quotient E$ makes the diagram:  
...
\end{tikzcd}\]
commutes. Lastly, we prove (iii). Consider the exact sequence of $A$-modules: 
% https://q.uiver.app/#q=WzAsNCxbMCwwLCJNJyJdLFsyLDAsIk0iXSxbNCwwLCJNJyciXSxbNiwwLCIwIl0sWzAsMSwidSJdLFsxLDIsInYiLDAseyJzdHlsZSI6eyJoZWFkIjp7Im5hbWUiOiJlcGkifX19XSxbMiwzXV0=
\[\begin{tikzcd}
	{M'} && M && {M''} && 0
	\arrow["u", from=1-1, to=1-3]
	\arrow["v", two heads, from=1-3, to=1-5]
	\arrow[from=1-5, to=1-7]
\end{tikzcd}\]
Let $P$ be an  $A$-module, and denote the contravariant functor $\Hom (-,P):\mathbf{Mod}_A \longrightarrow \mathbf{Mod}_A$ by $G_P$. By \customref{THhomp}{property of $G_P$}, we only have to prove 
% https://q.uiver.app/#q=WzAsNCxbMCwwLCIwIl0sWzEsMCwiXFxIb20oTScnIFxcb3RpbWVzIE4sUCkiXSxbMiwwLCJcXEhvbShNXFxvdGltZXMgTixQKSJdLFszLDAsIlxcSG9tKE0nXFxvdGltZXMgTixQKSJdLFswLDFdLFsxLDIsIkdfUCh2XFxvdGltZXMgMSkiXSxbMiwzLCJHX1AodVxcb3RpbWVzIDEpIl1d
\[\begin{tikzcd}
	0 & {\Hom(M'' \otimes N,P)} & {\Hom(M\otimes N,P)} & {\Hom(M'\otimes N,P)}
	\arrow[from=1-1, to=1-2]
	\arrow["{G_P(v\otimes 1)}", from=1-2, to=1-3]
	\arrow["{G_P(u\otimes 1)}", from=1-3, to=1-4]
\end{tikzcd}\]
is exact. Now, because tensor-hom adjunction gives an equivalent sequence: 
\begin{align*}
0 \longrightarrow \Hom  (M'',\Hom (N,P)) \overset{\tilde{v}}{\longrightarrow } \Hom (M, \Hom  (N,P)) \overset{\tilde{u} }{\longrightarrow } \Hom (M',\Hom  (N,P))
\end{align*}
we only have to prove this sequence is exact. Denote  $G_{\Hom (N,P)}\triangleq \Hom  (-, \Hom (N,P))$. The proof then follows from \customref{THhomp}{property of $G_{\Hom  (N,P)}$} and a routine check of $\tilde{v}= G_{\Hom (N,P)}(v)$. 
\end{proof}
...
\end{align*}
\end{Example}
Even though tensor product as a functor lack the functorality we desired, it still has lots of algebraic identities.  
\begin{theorem}
\label{THidt}
\label{THatpb}
...
\item  Let $N$ and  $P$ be two  $A$-modules. We have a natural isomorphism 
  ...
  \end{align*}
\item Let $N$ an  $(A,B)$-bimodule, and $P$ a  $B$-module. We have a natural $(A,B)$-bimodule isomorphism 
\begin{align*}
...
\end{align*}
\item Let $M[x]$ denote the formal abelian group of expression of the form 
  \begin{align*}
  m_0 + m_1 x + \cdots + m_rx^r,\quad \text{ where }m_i \in M
  \end{align*}
Then we can give $M[x]$ a natural $A[x]$-module structure, and it has a natural $A[x]$-module isomorphism 
\begin{align*}
M[x]\cong  A[x]\otimes _A M ,\quad \sum m_i x^i \mapsto \sum x^i \otimes_A m_i
\end{align*}
\end{enumerate}
\end{theorem}
\begin{proof}
(i), (iii), (iv), (v), and (vii) are routine. (ii) is essentially a corollary of (i). In short, because \customref{THbpt}{tensor product is right exact}, tensoring the exact sequence  
\begin{align*}
...
\end{proof}
The rest of this section is dedicated to some non-fundamental results. 
\begin{theorem}
  \textbf{(Tensoring with ground ring make it a direct sum of itself)} Let $f:A \rightarrow B$ be a ring homomorphism, $N$ be a  $B$-module, and $N_B \triangleq B \otimes _A N$. Then we have an $A$-module isomorphism 
\begin{align*}
N_B \cong  N \oplus  N 
\end{align*}
\end{theorem}
\begin{proof}
This is proved by \customref{THsl}{split lemma} and constructing a short exact sequence in $\mathbf{Mod}_A$ that splits: 
\begin{align*}
  0 \longrightarrow N \overset{g}{\longhookrightarrow} N_B \overset{p}{\longtwoheadrightarrow } N \longrightarrow 0
\end{align*}
In particular, one need to check that we have two $A$-module homomorphisms $g:N \rightarrow N_B$ and $p:N_N \rightarrow N$ well-defined by 
 \begin{align*}
g(x)\triangleq 1 \otimes  x \quad \text{ and }\quad p(b \otimes  y)\triangleq by
\end{align*}
Then because they clearly satisfy $p \circ g= \id_N$, the short sequence is exact and splits. 
\end{proof}
\begin{Example}{$g$ in general isn't an $B$-module homomorphism}{}
Let $A\triangleq \R$ and $B \triangleq N\triangleq \C$. Then $g \not \in \mathbf{Mod}_B$, since  
\begin{align*}
...
\begin{theorem}
\textbf{(Interesting results that use tensor and generalized Cayley-Hamilton)} Let $A$ be a ring and  $\pfi : A^m \rightarrow A^n$ be a module homomorphism. If $\pfi $ is injective, then  $m \leq n$. Conversely, if $\pfi$ is surjective, then $m\geq n$.  
\end{theorem}
\begin{proof}
The statement about injectivity is proved via \customref{THgCH}{generalized Cayley-Hamilton theorem}, and the statement about surjectivity is proved by a "tensoring residue field" argument.\\

Assume for a contradiction that $\pfi $ is injective and $m>n$. Consider the injective morphism $(a_1,\dots ,a_n)\mapsto (a_1,\dots ,a_n, 0 ,\dots, 0)$ denoted by  $\diota : A^n \hookrightarrow  A^m$. Then \customref{THbpom}{$\psi  \triangleq \diota \circ \pfi \in \operatorname{End}(A^m) $ is injective}.   Note that \customref{THgCH}{generalized Cayley-Hamilton theorem} implies that 
\begin{align*}
\psi^d + a_1\psi^{d-1} + \cdots + a_d \id_{A^m} =0
\end{align*}
for some minimal $d\inn $ and $a_i \in A$. Because $m>n$, the projection $(a_1,\dots , a_m) \mapsto a_m$ denoted by  $\pi  : A^m\twoheadrightarrow A$ makes $\pi  \circ \psi =0 $. Taking $\pi $ on both side of the above equation then shows that $a_d=0$, which gives us 
\begin{align*}
\psi (\psi^{d-1}+a_1 \psi^{d-2}+ \cdots + a_{d-1}\id_{A^m})=0 
\end{align*}
The contradiction that $\psi$ is not injective then follows from noting that minimality\footnote{If $d=1$, then at the step of $a_d=0$, we already have a contradiction, i.e.,  $\psi=0$.} of $d$ implies that $\psi ^{d-1} + a_1 \psi^{d-2}+ \cdots + a_{d-1}\id_{A^m}$ has nonzero image. \\
...
If $\pfi $ is surjective, then $1 \otimes  \pfi : k \otimes  A^m \twoheadrightarrow  k \otimes  A^n $ is also surjective by \customref{THbpt}{right exactness of tensor product}, where $k\triangleq  A \quotient \mathfrak{m}$ is some residue field. The proof for $m \geq n$ then follows from noting that $k \otimes  A^m$ as a $k$-vector space is  $m$-dimensional\footnote{One way to see such is to use the $A$-module isomorphism $\customref{THidt}{k \otimes  A^m \cong A^m \quotient \mathfrak{m}A^m}$, in which one need to check that the natural  $k$-vector space structure on  $A^m \quotient \mathfrak{m}A^m$ is $m$-dimensional and that the  $A$-module isomorphism is indeed a $k$-vector space morphism.} and that $1 \otimes  \pfi $ is $k$-linear. \\
...
Let $A$ be a ring and $M$ an  $A$-module. Given a multiplicatively closed subset  $S\subseteq A$ containing $1$, we say an $A$-module homomorphism $M \longrightarrow S^{-1}M$ satisfies the \textbf{universal property of localization by $S$} if 
\begin{enumerate}[label=(\roman*)]
  \item Every $s \in S$ acts invertibly on $S^{-1}M$. 
  \item For all $A$-module  $N$ on which all  $s \in S$ acts invertibly and any $A$-module homomorphism  $f:M \rightarrow N$ there exists a unique $A$-module homomorphism  $\tilde{f}$ that makes the diagram 
% https://q.uiver.app/#q=WzAsMyxbMCwwLCJNIl0sWzIsMCwiU157LTF9TSJdLFsyLDIsIk4iXSxbMCwyLCJmIiwyXSxbMCwxXSxbMSwyLCJcXHRpbGRle2Z9IiwxLHsic3R5bGUiOnsiYm9keSI6eyJuYW1lIjoiZGFzaGVkIn19fV1d
\[\begin{tikzcd}
	M && {S^{-1}M} \\
	\\
	&& N
	...
	\arrow["f"', from=1-1, to=3-3]
	\arrow["{\tilde{f}}"{description}, dashed, from=1-3, to=3-3]
\end{tikzcd}\]
\end{enumerate}
Clearly, we can construct $S^{-1}M$ to be the quotient of $S \times M$ by setting 
\begin{align*}
\frac{m}{s}= \frac{\tilde{m}}{\tilde{s}}\overset{\triangle}{\iff }u(m \tilde{s}- \tilde{m}s  )=0,\quad \text{ for some }u \in S
\end{align*}
And we can see that localization forms an exact functor 
\begin{align*}
S^{-1}f\left( \frac{m}{s} \right) \triangleq \frac{f(m)}{s}
\end{align*}
In particular, we can give $S^{-1}A$ a natural ring structure, and all $S^{-1}M$ have a natural $S^{-1}A$-module structure. Interestingly, we have a natural $S^{-1}A$-module isomorphism  
\begin{align*}
S^{-1}M \cong  S^{-1}A \otimes _A M,\quad \frac{m}{s}\longmapsto \frac{1}{s} \otimes_A  m  
\end{align*}
...
\begin{Example}{}{}
Consider $6, 2 (1+ \sqrt{5}i) \in \Z [\sqrt{-5}]$. $6$ has divisors $1,2,3,1\pm \sqrt{5}i$, so they have common divisors  $2,1+ \sqrt{5}i$, but no greatest common divisor. 
\end{Example}
...
% \begin{abstract}
%   For obvious reason, an \emph{intrinsic} treatment of homological algebra must be fully categorical in the sense that one has no need to ever mention any specific category in their path to a definition of \textbf{abelian categories} or even to a definition of \textbf{derived functors}. \\ 
%
% Such treatment is more or less undesirable. In practice, most of us care about only a very few categories, which makes the effort needed to verify that their statements for a specific category or functor really can be translated back into the purely categorical statement very painful. \\
%
% For example, to avoid such pain, Hartshorne defines a sequence of sheaves to be exact if and only if the sequence is exact on every open set. Obviously, this is not the "word-to-word" definition of exact sequence in general abelian categories, and there is a moderately long path to translate his definition to the definition in general abelian categories, which, when accumulated with other similar situations, is an effort not all of us need to go through.  \\
%
%   Therefore, here we take the approach of starting things rigorously and intrinsic until ?, in which we break the rigor and allow the reader to \emph{interpret the statement however they need}. The justification is, not every one need every construction, and even if they do, not every of them wishes to see a proof, and, more importantly, even if they do, \emph{our stated theorem offers a path for them to come up with a proof} with enough patient spent.  \\
%
%
%   Note that in general we don't care about set-theoretic concerns. Jan 13 2026, Eric Liu. 
% \end{abstract}
...
\begin{enumerate}[label=(\roman*)]
  \item For all $A \in \mathcal{C}$, there exists an \textbf{identity morphism} $1_A \in \operatorname{End}(A)$ that makes $f=f \circ 1_A$ and $g=1_B \circ g$ for all $f: A \rightarrow B$ and $g:B \rightarrow C$. \textbf{(Identity)}
...
f \circ g= 1_B
\end{align*}
If simultaneously $g$ is both a left and right inverse of  $f$, then we say $f$ and $g$ are  \textbf{inverse to each other}, that they are both \textbf{isomorphisms}, and $A,B$ are  \textbf{isomorphic}. Let $\mathcal{C}$ be a category. A morphism $f: A \hookrightarrow  B$ is said to be a \textbf{monomorphism} if for all $g,h: C \rightarrow A$, we have 
\begin{align*}
...
\begin{theorem}
\label{THbpom}
\textbf{(Basic properties of morphisms)} Let $\mathcal{C}$ be a category and $f: A \rightarrow B$ be a morphism in $\mathcal{C}$. Then 
\begin{enumerate}[label=(\roman*)]
...
Routine. 
\end{proof}
Let $B \in \mathcal{C}$. A \textbf{subobject of $B$} is an object $A$ together with a monomorphism  $\diota: A \hookrightarrow  B $, and a \textbf{quotient of $B$} is an object $C$ with an epimorphism $\pi  : B \twoheadrightarrow C $. An \textbf{initial object} $I \in \mathcal{C}$ is an object such that for all $A \in \mathcal{C}$, there exists exactly one morphism from $I$ to  $A$. A \textbf{terminal object} $T\in \mathcal{C}$ is an object such that for all $A\in \mathcal{C}$, there exists exactly one morphism from $A$ to  $T$. Clearly, initial objects and terminal objects, even though may not always exists, are unique up to isomorphisms. A \textbf{zero object} is an object that is both an initial and terminal object. \\ 
...
that satisfies  
\begin{align*}
F(1 _A)=1_{F(A)}\quad \text{ and }\quad F(g\circ h)= F(g)\circ F(h)
\end{align*}
...
\end{tikzcd}\]
commutes. A cone $(\lim F,\pfi )$ to $F$ is said to satisfy the \textbf{universal property of limit} if for all cones $(N,\psi )$ to $F$ there exists a unique morphism $u: N \rightarrow \lim F$ that makes the diagram 
...
\end{tikzcd}\]
commutes for all $f:X \rightarrow Y$. A \textbf{directed set} is a partially ordered set in which every two elements have an upper bound. Let $I$ be a directed set and $\mathcal{C}$ a category. By an \textbf{inverse system in} $\mathcal{C}$, we mean a collection of object $A_i$ in $\mathcal{C}$ indexed by $I$ together with each relation $i \leq j$ (\emph{note the order}) a morphism $f_{ij}:A_j \rightarrow A_i$ in $\mathcal{C}$ such that: 
\begin{enumerate}[label=(\roman*)]
  \item $f_{ii}$ are the identities on $A_i$.  
  \item $f_{ik}=f_{ij}\circ f_{jk}$  for all $i\leq j \leq k$
\end{enumerate}
Clearly, an inverse system in $\mathcal{C}$ itself forms a category $\mathcal{I}$ the obvious way, and we have an obvious covariant functor $F:\mathcal{I}\rightarrow \mathcal{C}$. The limit of $F$ is then called the  \textbf{inverse limit of $A_i$} denoted by $\inverselimit A_i$ and $f_i:\inverselimit A_i \rightarrow A_i$.\\ 
...
\end{tikzcd}\]
commute. A co-cone $(\colim F,\pfi )$ to $F$ is said to satisfy the \textbf{universal property of colimit} if for all co-cones $(N,\psi )$ to $F$ there exists a unique morphism $u: \colim F \rightarrow N$ that makes the diagram 
...
\end{tikzcd}\]
commute for all $f:X \rightarrow Y$. Let $I$ be a directed set. By a \textbf{directed system in} $\mathcal{C}$, we mean a collection of object $A_i$ in $\mathcal{C}$ indexed by $I$ together with each relation $i \leq j$ (again, \emph{note the order}) a morphism $f_{ij}:A_i \rightarrow A_j$ in $\mathcal{C}$ such that: 
\begin{enumerate}[label=(\roman*)]
  \item $f_{ii}$ are the identities on $A_i$.  
  \item $f_{ik}=f_{jk}\circ f_{ij}$  for all $i\leq j \leq k$
\end{enumerate}
Clearly, a directed system in $\mathcal{C}$ forms a category $\mathcal{I}$ the obvious way, and we have an obvious covariant functor $F:\mathcal{I}\rightarrow \mathcal{C}$. The colimit of $F$ is then called the  \textbf{direct limit of $A_i$} denoted by $\directlimit A_i$ and $f_i: A_i \rightarrow \directlimit A_i$. \\
...
\begin{theorem}
\textbf{(Existence of products and coproduct in familiar categories)} Let $A$ be a ring. Product and coproduct always exists 
\begin{enumerate}[label=(\roman*)]
  \item in $\mathbf{Set}$ and $\mathbf{Top}$, in the form of Cartesian product and disjoint union.  
  \item in $\mathbf{Gp}$, in the form of direct product and free product. 
  \item in $\mathbf{Ring}$, in the form of direct product and tensor product as  $\Z$-algebras.  
  \item in $\mathbf{Mod}_A$ and $\mathbf{Ab}$, in the form of direct product and direct sum. 
\end{enumerate}
\end{theorem}
\begin{proof}
Routine. 
\end{proof}
Note that by the \textbf{free product $A_1 * A_2$ of two groups}, we mean the group defined by presentation 
\begin{align*}
...
\begin{theorem}
\label{THflt}
\textbf{(Finite limit theorem)} Let $\mathcal{C}$ be a category. Then the following two statements are equivalent 
\begin{enumerate}[label=(\roman*)]
  \item All finite (including the arrows) diagrams in $\mathcal{C}$ have a limit. 
  \item All finite product and equalizer exist.  
\end{enumerate}
The following two statements are also equivalent 
\begin{enumerate}[label=(\roman*)]
  \item All finite (including the arrows) diagrams in $\mathcal{C}$ have a colimit. 
  \item All finite coproduct and coequalizer exist.  
\end{enumerate}
\end{theorem}
\begin{proof}
Because the proofs for two equivalence are dual to each other, we only prove the first one. Clearly (i)$\implies $(ii). We now prove  (ii)$\implies $(i).   Let a diagram in  $\mathcal{C}$ consists of objects $M_i$ indexed by  $ i\in I$ and arrows $a$ indexed by $A$. For all $a \in A$, denote its source and target by $s(a)$ and $t(a)$. Denote the canonical morphism by 
\begin{align*}
p_i:\prod _{i \in I}M_i \rightarrow M_i\quad \text{ and }\quad \tilde{p}_a : \prod _{a\in A}M_{t(a)}\rightarrow M_{t(a)} 
\end{align*}
By universal property, we then know there exist $\pfi  , \psi: \prod_{i \in I}M_i \rightarrow \prod_{a \in A}M_{t(a)}$ that makes 
\begin{align*}
\tilde{p}_{a}\circ \pfi  = p_{t(a)} \quad \text{ and }\quad \tilde{p}_{a} \circ \psi = a \circ p_{s(a)}
\end{align*}
The proof then follows from verifying that the equalizer  $\operatorname{eq}(\pfi ,\psi)$ together with $p\circ _i\circ  h$ satisfies the universal property of limit of the diagram, where $h:\operatorname{eq}(\pfi ,\psi)\rightarrow \prod_{i \in I}M_i$ is the canonical morphism of equalizer. 
\end{proof}
...
\end{align*}
where $A_1*A_2$. In $\mathbf{Ab}$ and $\mathbf{Mod}_R$, their fiber coproduct can be constructed as a quotient of their direct product    
\begin{align*}
...
Routine. 
\end{proof}
Given two covariant functors $F,F':\mathcal{C} \rightarrow \mathcal{D}$, by a \textbf{natural transformation} $\eta  :F\rightsquigarrow F'$, we mean a class of morphisms $\eta_A:F(A)\rightarrow F'(A)$ in $\mathcal{D}$ indexed by $\mathcal{C}$ that makes the diagram: 
% https://q.uiver.app/#q=WzAsNCxbMCwwLCJGKEEpIl0sWzIsMCwiRicoQSkiXSxbMCwyLCJGKEIpIl0sWzIsMiwiRicoQikiXSxbMCwxLCJcXGV0YV9BIl0sWzIsMywiXFxldGFfQiJdLFswLDIsIkYoZikiLDJdLFsxLDMsIkYnKGYpIl1d
\[\begin{tikzcd}
	{F(A)} && {F'(A)} \\
	\\
	{F(B)} && {F'(B)}
...
\end{tikzcd}\]
commutes for all $f:A \rightarrow B$ in $\mathcal{C}$. The \textbf{functor category} $\operatorname{Func}(\mathcal{C},\mathcal{D})$ is then the category whose objects is the class of covariant functors from $\mathcal{C}$ to $\mathcal{D}$ and whose morphisms are natural transformations. We say a category $\mathcal{C}$ is \textbf{small} if the class $\operatorname{ob}(\mathcal{C} )$ is a set.   \\

\section{Additive Categories}
An \textbf{additive category} $\mathcal{A} $ is a category such that $\Hom  (A,B)$ is an abelian group (thus a set) for all $A, B \in \mathcal{A}$ that satisfies 
\begin{align*}
  (f_1+ f_2 )\circ g= f_1 \circ g+ f_2 \circ g \quad \text{ and }\quad f \circ (g_1 +g_2)= f \circ g_1 + f \circ g_2
\end{align*}
and 
\begin{enumerate}[label=(\roman*)]
\item $\mathcal{A}$ admits zero object.  
\item $\mathcal{A}$ admits \emph{finite} coproducts.  
\end{enumerate}
Let $f:A\rightarrow B$ be a morphism in an additive category $\mathcal{A}$. We say a morphism $\diota:  \operatorname{ker}(f)\hookrightarrow  A$ satisfies the \textbf{universal property of kernel} if 
\begin{enumerate}[label=(\roman*)]
  \item $f \circ \diota=0$. 
  \item For all  $h:C \rightarrow A$ that makes $f\circ h=0$, there exists a unique morphism $u:C\rightarrow \operatorname{ker}(f)$ that makes the diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{\operatorname{ker}(f)} && A && B \\
	\\
	{C}
	\arrow["\diota", hook, from=1-1, to=1-3]
...
commutes. 
\end{enumerate}
We say a morphism $\pi :B \twoheadrightarrow  \operatorname{Coker}(f)$ satisfies the \textbf{universal property of cokernel} if 
\begin{enumerate}[label=(\roman*)]
  \item $\pi  \circ f=0$. 
  \item For all $h : B \rightarrow C$ that makes $h \circ f=0$, there exists a unique morphism $u:\operatorname{Coker}(f)$ that makes the diagram: 
% https://q.uiver.app/#q=WzAsNCxbMCwwLCJBIl0sWzIsMCwiQiJdLFs0LDAsIlxcb3BlcmF0b3JuYW1le0Nva2VyfShmKSJdLFs0LDIsIkMiXSxbMCwxLCJmIl0sWzEsMiwiXFxwaSIsMCx7InN0eWxlIjp7ImhlYWQiOnsibmFtZSI6ImVwaSJ9fX1dLFsxLDMsImgiLDJdLFsyLDMsInUiLDEseyJzdHlsZSI6eyJib2R5Ijp7Im5hbWUiOiJkYXNoZWQifX19XV0=
\[\begin{tikzcd}
	A && B && {\operatorname{Coker}(f)} \\
	\\
	&&&& C
	\arrow["f", from=1-1, to=1-3]
	\arrow["\pi", two heads, from=1-3, to=1-5]
	\arrow["h"', from=1-3, to=3-5]
	\arrow["u"{description}, dashed, from=1-5, to=3-5]
\end{tikzcd}\]
commutes. 
...
\begin{theorem}
\label{THkar}
\textbf{(Basic properties of additive categories)} Let $\mathcal{A}$ be an additive category. Then 
\begin{enumerate}[label=(\roman*)]
  \item Let $A$ and $B$ be a pair of objects in  $\mathcal{A}$, and $Z$ be a zero object in  $\mathcal{A}$ with  
\begin{align*}
A \overset{f}{\longrightarrow }   Z \overset{g}{\longrightarrow } B
\end{align*}
Then $g\circ f$ is the zero in the abelian group $\Hom (A,B)$.  
  \item Given a \emph{finite} collection of objects $A_1,\dots ,A_n \in \mathcal{A}$ and a object $A$ together with a collection of morphism $A_i \overset{\diota_i}{\longrightarrow }A \overset{p_i}{\longrightarrow }A_i$ that satisfies 
\begin{align*}
p_i \circ \diota_j \triangleq \begin{cases}
  \id_{A_i}& \text{ if $j=i$ }\\
  0& \text{ if $j\neq i$ }
\end{cases}  \quad \text{ and }\quad \sum \diota_i \circ p_i = \id_A
\end{align*}
then the morphisms $\diota_i$ and $p_i$ respectively satisfies the universal property of coproduct and product. In particular, any finite product and finite coproduct can be made a coproduct and product this way.    
\item $Z\in \mathcal{A}$ is a zero object $\iff \id_{Z}$ is the zero in $\Hom  (Z,Z)$.   
\item Canonical morphisms of \emph{finite} products and coproducts in $\mathcal{A}$ are respectively epimorphisms and monomorphisms. 
\item  Let $f:A \rightarrow B$ be a morphism in $\mathcal{A}$ that has both $\diota: \operatorname{ker}(f)\hookrightarrow A $ and $\pi  : B \twoheadrightarrow \operatorname{Coker}(f)$. Then $\diota$ is indeed a monomorphism, and $\pi $ is indeed an epimorphism.  
\end{enumerate}
\end{theorem}
\begin{proof}
They are all routine. Note that (iv) follows from (ii) and the fact that \customref{THbpom}{left inverses are epimorphisms and right inverses are monomorphisms}.
\end{proof}



Clearly, given an additive category $\mathcal{A}$ and a small category $\mathcal{C}$, the functor category $\operatorname{Func}(\mathcal{C},\mathcal{A})$ form an additive category the obvious way. Also, if all kernels and cokernels exists in $\mathcal{A}$, we may construct kernels and cokernels in $\operatorname{Func}(\mathcal{C},\mathcal{A})$ component wise.  \\



Let $\mathcal{C}$ and $\mathcal{D}$ be categories. A \textbf{contravariant functor} $G: \mathcal{C}\rightarrow \mathcal{D}  $ is a map of objects $G: \operatorname{ob}(\mathcal{C} )\rightarrow \operatorname{ob}(\mathcal{D} )$ together with a map of morphisms: 
\begin{align*}
...
\end{align*}
Let $F: \mathcal{A}\rightarrow \mathcal{B}$ and $G:\mathcal{A}\rightarrow \mathcal{B}$ be respectively covariant and contravariant functors between additive categories. We say $F$ and $G$ are \textbf{additive} if for all $A,B \in \mathcal{A}$, respectively, the map $\Hom _{\mathcal{A} }(A,B) \rightarrow \Hom _\mathcal{B}(F(A),F(B))$ and the map $\Hom_{\mathcal{A} }(A,B)\rightarrow \Hom _{\mathcal{B} }(G(B),G(A)) $ are group homomorphisms.
\begin{theorem}
\textbf{(Basic property of additive functor)} \label{THbpadf} Let $F: \mathcal{A}\rightarrow \mathcal{B}$ and $G: \mathcal{A}\rightarrow \mathcal{B}$ be additive covariant and additive contravariant additive functor between additive categories. Then  
\begin{enumerate}[label=(\roman*)]
  \item $F$ and  $G$ both map zero object to zero objects.  
  \item If \emph{finite} $A_i \longrightarrow A$ satisfies universal property of coproduct, then $F(A_i)\longrightarrow F(A)$ satisfies universal property  of coproduct. Dually, if \emph{finite} $A \longrightarrow A_i$  satisfies universal property of product, then $F(A)\longrightarrow F(A_i)$ satisfies universal property  of product.
\end{enumerate}
\end{theorem}
\begin{proof}
  (i) follows from the \customref{THkar}{characterization of zero objects in additive categories}. (ii) and (iii) follows from the \customref{THkar}{characterization of biproduct}. 
\end{proof}
...
Suppose the \emph{additive} category $\mathcal{A}$ admits all kernels and cokernels. We then define the \textbf{image} and \textbf{coimage} of a morphism $f: A \rightarrow B$ to be 
\begin{align*}
\operatorname{Im}(f)\triangleq  \operatorname{ker}(B \twoheadrightarrow  \operatorname{Coker}(f))\quad \text{ and }\quad \operatorname{Coim}(f)\triangleq  \operatorname{Coker}(\operatorname{ker}(f)\hookrightarrow   A)
\end{align*}
In other word, given the canonical morphisms: 
\begin{align*}
\operatorname{ker}(f) \overset{\diota }{\longhookrightarrow } A \overset{f}{\longrightarrow }B \overset{\pi }{\longtwoheadrightarrow } \operatorname{Coker}(f)
\end{align*}
The morphism $\pi': A \twoheadrightarrow  \operatorname{Im}(f)$ satisfies the \textbf{universal property of coimages} if 
\begin{enumerate}[label=(\roman*)]
  \item $\pi '\circ \diota=0 $. 
  \item For all $h: A \rightarrow C$ that makes $h \circ \diota =0 $, there exists a unique morphism $u:\operatorname{Coim}(f)\rightarrow C$ that makes the diagram:   
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{\operatorname{ker}(f)} && A && {\operatorname{Coim}(f)} \\
	\\
	&&&& C
	\arrow["\diota", hook, from=1-1, to=1-3]
	\arrow["{\pi'}", two heads, from=1-3, to=1-5]
...
commutes. 
\end{enumerate}
and the morphism $\diota' : \operatorname{Im}(f)\hookrightarrow B$ satisfies the \textbf{universal property of image} if 
\begin{enumerate}[label=(\roman*)]
  \item $\pi  \circ \diota' =0$. 
  \item For all $h:C \rightarrow B$ that makes $\pi  \circ h =0 $, there exists a unique morphism $u: C \rightarrow \operatorname{Im}(f)$ that makes the diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{\operatorname{Im}(f)} && B && {\operatorname{Coker}(f)} \\
	\\
	C
	\arrow["{\diota'}", hook, from=1-1, to=1-3]
	\arrow["\pi", two heads, from=1-3, to=1-5]
...
\begin{theorem}
\label{THeauc}
\textbf{(Existence and uniqueness of the canonical morphism from coimage to image)} Let $\mathcal{A}$ be an additive category that admits all kernels and cokernels and $f: A \rightarrow  B$ be a morphism in $\mathcal{A}$. Then there exists a unique morphism $u: \operatorname{Coim}(f)\rightarrow \operatorname{Im}(f)$ that makes the diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	A && B \\
	\\
	{\operatorname{Coim}(f)} && {\operatorname{Im}(f)}
	\arrow["f", from=1-1, to=1-3]
	\arrow["{\pi '}"', two heads, from=1-1, to=3-1]
	\arrow["u"', from=3-1, to=3-3]
	\arrow["{\diota'}"', hook, from=3-3, to=1-3]
\end{tikzcd}\]
commute. 
\end{theorem}
\begin{proof}
Note that the uniqueness of $u$ follows from \customref{THkar}{the fact that $\diota'$ is a monomorphism and $\pi '$ is an epimorphism}. We now prove its existence. Because $f \circ  \diota =0$, by universal property of coimage, there exists a unique morphism $u':\operatorname{Coim}(f)\rightarrow B$ such that $f=u' \circ \pi '$. Therefore, we can deduce 
\begin{align*}
\pi  \circ  f =0  \implies  \pi  \circ u' \circ \pi'  =0 
\end{align*}
Then, because $0\circ \pi'=0$ and because \customref{THkar}{$\pi '$ is an epimorphism}, we see $\pi  \circ u'=0$. This then by universal property of images implies the existence of some $u: \operatorname{Coim}(f)\rightarrow \operatorname{Im}(f)$ that makes $u'=\diota' \circ  u$. Therefore, 
\begin{align*}
f= u' \circ \pi ' = \diota ' \circ  u \circ  \pi '  
\end{align*}
as desired. 
\end{proof}
We say $f$ is \textbf{strict} if such canonical morphism $u: \operatorname{Coim}(f)\rightarrow \operatorname{Im}(f)$ is an isomorphism, and we say this  \emph{additive} category $\mathcal{A}$ is \textbf{abelian} if all of its morphisms are strict. Note that clearly, given an abelian category $\mathcal{A}$ and an small category $\mathcal{C}$, the functor category $\operatorname{Func}(\mathcal{C},\mathcal{A})$ is also abelian, and the image and coimages can be constructed componentwise. 
\begin{theorem}
\label{THbpac}
\textbf{(Basic properties of abelian categories)} Let $\mathcal{A}$ be an abelian category. Then 
\begin{enumerate}[label=(\roman*)]
  \item All finite (including the arrows) diagrams in $\mathcal{A}$ have limits and colimits.  
  \item Fiber products of $f_1:A_1 \rightarrow B$ and $f_2 : A_2 \rightarrow B$ exist in $\mathcal{A}$ in the form of
\begin{align*}
A_1 \times_B A_2 \triangleq \operatorname{ker}\left((f_1,-f_2):A_1 \oplus  A_2 \rightarrow B \right)
\end{align*}
\item Fiber coproducts of $g_1:B \rightarrow A_1$ and $g_2:B \rightarrow A_2$ exists in $\mathcal{A}$ in the form of 
  \begin{align*}
  A_1+_B A_2\triangleq \operatorname{Coker}((g_1,-g_2): B \rightarrow A_1 \oplus A_2 ) 
  \end{align*}
\end{enumerate}
\end{theorem}
\begin{proof}
  (i) is a corollary of \customref{THflt}{finite limit theorem}, in which one construct equalizer and coequalizer by 
  \begin{align*}
  \operatorname{eq}(f,g)\triangleq \operatorname{ker}(f-g)\quad \text{ and }\quad \operatorname{coeq}(f,g)\triangleq \operatorname{Coker}(f-g)
  \end{align*}
(ii) and (iii) are routine checking. 
\end{proof}
 \begin{theorem}
\label{THeauh}
 \textbf{(Existence and uniqueness of the canonical morphism for homologies)} Let $\mathcal{A}$ be an abelian category in which we have morphisms $f:A \rightarrow B$ and $g: B \rightarrow C$ that satisfies $g \circ f=0$. Denote the canonical morphisms of coimage and image of $f$ by $\pi ' : A \twoheadrightarrow \operatorname{Coim}(f)$ and $\diota ' : \operatorname{Im}(f)\hookrightarrow  B $, and let $u: \operatorname{Coim}(f)\rightarrow \operatorname{Im}(f)$ be the unique isomorphism that satisfies $f=\diota' \circ  u \circ \pi' $, which \customref{THeauc}{we have proved to exist uniquely}. Denote $\tilde{f}\triangleq u \circ \pi ': A \rightarrow \operatorname{Im}(f)$. Then there exists a unique morphism  $\tilde{u}:\operatorname{Im}(f)\rightarrow \operatorname{ker}(g)$ such that     
\begin{align*}
f= \diota \circ  \tilde{u} \circ \tilde{f}   
\end{align*}
where $\diota : \operatorname{ker}(g)\rightarrow B $ is the canonical morphism of kernel of $g$. 
\end{theorem}
\begin{proof}
  \customref{THbpom}{Because $u$ is an isomorphism and  $\pi '$ is an epimorphism, we know $\tilde{f}$ is an epimorphism}. The uniqueness of $\tilde{u}$ then follows from the fact that $\diota$ is a monomorphism. We now prove its existence.\\

Denote the canonical morphism of cokernel of $f$ by  $\pi  :B \twoheadrightarrow \operatorname{Coker}(f)$. Because  $g \circ f=0$, by the universal property of cokernel of $f$, there exists a unique morphism $\tilde{g}: \operatorname{Coker}(f)\rightarrow C$ that makes $g= \tilde{g}\circ \pi$. Therefore, because $\pi  \circ \diota ' =0$ by definition of image of $f$, we have:
\begin{align*}
g \circ \diota ' = \tilde{g} \circ \pi  \circ \diota' =    g \circ  0 =0 
\end{align*}
This then by universal property of kernel of $g$ induce a unique morphism $\tilde{u} :\operatorname{Im}(f)\rightarrow \operatorname{ker}(g)$ that makes $ \diota' = \diota \circ \tilde{u}$. By premise, we now see 
\begin{align*}
\diota \circ  \tilde{u} \circ  \tilde{f} = \diota' \circ u \circ \pi '  = f
\end{align*}
as desired. 
\end{proof}
 Let $\mathcal{A}$ be an abelian category in which we have morphisms $f:A \rightarrow B$ and $g: B \rightarrow C$ that satisfies $g \circ f=0$. The \textbf{(co) homology} at this point is then defined to be $\operatorname{Coker}(\tilde{u})$ where $\tilde{u}:\operatorname{Im}(f)\rightarrow \operatorname{ker}(g)$ is from \myref{theorem}{THeauh}.   \\

We shall make an important remark here. Let $A$ be a ring. For completeness, the theorems in the rest of this chapter will still be stated in the language of abelian categories, but the proof will only be done for  $\mathbf{Mod}_A$, since a proof for the latter can always be easily modified to be a proof for the former. Moreover, for the sake of style, we will use $\oplus $ in place of $\coprod $. 
\begin{theorem}
\label{THsl}
\textbf{(Split lemma)} Let  
\begin{align*}
0 \longrightarrow A  \overset{q}{\longhookrightarrow } B \overset{r}{\longtwoheadrightarrow   } C \longrightarrow 0
\end{align*}
be a short exact sequence in an abelian category $\mathcal{A}$. The followings are equivalent: 
\begin{enumerate}[label=(\roman*)]
  ...
  \item There exists an isomorphism $h:B \rightarrow A \oplus   C$ such that $h \circ  q:A \rightarrow A \oplus   C$ and $r \circ h^{-1}: A \oplus   C \rightarrow C$ are the canonical morphism.  \textbf{(Direct sum)}
\end{enumerate}
\end{theorem}
\begin{proof}
Routine. 
...
\begin{theorem}
\label{THsnake}
\textbf{(Snake lemma)} Let 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&& {M'} && M && {M''} && 0 \\
	\\
	0 && {N'} && N && {N''}
	...
	\arrow["{v'}"', from=3-5, to=3-7]
\end{tikzcd}\]
be a commutative diagram in an abelian category $\mathcal{A}$ with rows exact. Then we have a \emph{canonical} exact sequence:  
\begin{align*}
   \operatorname{ker}(f') \overset{u}{\longrightarrow  } \operatorname{ker}(f) \overset{v}{\longrightarrow } \operatorname{ker}(f'')\overset{d}{\longrightarrow }\operatorname{Coker}(f') \overset{\tilde{u}'}{\longrightarrow }\operatorname{Coker}(f) \overset{\tilde{v}' }{\longrightarrow  }\operatorname{Coker}(f'')
\end{align*}
where 
\begin{enumerate}[label=(\roman*)]
  \item The morphism $u: \operatorname{ker}(f')\rightarrow \operatorname{ker}(f)$ is injective if $u:M '\rightarrow M$. 
  \item The morphism $\tilde{v}': \operatorname{Coker}(f)\rightarrow \operatorname{Coker}(\tilde{f}'' ) $ is surjective if $v': N ' \rightarrow N''$ is. 
\end{enumerate}
...
\end{theorem}
\begin{proof}
Routine. Here, we give an outline for inducing of the \textbf{boundary homomorphism} $d: \operatorname{ker}(f'')\rightarrow \operatorname{Coker}(f')$. Let $x'' \in \operatorname{ker}(f'')$. Because $v$ is surjective, we know  $x''=v(x)$ for some $x \in M$. Therefore, $v' \circ  f(x)=f''\circ  v(x)=0$, which implies $f(x)\in \operatorname{ker}(v')=\operatorname{Im}(u')$, that is, $f(x)=u'(y')$ for some $y' \in N'$. The boundary homomorphism can be induced by 
\begin{align*}
d(x'')\triangleq y' + \operatorname{Im}(f')
\end{align*}
\end{proof}
Let $\mathcal{A}$ be an abelian category and $\mathcal{C}$ be a subclass of $\mathcal{A}$ that contains a zero object. A function $\ld :\mathcal{C}\rightarrow \Z$ is said to be \textbf{additive} if for all short exact sequence 
\begin{align*}
0 \longrightarrow A' \longhookrightarrow A \longtwoheadrightarrow A'' \longrightarrow 0
\end{align*}
in $\mathcal{C}$, the function $\ld$ satisfies 
\begin{align*}
\ld (A')- \ld  (A) + \ld (A'')=0 \quad \text{ and }\quad \ld (0)=0 
\end{align*}
\begin{theorem}
\textbf{(Additive functions on long exact sequences)} Let $\mathcal{A}$ be an abelian category, $\mathcal{C}$ a subclass of $\mathcal{A}$ that contains a zero object, and an additive function $\ld : \mathcal{C}\rightarrow \Z $. Given an exact sequence  
 \begin{align*}
0\longrightarrow M_0 \longrightarrow M_1 \longrightarrow \cdots \longrightarrow M_n  \longrightarrow 0
\end{align*}
whose all terms, including the kernels, are in $\mathcal{C}$, we have  
\begin{align*}
...
\end{align*}
\end{proof}
\section{Homologies and Cohomologies}
Let $\mathcal{A}$ be an abelian category. A \textbf{chain} $A_{\bullet}$ in $\mathcal{A}$ is a sequence of the form: 
% https://q.uiver.app/#q=WzAsNSxbNCwwLCJBX2kiXSxbMiwwLCJBX3tpLTF9Il0sWzYsMCwiQV97aSsxfSJdLFswLDAsIlxcY2RvdHMiXSxbOCwwLCJcXGNkb3RzIl0sWzAsMSwiZF9pIiwyXSxbMiwwLCJkX3tpKzF9IiwyXSxbMSwzXSxbNCwyXV0=
\[\begin{tikzcd}
	\cdots && {A_{i-1}} && {A_i} && {A_{i+1}} && \cdots
	\arrow[from=1-3, to=1-1]
	\arrow["{d_i}"', from=1-5, to=1-3]
	\arrow["{d_{i+1}}"', from=1-7, to=1-5]
	\arrow[from=1-9, to=1-7]
\end{tikzcd}\]
in which $d_{i}\circ d_{i+1}=0$ for all $i\inz$, and a \textbf{cochain} $A^{\bullet}$ in $\mathcal{A}$ is a sequence of the form:  
% https://q.uiver.app/#q=WzAsNSxbNCwwLCJBXmkiXSxbMiwwLCJBXntpLTF9Il0sWzYsMCwiQV57aSsxfSJdLFswLDAsIlxcY2RvdHMiXSxbOCwwLCJcXGNkb3RzIl0sWzEsMCwiZF57aS0xfSJdLFswLDIsImReaSJdLFszLDFdLFsyLDRdXQ==
\[\begin{tikzcd}
	\cdots && {A^{i-1}} && {A^i} && {A^{i+1}} && \cdots
	\arrow[from=1-1, to=1-3]
	\arrow["{d^{i-1}}", from=1-3, to=1-5]
	\arrow["{d^i}", from=1-5, to=1-7]
	\arrow[from=1-7, to=1-9]
\end{tikzcd}\]
in which $d^i \circ d^{i-1}=0$ for all $i\inz$. The \textbf{homology} $H_\bullet(A_\bullet)$ and the \textbf{cohomology} $H^\bullet (A^\bullet)$ are then defined by: 
\begin{align*}
H_i(A_\bullet)\triangleq \frac{\operatorname{ker}(d_i)}{\operatorname{Im}(d_{i+1})}\quad \text{ and }\quad H^i(A^\bullet) \triangleq \frac{\operatorname{ker}(d^i)}{\operatorname{Im}(d^{i-1})}
\end{align*}
Clearly, we can make the class $\mathbf{Ch}(\mathcal{A})$ of chains in $\mathcal{A}$ an abelian category by requiring the morphisms $f_{\bullet}:A_\bullet \rightarrow B_\bullet $ to be sets $f_i:A_i \rightarrow B_i$ of morphisms that makes 
\begin{align*}
d_i \circ f_i =  f_{i-1} \circ d_i 
\end{align*}
In other words, $f_i$ make the diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	\cdots && {A_{i-1}} && {A_i} && {A_{i+1}} && \cdots \\
	\\
	\cdots && {B_{i-1}} && {B_i} && {B_{i+1}} && \cdots
	\arrow[from=1-3, to=1-1]
	\arrow["{f_{i-1}}"{description}, from=1-3, to=3-3]
	\arrow["{d_i}"', from=1-5, to=1-3]
	\arrow["{f_i}"{description}, from=1-5, to=3-5]
	\arrow["{d_{i+1}}"', from=1-7, to=1-5]
	\arrow["{f_{i+1}}"{description}, from=1-7, to=3-7]
	\arrow[from=1-9, to=1-7]
	\arrow[from=3-3, to=3-1]
	\arrow["{d_i}", from=3-5, to=3-3]
	\arrow["{d_{i+1}}", from=3-7, to=3-5]
	\arrow[from=3-9, to=3-7]
\end{tikzcd}\]
commute. Similarly, we have the abelian category $\mathbf{cCh}(\mathcal{A})$ of cochains in $\mathcal{A}$ in which the morphisms  $f^\bullet : A^\bullet \rightarrow B^\bullet$ satisfies: 
\begin{align*}
d_i \circ f_i = f_{i+1}\circ d_i
\end{align*}
Given a morphism $f_\bullet: A_\bullet \rightarrow B_\bullet$ between two chains, clearly, for each $i$ we have a natural morphism 
\begin{align*}
f_i^* : H_i (A_\bullet)\rightarrow H_i (B_\bullet)\quad \text{ and }\quad f_i^*(\overline{x})\triangleq \overline{f_i(x)}
\end{align*}
Similarly, given a morphism $f^\bullet: A^\bullet\rightarrow B^\bullet$ between two cochains, for each $i$ we have a natural morphism 
\begin{align*}
f^i_* : H^i(A^\bullet)\rightarrow H^i(B^\bullet)\quad \text{ and }\quad f^i_*(\overline{x})\triangleq \overline{f^i(x)}
\end{align*}
Therefore, for each $i\inz$, \emph{the $i$-th homology and cohomology are in nature functors}: 
\begin{align*}
H_i:\mathbf{Ch}(\mathcal{A})\rightarrow \mathcal{A}\quad \text{ and }\quad H^i: \mathbf{cCh}(\mathcal{A})\rightarrow \mathcal{A} 
\end{align*}
Note that sequences in $\mathbf{Ch}(\mathcal{A})$ and $\mathbf{cCh}(\mathcal{A})$ are exact if and only if it is exact componentwise.
\begin{theorem}
\label{THses}
\textbf{(Short exact sequence of complexes induce a long exact sequence of (co) homologies)} Let $\mathcal{A}$ be an abelian category. Given a short exact sequence of chains  
% https://q.uiver.app/#q=WzAsNixbMCwwLCIwIl0sWzIsMCwiQV97XFxidWxsZXR9Il0sWzQsMCwiQl9cXGJ1bGxldCJdLFs2LDAsIkNfe1xcYnVsbGV0fSJdLFs3LDBdLFs4LDAsIjAiXSxbMCwxXSxbMSwyLCJmX3tcXGJ1bGxldH0iXSxbMiwzLCJnX3tcXGJ1bGxldH0iXSxbMyw1XV0=
\[\begin{tikzcd}
	0 && {A_{\bullet}} && {B_\bullet} && {C_{\bullet}} & {} & 0
	\arrow[from=1-1, to=1-3]
	\arrow["{f_{\bullet}}", from=1-3, to=1-5]
	\arrow["{g_{\bullet}}", from=1-5, to=1-7]
	\arrow[from=1-7, to=1-9]
\end{tikzcd}\]
in $\mathcal{A}$, we have a \emph{canonical} long exact sequence of homologies: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{\cdots } & {H_i(A_\bullet)} & {H_i(B_\bullet)} & {H_i(C_\bullet)} & {H_{i-1}(A_\bullet)} & \cdots
	\arrow[from=1-1, to=1-2]
	\arrow["{f_i^*}", from=1-2, to=1-3]
	\arrow["{g_i^*}", from=1-3, to=1-4]
	\arrow["{\partial_i}", from=1-4, to=1-5]
	\arrow[from=1-5, to=1-6]
\end{tikzcd}\]
Dually, given a short exact sequence of cochains 
% https://q.uiver.app/#q=WzAsNixbMCwwLCIwIl0sWzIsMCwiQV57XFxidWxsZXR9Il0sWzQsMCwiQl5cXGJ1bGxldCJdLFs2LDAsIkNee1xcYnVsbGV0fSJdLFs3LDBdLFs4LDAsIjAiXSxbMCwxXSxbMSwyLCJmXntcXGJ1bGxldH0iXSxbMiwzLCJnXntcXGJ1bGxldH0iXSxbMyw1XV0=
\[\begin{tikzcd}
	0 && {A^{\bullet}} && {B^\bullet} && {C^{\bullet}} & {} & 0
	\arrow[from=1-1, to=1-3]
	\arrow["{f^{\bullet}}", from=1-3, to=1-5]
	\arrow["{g^{\bullet}}", from=1-5, to=1-7]
	\arrow[from=1-7, to=1-9]
\end{tikzcd}\]
in $\mathcal{A}$, we have a \emph{canonical} long exact sequence of cohomologies: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{\cdots } & {H^i(A^\bullet)} & {H^i(B^\bullet)} & {H^i(C^\bullet)} & {H^{i+1}(A^\bullet)} & \cdots
	\arrow[from=1-1, to=1-2]
	\arrow["{f^i_*}", from=1-2, to=1-3]
	\arrow["{g_*^i}", from=1-3, to=1-4]
	\arrow["{\partial^i}", from=1-4, to=1-5]
	\arrow[from=1-5, to=1-6]
\end{tikzcd}\]
\end{theorem}
\begin{proof}
Routine. The inducing of the boundary homomorphisms is done the same way as in \customref{THsnake}{snake lemma}. 
\end{proof}
Let $f_\bullet,g_\bullet:A_\bullet \rightarrow B_\bullet$ be two morphisms from one chain to another. A \textbf{chain homotopy} $s_{\bullet}:f_\bullet \rightarrow g_\bullet$ is a set of morphisms $s_i: A_i \rightarrow B_{i+1}$: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	\cdots && {A_{i-1}} && {A_i} && {A_{i+1}} && \cdots \\
	\\
	\cdots && {B_{i-1}} && {B_i} && {B_{i+1}} && \cdots
	...
	\arrow["{s_{i-1}}"{description}, from=1-3, to=3-5]
	\arrow["d"', from=1-5, to=1-3]
	\arrow["{f_i-g_i}"{description}, from=1-5, to=3-5]
	\arrow["{s_i}"{description}, from=1-5, to=3-7]
	\arrow["d"', from=1-7, to=1-5]
	\arrow[from=1-7, to=3-7]
	\arrow["d"', from=1-9, to=1-7]
	\arrow["d", from=3-3, to=3-1]
	\arrow["d", from=3-5, to=3-3]
	\arrow["d", from=3-7, to=3-5]
	\arrow["d", from=3-9, to=3-7]
\end{tikzcd}\]
that make 
\begin{align}
\label{EQhomo}
f-g=d \circ s + s \circ d
\end{align}
Dually, given two morphisms $f^\bullet,g^\bullet: A^\bullet \rightarrow B^\bullet$ from one cochain to another, a \textbf{cochain homotopy}  $s^\bullet:f^\bullet \rightarrow g^\bullet$ is a set  of morphisms $s^i: A^i \rightarrow B^{i-1}$
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	\cdots && {A_{i-1}} && {A_i} && {A_{i+1}} && \cdots \\
	\\
	\cdots && {B_{i-1}} && {B_i} && {B_{i+1}} && \cdots
	...
	\arrow["{s_i}"{description}, from=1-5, to=3-3]
	\arrow["{f_i-g_i}"{description}, from=1-5, to=3-5]
	\arrow["d", from=1-7, to=1-9]
	\arrow["{s_{i+1}}"{description}, from=1-7, to=3-5]
	\arrow[from=1-7, to=3-7]
...
\end{tikzcd}\]
that satisfies \myref{equation}{EQhomo}.  Clearly, chain and cochain homotopic are respectively equivalence relation in the Hom-sets of $\mathbf{Ch}(\mathcal{A})$ and $\mathbf{cCh}(\mathcal{A})$. 
\begin{theorem}
\label{THbph}
\textbf{(Basic property of homotopic)} Let $\mathcal{A}$ be an abelian category and $f_\bullet,g_\bullet: A_\bullet \rightarrow B_\bullet$ be two morphisms from one chain in $\mathcal{A}$ to another. If they are homotopic, then 
\begin{align*}
f_i^* =g_i^* : H_i(A_\bullet)\rightarrow H_i(B_\bullet)
\end{align*}
Dually, given two morphisms $f^\bullet , g^\bullet: A^\bullet \rightarrow B^\bullet$ from one cochain in $\mathcal{A}$ to another. If they are homotopic,  then 
\begin{align*}
f^i_*=g^i_* : H^i(A^\bullet)\rightarrow H^i(B^\bullet)
\end{align*}
\end{theorem}
\begin{proof}
Routine. 
...
\begin{equiv_def}
\textbf{(Exactness of functors)} Let $\mathcal{A} $ and $\mathcal{B}$ be two abelian categories. An \emph{additive} (both covariant and contravariant) functor $F:\mathcal{A} \rightarrow \mathcal{B}$ is said to be an \textbf{exact functor} if any of the followings hold true
\begin{enumerate}[label=(\roman*)]
  \item $F$ maps exact sequence to exact sequence.  
  \item $F$ maps short exact sequence to short exact sequence. 
\end{enumerate}
\end{equiv_def}
\begin{proof}
Because the proof for contravariant functors is dual to the proof for covariant functors, we only prove for covariant functors.  (i)$\implies $(ii) is clear, so we only have to prove  (ii)$\implies $(i). This is done by \textbf{breaking up a long exact sequence into multiple short exact sequences}. That is, given an exact sequence: 
\begin{align*}
A \overset{f}{\longrightarrow }  B \overset{g}{\longrightarrow } C
\end{align*}
in $\mathcal{A}$, we construct a commutative diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&& 0 && 0 &&&& 0 \\
	&&& {\operatorname{Im}(f)} &&&& {\operatorname{Im}(g)} \\
	&& A && B && C \\
	& A &&&& {\operatorname{ker}(g)} \\
	0 &&&& 0 && 0
	\arrow[from=1-3, to=2-4]
	\arrow[from=2-4, to=1-5]
	\arrow["y"{description}, hook, from=2-4, to=3-5]
	\arrow[from=2-8, to=1-9]
	\arrow["x"{description}, two heads, from=3-3, to=2-4]
	\arrow["f"{description}, from=3-3, to=3-5]
	\arrow["g"{description}, from=3-5, to=3-7]
	\arrow["z"{description}, two heads, from=3-5, to=4-6]
	\arrow[two heads, from=3-7, to=2-8]
	\arrow[hook, from=4-2, to=3-3]
	\arrow["w"{description}, hook, from=4-6, to=3-7]
	\arrow[two heads, from=4-6, to=5-7]
	\arrow[from=5-1, to=4-2]
	\arrow[from=5-5, to=4-6]
\end{tikzcd}\]
Now, because $F$ preserves short exact sequence, we know $F(x)$ is surjective, so from $F(f)=F(y \circ x)=F(y)\circ F(x)$, we know $\operatorname{Im}(F(f))=\operatorname{Im}(F(y))$. Similarly, we can show $\operatorname{ker}(F(g))=\operatorname{ker}(F(z))$. Then because $F$ preserves the middle short exact sequence, we have 
\begin{align*}
\operatorname{Im}(F(f))=\operatorname{Im}(F(y))= \operatorname{ker}(F(z))=\operatorname{ker}(F(g))
\end{align*}
as desired. 
\end{proof}
\begin{equiv_def}
\label{EDlare}
  \textbf{(Left and right exactness of functors)} Let $\mathcal{A},\mathcal{B}$ be two abelian categories. An additive covariant functor $F: \mathcal{A}\rightarrow \mathcal{B}$ is said to be \textbf{right exact} if any of the followings hold true 
\begin{enumerate}[label=(\roman*)]
  \item  $A \longrightarrow  B \longtwoheadrightarrow  C \longrightarrow  0 $ is exact $ \implies  F(A)\longrightarrow F(B)\longtwoheadrightarrow  F(C) \longrightarrow 0$  is exact.  
  \item $0 \longrightarrow A \longhookrightarrow B \longtwoheadrightarrow C \longrightarrow 0$ is exact $\implies F(A)\longrightarrow F(B)\longtwoheadrightarrow F(C)\longrightarrow 0$ is exact.  
\end{enumerate}
Also, $F: \mathcal{A}\rightarrow \mathcal{B}$ is said to be \textbf{left exact} if any of the followings hold true
\begin{enumerate}[label=(\roman*)]
  \item $0 \longrightarrow A \longhookrightarrow B \longrightarrow C$ is exact $\implies 0 \longrightarrow F(A)\longhookrightarrow F(B)\longrightarrow F(C)$ is exact. 
  \item $0\longrightarrow A \longhookrightarrow B \longtwoheadrightarrow C \longrightarrow 0$ is exact $\implies 0 \longrightarrow F(A)\longhookrightarrow F(B)\longrightarrow F(C)$ is exact. 
\end{enumerate} 
An additive contravariant functor $G: \mathcal{A}\rightarrow \mathcal{B}$ is said to be \textbf{right exact} if any of the followings hold true
\begin{enumerate}[label=(\roman*)]
  \item $0 \longrightarrow A \longhookrightarrow B \longrightarrow C$ is exact $\implies  G(C) \longrightarrow  G(B) \longtwoheadrightarrow  G(A)\longrightarrow 0$ is exact.  
  \item $0\longrightarrow A \longhookrightarrow B \longtwoheadrightarrow C \longrightarrow 0$ is exact $\implies  G(C)\longrightarrow  G(B)\longtwoheadrightarrow  G(A) \longrightarrow 0$ is exact. 
\end{enumerate}
Also, $G: \mathcal{A}\rightarrow \mathcal{B}$ is said to be \textbf{left exact} if any of the followings hold true
\begin{enumerate}[label=(\roman*)]
  \item $A\longrightarrow B \longtwoheadrightarrow C \longrightarrow 0$ is exact $\implies 0\longrightarrow G(C)\longhookrightarrow G(B)\longrightarrow  G(A)$ is exact. 
  \item $0\longrightarrow A\longhookrightarrow B \longtwoheadrightarrow C \longrightarrow 0$ is exact $\implies  0\longrightarrow G(C)\longhookrightarrow G(B)\longrightarrow G(A)$ is exact. 
\end{enumerate}
\end{equiv_def}
\begin{proof}
Routine. 
...
\end{proof}
Let $\mathcal{A}$ be an abelian category. Clearly, for all $N\in \mathcal{A}$  we have an additive \emph{covariant} functor $\Hom (N,-):\mathcal{A}\rightarrow \mathbf{Ab}$ defined by 
\begin{align*}
A \mapsto  \Hom (N,A)\quad \text{ and }\quad g\overset{F(f)}{\longmapsto } f \circ g 
\end{align*}
and an additive \emph{contravariant} functor $\Hom  (-,N):\mathcal{A}\rightarrow \mathbf{Ab}$ defined by 
\begin{align*}
A \mapsto \Hom  (A,N)\quad \text{ and }\quad g \overset{G(f)}{\longmapsto } g \circ f
\end{align*}
The memorization here is that the $\Hom $ is covariant in the second variable and contravariant in the first variable.
\begin{theorem}
\label{THhomp}
\textbf{(Both $\Hom $ functors are left exact)} Let $\mathcal{A}$ be an abelian category. Then  
\begin{enumerate}[label=(\roman*)]
   \item The sequence: 
\begin{align*}
M' \overset{u}{\longrightarrow } M \overset{v}{\longtwoheadrightarrow  } M'' \longrightarrow 0
\end{align*}
in $\mathcal{A}$ is exact $\iff $ For all $N \in \mathcal{A}$, the sequence: 
\begin{align*}
0 \longrightarrow \Hom (M'',N) \overset{G(v)}{\longhookrightarrow  } \Hom  (M,N) \overset{G(u)}{\longrightarrow } \Hom (M',N) 
\end{align*}
in $\mathbf{Ab}$ is exact, where $G$ is the contravariant functor $\Hom  (-,N): \mathcal{A}\rightarrow \mathbf{Ab} $. 
\item The sequence: 
\begin{align*}
0\longrightarrow   M' \overset{u}{\longhookrightarrow  } M \overset{v}{\longrightarrow } M'' 
\end{align*}
in $\mathcal{A}$ is exact $\iff $ For all $N \in \mathcal{A}$, the sequence: 
\begin{align*}
0 \longrightarrow  \Hom (N,M') \overset{F(u)}{\longhookrightarrow  } \Hom  (N,M) \overset{F(v)}{\longrightarrow  } \Hom (N,M'') 
\end{align*}
in $\mathbf{Ab}$ is exact, where $F$ is the covariant functor $\Hom  (N,-):\mathcal{A}\rightarrow \mathbf{Ab}$. 
\end{enumerate}
\end{theorem}
\begin{proof}
Since the two proofs are dual to each other, we only prove for the contravariant functor $G$. The direction $(\implies )$ is to say that $G$ is left exact. We first prove this direction. The fact that functors preserve chain complex is clear, so we only have to prove 
\begin{align*}
  \vi{\operatorname{ker}(G(v))\leq 0\quad \text{ and }\quad \operatorname{ker}(G(u))\leq \operatorname{Im}(G(v))}
\end{align*}
Let $g \in \operatorname{ker}(G(v))$. It then follows from surjectivity of $v$ and  $g \circ v= G(v)(g)=0$ that $g=0$. Let $f \in \operatorname{ker}(G(u))$. Because $f \circ u=0$, we know $\operatorname{ker}(f)\leq \operatorname{Im}(u)=\operatorname{ker}(v)$. This then by universal property induces a unique morphism $\tilde{f}:M ''\rightarrow N $ that makes $f= G(v)(\tilde{f})$. $\vdone$ \\

$(\impliedby)$: To prove $v \circ  u =0$, one set $N \triangleq M'' $ and consider $\id_{M''}$.  It remains to prove 
\begin{align*}
  \blue{\pi  : M '' \twoheadrightarrow \operatorname{Coker}(v)\text{ is zero in }\Hom (M'',\operatorname{Coker}(v))\quad \text{ and }\quad \operatorname{ker}(v)\leq \operatorname{Im}(u)}
\end{align*}
For the first statement, set $N \triangleq \operatorname{Coker}(v)$. Because $G(v)(\pi)=\pi  \circ v =0 $ by definition, we see $\pi  \in \operatorname{ker}(G(v))$, which implies $\pi=0$. For the second statement, set $N \triangleq \operatorname{Coker}(u)$ and  let $p:M \twoheadrightarrow \operatorname{Coker}(u)$ be the canonical morphism. Because $\operatorname{ker}(p)=\operatorname{Im}(u)$, we only have to prove $\operatorname{ker}(v)\leq \operatorname{ker}(p)$. Now, since $p \in \operatorname{ker}(G(u))=\operatorname{Im}(G(v))$, we know $p=\psi \circ v$, which implies $\operatorname{ker}(v)\leq \operatorname{ker}(p)$.  $\bdone$ 
\end{proof}
\section{Resolutions}
Let $\mathcal{C}$ be a category. We say $I \in \mathcal{C}$ is an \textbf{injective object} if for all monomorphism $m:M\hookrightarrow X$ and morphism $f: M \rightarrow I$, there exists a unique morphism $\tilde{f}:X \rightarrow I $ that makes the diagram:   
% https://q.uiver.app/#q=WzAsMyxbMCwwLCJNIl0sWzIsMCwiWCJdLFsyLDIsIkkiXSxbMCwyLCJmIiwyXSxbMCwxLCJtIiwwLHsic3R5bGUiOnsidGFpbCI6eyJuYW1lIjoiaG9vayIsInNpZGUiOiJ0b3AifX19XSxbMSwyLCJcXHRpbGRle2Z9IiwwLHsic3R5bGUiOnsiYm9keSI6eyJuYW1lIjoiZGFzaGVkIn19fV1d
\[\begin{tikzcd}
	M && X \\
	\\
	&& I
	\arrow["m", hook, from=1-1, to=1-3]
	\arrow["f"', from=1-1, to=3-3]
	\arrow["{\tilde{f}}", dashed, from=1-3, to=3-3]
\end{tikzcd}\]
commutes. We say $P\in \mathcal{C} $ is a \textbf{projective object} if for all epimorphism $e:E \twoheadrightarrow X$ and morphism $f:P \rightarrow X$ there exists a unique morphism $\tilde{f}: P \rightarrow E$ that makes the diagram:  
% https://q.uiver.app/#q=WzAsMyxbMiwwLCJQIl0sWzIsMiwiRSJdLFswLDAsIlgiXSxbMCwxLCJcXHRpbGRle2Z9IiwwLHsic3R5bGUiOnsiYm9keSI6eyJuYW1lIjoiZGFzaGVkIn19fV0sWzEsMiwiZSIsMCx7InN0eWxlIjp7ImhlYWQiOnsibmFtZSI6ImVwaSJ9fX1dLFswLDIsImYiLDJdXQ==
\[\begin{tikzcd}
	X && P \\
	\\
	&& E
	...
	\arrow["{\tilde{f}}", dashed, from=1-3, to=3-3]
	\arrow["e", two heads, from=3-3, to=1-1]
\end{tikzcd}\]
commutes.  In other words, every morphism to an injective object factors through every monomorphism from its source, and every morphism from a projective object factors through every epimorphism to its target.  
\begin{equiv_def}
\textbf{(Injective and projective objects in abelian categories)} Let $\mathcal{A}$ be an abelian category and $I,P \in \mathcal{A}$. Then the followings are equivalent: 
\begin{enumerate}[label=(\roman*)]
  \item $I$ is an injective object.  
  \item The contravariant functor $\Hom (-,I): \mathcal{A}\rightarrow \mathbf{Ab}$ is exact. 
  \item Every short exact sequence of the form 
    \begin{align*}
    0 \longrightarrow I \longhookrightarrow M \longtwoheadrightarrow  M'' \longrightarrow 0
    \end{align*}
    splits. 
\end{enumerate}
Also, the followings are equivalent: 
\begin{enumerate}[label=(\roman*)]
  \item $P$ is a projective object.  
  \item The covariant functor $\Hom  (P,-):\mathcal{A}\rightarrow \mathbf{Ab}$ is exact. 
  \item Every short sequence of the form 
    \begin{align*}
    0 \rightarrow M' \longhookrightarrow M \longtwoheadrightarrow P \longrightarrow 0
    \end{align*}
    splits. 
\end{enumerate}
\end{equiv_def}
\begin{proof}
Because the proof for projective object is dual to the proof for injective object, we only prove for injective object.\\

(i)$\implies $(ii): Because \customref{THhomp}{$G\triangleq \Hom (-,I): \mathcal{A}\rightarrow \mathbf{Ab}$ is left exact}, we only have to prove that for injective $u:M' \rightarrow M$, the morphism $G(u): \Hom  (M,I)\rightarrow \Hom (M',I)$ is surjective. Let $f \in \Hom  (M',I)$. It then follows from universal property of injective object that there exists some (unique) $\tilde{f}: M \rightarrow I $ that makes 
\begin{align*}
f= u \circ \tilde{f} = G(u) (\tilde{f}) 
\end{align*}
(ii)$\implies $(iii): Denote the injective morphism by  $m: I \rightarrow M$. The proof then follows from noting that because $G(m):\Hom  (M,I)\rightarrow \Hom  (I,I)$ is surjective, there exists some $r: \Hom  (M,I)$ that makes the sequence left splits: 
\begin{align*}
\id_I= G(m)(r)=r \circ m
\end{align*}
(iii)$\implies $(i): Clearly, given the diagram: 
% https://q.uiver.app/#q=WzAsMyxbMCwwLCJNIl0sWzIsMCwiWCJdLFsyLDIsIkkiXSxbMCwyLCJmIiwyXSxbMCwxLCJtIiwwLHsic3R5bGUiOnsidGFpbCI6eyJuYW1lIjoiaG9vayIsInNpZGUiOiJ0b3AifX19XV0=
\[\begin{tikzcd}
	M && X \\
	\\
	&& I
	...
	\arrow["f"', from=1-1, to=3-3]
\end{tikzcd}\]
we may form their pushout: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	M && X && {X+_M I} \\
	\\
	&& I
	...
	\arrow["g"', hook, from=3-3, to=1-5]
\end{tikzcd}\]
where $g$ can be easily checked to be injective from \customref{THbpac}{the construction of pushouts in abelian category}. Then from premise, there exists some $h:X +_M I \rightarrow I$ that makes $h \circ g=\id_I$. We now see that $h \circ j$ is the morphism we are looking for: 
\begin{align*}
h \circ j \circ m = h \circ g \circ f = \id_I \circ f = f
\end{align*}
\end{proof}
\begin{theorem}
\textbf{(Projective objects are closed under coproduct and injective objects are closed under product)} Let $\mathcal{C}$ be a category. 
\begin{enumerate}[label=(\roman*)]
  \item If \emph{possibly infinite} $A_i \in \mathcal{C}$ are projective and coproduct $\bigoplus A_i$ exists, then $\bigoplus A_i$ is projective.   
  \item If \emph{possibly infinite} $A_i \in \mathcal{C}$ are injective and product $\prod  A_i$ exists, then $\prod A_i$ is injective.  
\end{enumerate}
\end{theorem}
\begin{proof}
Routine. 
\end{proof}
Let $\mathcal{C}$ be a category. We say $\mathcal{C}$ \textbf{has enough injective} if for all $X \in \mathcal{C}$ there exists a monomorphism $X \hookrightarrow I$ from $X$ to an injective object  $I \in \mathcal{C}$, and we say $\mathcal{C}$ \textbf{has enough projective} if for all $X \in \mathcal{C}$ there exists an epimorphism $P \twoheadrightarrow X$ from a projective object $P\in \mathcal{C}$ to $X$. Given an abelian category $\mathcal{A}$ with enough injective, for each $X \in \mathcal{A} $, we may form an exact cochain:  
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&&&&& {\operatorname{Coker}(f_{-1})} && {\operatorname{Coker}(f_0)} \\
	0 && X && {I^0} && {I^1} && {\cdots }
	\arrow["{f_0}", hook, from=1-6, to=2-7]
	\arrow["{f_1}", hook, from=1-8, to=2-9]
	\arrow[from=2-1, to=2-3]
	\arrow["{d^{-1}\triangleq f_{-1}}"', hook, from=2-3, to=2-5]
	\arrow["{\pi_0}", two heads, from=2-5, to=1-6]
	\arrow["{d^{0}}"', from=2-5, to=2-7]
	\arrow["{\pi_1}", two heads, from=2-7, to=1-8]
	\arrow["{d^1}"', from=2-7, to=2-9]
\end{tikzcd}\]
called \textbf{an injective resolution of} $X$. Dually, given an abelian category $\mathcal{A}$ with enough projective, for each $X \in \mathcal{A}$, we may form an exact chain:  
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&&&&& {\operatorname{ker}(g_{-1})} && {\operatorname{ker}(g_0)} \\
	0 && X && {P_0} && {P_1} && {\cdots }
	\arrow[hook', from=1-6, to=2-5]
	\arrow[hook', from=1-8, to=2-7]
	\arrow[from=2-3, to=2-1]
	\arrow["{d_{-1}\triangleq g_{-1}}", two heads, from=2-5, to=2-3]
	\arrow["{g_0}"', two heads, from=2-7, to=1-6]
	\arrow["{d_0}", from=2-7, to=2-5]
	\arrow["{g_1}"', two heads, from=2-9, to=1-8]
	\arrow["{d_1}", from=2-9, to=2-7]
\end{tikzcd}\]
called \textbf{a projective resolution of $X$}. 
\begin{theorem}
\label{THeaulr}
\textbf{(Morphisms induce (co) chain morphism unique up to homotopy)} Let $\mathcal{A}$ be an abelian category. Given two injective resolutions $X \rightarrow I^\bullet$ and $Y \rightarrow Q^\bullet$ in $\mathcal{A}$ and a morphism $f:X \rightarrow Y$, there exists a cochain morphism $f^\bullet:I^\bullet \rightarrow Q^\bullet $ that makes $f^{-1}=f$, unique up to homotopy. Dually, given two projective resolution $X \leftarrow P_\bullet $ and $Y \leftarrow Q_\bullet$ in $\mathcal{A}$ and a morphism $f: X \rightarrow Y$, there exists a chain morphism  $f_\bullet: P_\bullet \rightarrow Q_\bullet $ that makes $f_{-1}=f$, unique up to homotopy. 
\end{theorem}
\begin{proof}
Because the two proofs are dual to each other, we only prove for injective resolutions. We first prove the existence, so we start with the diagram 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	0 && X && {I^0} && {I^1} && \cdots \\
	\\
	0 && Y && {Q^0} && {Q^1} && \cdots
	\arrow[from=1-1, to=1-3]
	\arrow["{d^{-1}}", hook, from=1-3, to=1-5]
	\arrow["f"{description}, from=1-3, to=3-3]
	\arrow["{d^0}", from=1-5, to=1-7]
	\arrow[from=1-7, to=1-9]
	\arrow[from=3-1, to=3-3]
	\arrow["{\epsilon^{-1}}"', hook', from=3-3, to=3-5]
	\arrow["{\epsilon^0}"', from=3-5, to=3-7]
	\arrow[from=3-7, to=3-9]
\end{tikzcd}\]
Because $d^{-1}: X \hookrightarrow I^0$ is by definition injective, we may construct $f$ using the universal property of $I^0$, giving us a commutative diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	0 && X && {I^0} && {I^1} && \cdots \\
	\\
	0 && Y && {Q^0} && {Q^1} && \cdots
	...
	\arrow["f"{description}, from=1-3, to=3-3]
	\arrow["{d^0}", from=1-5, to=1-7]
	\arrow["{f^0}"{description}, dashed, from=1-5, to=3-5]
	\arrow[from=1-7, to=1-9]
	\arrow[from=3-1, to=3-3]
	\arrow["{\epsilon^{-1}}"', hook', from=3-3, to=3-5]
	\arrow["{\epsilon^0}"', from=3-5, to=3-7]
...
Then because 
\begin{align*}
\epsilon^0 \circ f^0 \circ d^{-1} = \epsilon ^0 \circ \epsilon ^{-1} \circ f = 0
\end{align*}
By universal property of $\operatorname{Coker}(d^{-1})$, there exist a morphism $u: \operatorname{Coker}(d^-1)\rightarrow Q^0$ that makes the diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&&&&& {\operatorname{Coker}(d^{-1})} \\
	0 && X && {I^0} && {I^1} && \cdots \\
	\\
	0 && Y && {Q^0} && {Q^1} && \cdots
	\arrow[hook, from=1-6, to=2-7]
	\arrow["u", curve={height=30pt}, dashed, from=1-6, to=4-7]
	\arrow[from=2-1, to=2-3]
	\arrow["{d^{-1}}", hook, from=2-3, to=2-5]
	\arrow["f"{description}, from=2-3, to=4-3]
	\arrow["\pi", two heads, from=2-5, to=1-6]
	\arrow["{d^0}", from=2-5, to=2-7]
	\arrow["{f^0}"{description}, from=2-5, to=4-5]
	\arrow[from=2-7, to=2-9]
	\arrow[from=4-1, to=4-3]
	\arrow["{\epsilon^{-1}}"', hook', from=4-3, to=4-5]
	\arrow["{\epsilon^0}"', from=4-5, to=4-7]
	\arrow[from=4-7, to=4-9]
\end{tikzcd}\]
commutes. The construction of $f^1$ then follows from universal property of  $I^1:$
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&&&&& {\operatorname{Coker}(d^{-1})} \\
	0 && X && {I^0} && {I^1} && \cdots \\
	\\
	0 && Y && {Q^0} && {Q^1} && \cdots
	\arrow[hook, from=1-6, to=2-7]
	\arrow["u", curve={height=30pt}, from=1-6, to=4-7]
	\arrow[from=2-1, to=2-3]
	\arrow["{d^{-1}}", hook, from=2-3, to=2-5]
	\arrow["f"{description}, from=2-3, to=4-3]
	\arrow["\pi", two heads, from=2-5, to=1-6]
	...
	\arrow["{f^0}"{description}, from=2-5, to=4-5]
	\arrow[from=2-7, to=2-9]
	\arrow["{f^1}"{description}, dashed, from=2-7, to=4-7]
	\arrow[from=4-1, to=4-3]
	\arrow["{\epsilon^{-1}}"', hook', from=4-3, to=4-5]
	\arrow["{\epsilon^0}"', from=4-5, to=4-7]
	\arrow[from=4-7, to=4-9]
\end{tikzcd}\]
The construction of $f^n$ for  $n\geq 2$ are similar to that of $f^1$. We now prove its uniqueness up to homotopic. Clearly, it suffices to show that given $f\triangleq 0$, the induced chain morphism  $f^\bullet$ is homotopic to $0^\bullet$. In other words, we only have to construct $s^i: I^i \rightarrow Q^{i-1}$ to satisfies 
\begin{align*}
0= s \circ d + d \circ s
\end{align*}
Clearly, $s^i\triangleq 0$ for $i\leq 0$ satisfies 
\begin{align*}
0^i=s \circ  d + d \circ  s,\quad \text{ for all }i\leq -1
\end{align*}
We now construct $s^1$. Because 
\begin{align*}
  f^0 \circ d^{-1} = \epsilon ^{-1} \circ f=0 
\end{align*}
By universal property of $\operatorname{Coker}(d^{-1})$ induces a commutative diagram:  
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	& X \\
	& {I^0} \\
	{\operatorname{Coker}(d^{-1})} \\
	& {Q^0}
	\arrow["{d^{-1}}", hook, from=1-2, to=2-2]
	\arrow["\pi"', two heads, from=2-2, to=3-1]
	\arrow["{f^0}", curve={height=-18pt}, from=2-2, to=4-2]
	\arrow["{\tilde{f}^0}"{description}, dashed, from=3-1, to=4-2]
\end{tikzcd}\]
which by injectivity of $I^1$ induces a commutative diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	& X \\
	& {I^0} \\
	{\operatorname{Coker}(d^{-1})} & {I^1} \\
	& {Q^0}
	\arrow["{d^{-1}}", hook, from=1-2, to=2-2]
	\arrow["\pi"', two heads, from=2-2, to=3-1]
	\arrow["{d^0}"', from=2-2, to=3-2]
	\arrow["{f^0}", curve={height=-18pt}, from=2-2, to=4-2]
	\arrow[hook, from=3-1, to=3-2]
	\arrow["{\tilde{f}^0}"', from=3-1, to=4-2]
	\arrow["{s^1}"{description}, dashed, from=3-2, to=4-2]
\end{tikzcd}\]
Because $s^0=0$, such  $s^1$ clearly is what we are looking for. Let  
 \begin{align*}
h^1\triangleq f^1 - \epsilon ^0 \circ s^1 \in \Hom (I^1 ,Q^1)
\end{align*}
We are then require to construct $s^2:I^2 \rightarrow Q^1$ that makes $h^1= s^2 \circ d^1$. First, because $f^0= s^1 \circ d^0$, we have  
\begin{align*}
h^1  \circ d^0 &= f^1 \circ d^0 - \epsilon ^0 \circ s^1 \circ d^0 \\
&= \epsilon ^0 \circ f^0 - \epsilon ^0 \circ  s^1 \circ d^0 = \epsilon ^0 \circ (f^0 - s^1 \circ d^0) =0 
\end{align*}
This then by universal property of $\operatorname{Coker}(d^0)$ induces a commutative diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&&& {\operatorname{Coker}(d^0)} \\
	{I^0} && {I^1} \\
	&& {} \\
	&& {Q^1}
	\arrow["{\tilde{h}^1}"{description}, curve={height=-12pt}, dashed, from=1-4, to=4-3]
	\arrow["{d^0}", from=2-1, to=2-3]
	\arrow["\pi", two heads, from=2-3, to=1-4]
	\arrow["{h^1}", from=2-3, to=4-3]
\end{tikzcd}\]
which by injectivity of $I^1$ induces a commutative diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&&& {\operatorname{Coker}(d^0)} \\
	{I^0} && {I^1} && {I^2} \\
	\\
	...
	\arrow["{\tilde{h}^1}"{description}, curve={height=-24pt}, from=1-4, to=4-3]
	\arrow["{d^0}", from=2-1, to=2-3]
	\arrow["\pi", two heads, from=2-3, to=1-4]
	\arrow["{d^1}", from=2-3, to=2-5]
	\arrow["{h^1}", from=2-3, to=4-3]
	\arrow["{s^2}"{description}, curve={height=-18pt}, dashed, from=2-5, to=4-3]
\end{tikzcd}\]
as desired. The construction of $s^n$ for  $n\geq 3$ is similar to that of $s^2$, so we win. 
\end{proof}
\begin{theorem}
\label{THhorseshoe}
\textbf{(Horseshoe Lemma)} Let $\mathcal{A}$ be an abelian category with enough projective and 
% https://q.uiver.app/#q=WzAsNSxbMCwwLCIwIl0sWzIsMCwiQSciXSxbNCwwLCJBIl0sWzYsMCwiQScnIl0sWzgsMCwiMCJdLFswLDFdLFsxLDIsImYiLDAseyJzdHlsZSI6eyJ0YWlsIjp7Im5hbWUiOiJob29rIiwic2lkZSI6InRvcCJ9fX1dLFsyLDMsImciLDAseyJzdHlsZSI6eyJoZWFkIjp7Im5hbWUiOiJlcGkifX19XSxbMyw0XV0=
\[\begin{tikzcd}
	0 && {A'} && A && {A''} && 0
	\arrow[from=1-1, to=1-3]
	\arrow["f", hook, from=1-3, to=1-5]
	\arrow["g", two heads, from=1-5, to=1-7]
	\arrow[from=1-7, to=1-9]
\end{tikzcd}\]
be an exact sequence in $\mathcal{A}$. Then for all projective resolutions  $P'_\bullet \rightarrow A'$ and $P''_\bullet \rightarrow A''$ for $A'$ and $A''$, there exists a projective resolution $P_\bullet \rightarrow A$ and a short exact sequence  
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	0 && {(P'_\bullet \rightarrow A')} && {(P_\bullet \rightarrow A)} && {(P''_\bullet \rightarrow A)} && 0
	\arrow[from=1-1, to=1-3]
	\arrow[hook, from=1-3, to=1-5]
	\arrow[two heads, from=1-5, to=1-7]
	\arrow[from=1-7, to=1-9]
\end{tikzcd}\]
in $\mathbf{Ch}(\mathcal{A})$. More precisely, we can construct a projective resolution of $A$ by 
 \begin{align*}
P_i \triangleq P_i' \oplus  P_i'',\quad \text{ for all }i
\end{align*}
with the natural morphisms $P'_i \longhookrightarrow P_i \longtwoheadrightarrow P''_i$ respectively forming a lift of $f$ and $g$. In other words, the natural morphisms give rise to a commutative diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&& 0 && 0 && 0 \\
	\\
	{\cdots } && {P'_1} && {P'_0} && {A'} && 0 \\
	...
	\arrow["g", two heads, from=5-7, to=7-7]
...
\end{tikzcd}\]
Dually, the same statement holds for abelian categories with enough injective. 
\end{theorem}
\begin{proof}
Let $P_i \triangleq P'_i \oplus  P''_i$. We start with the diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{P'_0} && {A'} && 0 \\
	\\
	{P_0} && A && 0 \\
	\\
	{P_0''} && {A''} && 0
	\arrow["{\epsilon'}", two heads, from=1-1, to=1-3]
	\arrow["{\diota_0}", hook, from=1-1, to=3-1]
	\arrow[from=1-3, to=1-5]
	\arrow["f", hook, from=1-3, to=3-3]
	\arrow["{\pi_0}", two heads, from=3-1, to=5-1]
	\arrow[from=3-3, to=3-5]
	\arrow["g", two heads, from=3-3, to=5-3]
	\arrow["{\epsilon''}"', two heads, from=5-1, to=5-3]
	\arrow[from=5-3, to=5-5]
\end{tikzcd}\]
Because $g$ is surjective, projectivity of $P_0''$ gives us a commutative diagram:  
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{P'_0} && {A'} && 0 \\
	\\
	{P_0} && A && 0 \\
	...
	\arrow[from=3-3, to=3-5]
	\arrow["g", two heads, from=3-3, to=5-3]
	\arrow["t"{description}, dashed, from=5-1, to=3-3]
	\arrow["{\epsilon''}"', two heads, from=5-1, to=5-3]
	\arrow[from=5-3, to=5-5]
\end{tikzcd}\]
Because $P_0=P'_0 \oplus  P_0$, universal property of coproduct gives us a  commutative diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{P'_0} && {A'} && 0 \\
	\\
	{P_0} && A && 0 \\
	...
	\arrow[from=1-3, to=1-5]
	\arrow["f", hook, from=1-3, to=3-3]
	\arrow["{\epsilon\triangleq t+ f \circ\epsilon'}", dashed, from=3-1, to=3-3]
	\arrow["{\pi_0}", two heads, from=3-1, to=5-1]
	\arrow[from=3-3, to=3-5]
	\arrow["g", two heads, from=3-3, to=5-3]
	\arrow["t"{description}, from=5-1, to=3-3]
	\arrow["{\epsilon''}"', two heads, from=5-1, to=5-3]
	\arrow[from=5-3, to=5-5]
\end{tikzcd}\]
Note that the reason we have $\epsilon = t \circ \pi _0$ is that by definition we have 
\begin{align*}
t \circ \pi_0 \circ \diota = t = \epsilon  \circ \diota    
\end{align*}
where $\diota: P_0'' \hookrightarrow  P_0 $ is the canonical morphism for coproduct, \customref{THkar}{which is a monomorphism}. Applying \customref{THsnake}{snake lemma} on the two columns, we have a commutative diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{P_1'} && {\operatorname{ker}(\epsilon')} && {P'_0} && {A'} && 0 \\
	\\
	{P_1} && {\operatorname{ker}(\epsilon)} && {P_0} && A && 0 \\
	\\
	{P_1''} && {\operatorname{ker}(\epsilon'')} && {P_0''} && {A''} && 0
	...
	\arrow[from=1-3, to=1-5]
	\arrow[hook, from=1-3, to=3-3]
	\arrow["{\epsilon'}", two heads, from=1-5, to=1-7]
	\arrow["{\diota_0}", hook, from=1-5, to=3-5]
	\arrow[from=1-7, to=1-9]
	\arrow["f", hook, from=1-7, to=3-7]
	\arrow[two heads, from=3-1, to=5-1]
	\arrow[from=3-3, to=3-5]
	\arrow[two heads, from=3-3, to=5-3]
	\arrow["\epsilon"{description}, two heads, from=3-5, to=3-7]
	\arrow["{\pi_0}", two heads, from=3-5, to=5-5]
	\arrow[from=3-7, to=3-9]
	\arrow["g", two heads, from=3-7, to=5-7]
	\arrow[two heads, from=5-1, to=5-3]
	\arrow[from=5-3, to=5-5]
	\arrow["t"{description}, from=5-5, to=3-7]
	\arrow["{\epsilon''}"', two heads, from=5-5, to=5-7]
	\arrow[from=5-7, to=5-9]
\end{tikzcd}\]
in which $\epsilon $ is surjective because in the application of snake lemma we have a exact sequence 
% https://q.uiver.app/#q=WzAsMyxbMCwwLCJcXG9wZXJhdG9ybmFtZXtDb2tlcn0oXFxlcHNpbG9uJyk9MCJdLFsyLDAsIlxcb3BlcmF0b3JuYW1le0Nva2VyfShcXGVwc2lsb24pIl0sWzQsMCwiXFxvcGVyYXRvcm5hbWV7Q29rZXJ9KFxcZXBzaWxvbicnKT0wIl0sWzAsMV0sWzEsMiwiIiwwLHsic3R5bGUiOnsiaGVhZCI6eyJuYW1lIjoiZXBpIn19fV1d
\[\begin{tikzcd}
	{\operatorname{Coker}(\epsilon')=0} && {\operatorname{Coker}(\epsilon)} && {\operatorname{Coker}(\epsilon'')=0}
	\arrow[from=1-1, to=1-3]
	\arrow[two heads, from=1-3, to=1-5]
\end{tikzcd}\]
which implies $\operatorname{Coker}(\epsilon )=0$. Lastly, the proof follows from noting that the rest of construction can be done the same way we construct $\epsilon $. 
\end{proof}
\section{$\delta$-Functors}
Let $\mathcal{A}$ and $\mathcal{B}$ be two abelian categories. A \textbf{homological covariant $\delta$-functor from $\mathcal{A}$ to $\mathcal{B} $} consists of the followings datum
\begin{enumerate}[label=(\roman*)]
  \item a sequence of additive functors $F_i : \mathcal{A}\rightarrow \mathcal{B}$. 
  \item For every  short exact sequence in $\mathcal{A}$ : 
% https://q.uiver.app/#q=WzAsNSxbMCwwLCIwIl0sWzIsMCwiQSJdLFs0LDAsIkIiXSxbNiwwLCJDIl0sWzgsMCwiMCJdLFswLDFdLFsxLDIsImYiXSxbMiwzLCJnIl0sWzMsNF1d
\[\begin{tikzcd}
	0 && A && B && C && 0
	\arrow[from=1-1, to=1-3]
	\arrow["f", from=1-3, to=1-5]
	...
	\arrow[from=1-7, to=1-9]
\end{tikzcd}\]
a sequence of morphisms $\delta_i: F_i(C)\rightarrow F_{i-1}(A)$ that gives rise to a long exact sequence in $\mathcal{A}$ : 
% https://q.uiver.app/#q=WzAsNixbMSwwLCJGX2koQSkiXSxbMiwwLCJGX2koQikiXSxbMywwLCJGX2koQykiXSxbNCwwLCJGX3tpLTF9KEEpIl0sWzUsMCwiXFxjZG90cyJdLFswLDAsIlxcY2RvdHMiXSxbMCwxLCJGX2koZikiXSxbMSwyLCJGX2koZykiXSxbMiwzLCJcXGRlbHRhX2kiXSxbMyw0XSxbNSwwXV0=
\[\begin{tikzcd}
	\cdots & {F_i(A)} & {F_i(B)} & {F_i(C)} & {F_{i-1}(A)} & \cdots
	\arrow[from=1-1, to=1-2]
	\arrow["{F_i(f)}", from=1-2, to=1-3]
	\arrow["{F_i(g)}", from=1-3, to=1-4]
	\arrow["{\delta_i}", from=1-4, to=1-5]
	\arrow[from=1-5, to=1-6]
\end{tikzcd}\]
\textbf{natural} in the sense that for each morphism of short exact sequences in $\mathcal{A}$: 
% https://q.uiver.app/#q=WzAsMTAsWzAsMCwiMCJdLFsyLDAsIkEiXSxbNCwwLCJCIl0sWzYsMCwiQyJdLFs4LDAsIjAiXSxbMCwyLCIwIl0sWzIsMiwiQSciXSxbNCwyLCJCJyJdLFs2LDIsIkMnIl0sWzgsMiwiMCJdLFswLDFdLFsxLDJdLFsyLDNdLFszLDRdLFs1LDZdLFs2LDddLFs3LDhdLFs4LDldLFsxLDZdLFsyLDddLFszLDhdXQ==
\[\begin{tikzcd}
	0 && A && B && C && 0 \\
	\\
	0 && {A'} && {B'} && {C'} && 0
	...
	\arrow[from=3-7, to=3-9]
\end{tikzcd}\]
we have a morphism of long exact sequences in $\mathcal{A}$ :  
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	\cdots & {F_i(A)} & {F_i(B)} & {F_i(C)} & {F_{i-1}(A)} & {\cdots } \\
	\cdots & {F_i(A')} & {F_i(B')} & {F_i(C')} & {F_{i-1}(A')} & \cdots
	\arrow[from=1-1, to=1-2]
...
\end{tikzcd}\]
\end{enumerate}
Dually, a \textbf{cohomological covariant $\delta$-functor from $\mathcal{A}$ to $\mathcal{B}$} consists of:
\begin{enumerate}[label=(\roman*)]
  \item a sequence of additive functors $F_i : \mathcal{A}\rightarrow \mathcal{B}$. 
  \item For every  short exact sequence in $\mathcal{A}$ : 
% https://q.uiver.app/#q=WzAsNSxbMCwwLCIwIl0sWzIsMCwiQSJdLFs0LDAsIkIiXSxbNiwwLCJDIl0sWzgsMCwiMCJdLFswLDFdLFsxLDIsImYiXSxbMiwzLCJnIl0sWzMsNF1d
\[\begin{tikzcd}
	...
	\arrow[from=1-7, to=1-9]
\end{tikzcd}\]
a sequence of morphisms $\delta_i: F_i(C) \rightarrow F_{i+1}(A)$ that gives rises to a long exact sequence in $\mathcal{A}$ :  
% https://q.uiver.app/#q=WzAsNixbMSwwLCJGX2koQSkiXSxbMiwwLCJGX2koQikiXSxbMywwLCJGX2koQykiXSxbNCwwLCJGX3tpKzF9KEEpIl0sWzUsMCwiXFxjZG90cyJdLFswLDAsIlxcY2RvdHMiXSxbMCwxLCJGX2koZikiXSxbMSwyLCJGX2koZykiXSxbMiwzLCJcXGRlbHRhX2kiXSxbMyw0XSxbNSwwXV0=
\[\begin{tikzcd}
	\cdots & {F_i(A)} & {F_i(B)} & {F_i(C)} & {F_{i+1}(A)} & \cdots
	\arrow[from=1-1, to=1-2]
	\arrow["{F_i(f)}", from=1-2, to=1-3]
...
\end{tikzcd}\]
we have a morphism of long exact sequences in $\mathcal{A}$: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	\cdots & {F_i(A)} & {F_i(B)} & {F_i(C)} & {F_{i+1}(A)} & {\cdots } \\
	\cdots & {F_i(A')} & {F_i(B')} & {F_i(C')} & {F_{i+1}(A')} & \cdots
	\arrow[from=1-1, to=1-2]
...
\end{tikzcd}\]
\end{enumerate}
Clearly, we have the \textbf{category of homological covariant $\delta$-functors from $\mathcal{A}$ to $\mathcal{B}$}, in which the morphisms from $(F_i,\delta_i)$ to $(G_i,\delta_i')$ is a family of natural transformation $\eta_i:F_i \rightsquigarrow G_i$ such that for all short exact sequence in $\mathcal{A}$: 
% https://q.uiver.app/#q=WzAsNSxbMCwwLCIwIl0sWzIsMCwiQSJdLFs0LDAsIkIiXSxbNiwwLCJDIl0sWzgsMCwiMCJdLFswLDFdLFsxLDJdLFsyLDNdLFszLDRdXQ==
\[\begin{tikzcd}
	0 && A && B && C && 0
	\arrow[from=1-1, to=1-3]
	\arrow[from=1-3, to=1-5]
...
\end{tikzcd}\]
the diagram:
% https://q.uiver.app/#q=WzAsNCxbMCwwLCJGX2koQykiXSxbMiwwLCJGX3tpLTF9KEEpIl0sWzAsMiwiR19pKEMpIl0sWzIsMiwiR197aS0xfShBKSJdLFswLDIsIihcXGV0YV9pKV9DIl0sWzEsMywiKFxcZXRhX3tpLTF9KV9BIl0sWzAsMSwiXFxkZWx0YV9pIiwxXSxbMiwzLCJcXGRlbHRhJ197aX0iLDFdXQ==
\[\begin{tikzcd}
	{F_i(C)} && {F_{i-1}(A)} \\
	\\
	{G_i(C)} && {G_{i-1}(A)}
	\arrow["{\delta_i}"{description}, from=1-1, to=1-3]
	\arrow["{(\eta_i)_C}", from=1-1, to=3-1]
	\arrow["{(\eta_{i-1})_A}", from=1-3, to=3-3]
	\arrow["{\delta'_{i}}"{description}, from=3-1, to=3-3]
\end{tikzcd}\]
commutes. A homological covariant $\delta$-functor $(F_i,\delta_i)$ from $\mathcal{A}$ to $\mathcal{B}$ is then said to be \textbf{universal} if given 
\begin{enumerate}[label=(\roman*)]
  \item any $\delta$-functor $(G_i,\delta_i')$ 
  \item any natural transformation $\eta_0: F_0\rightsquigarrow G_0 $ from $F_0$ to  $G_0$ 
\end{enumerate}
there exists a unique sequence of natural transformations $\eta_i : F_i  \rightsquigarrow G_i  $ that gives us a morphism $\eta: (G_i,\delta_i') \rightarrow (F_i, \delta_i)$ of homological covariant $\delta$-functors.  \\ 

Clearly, universal homological covariant $\delta$-functors from $\mathcal{A}$ to $\mathcal{B}$ are unique up to the $0$-th functor in the sense that given any two universal homological covariant $\delta$-functor $(F_i,\delta_i)$ and $(G_i, \delta_i')$ from $\mathcal{A}$ to $\mathcal{B}$, if $F_0$ and  $G_0:\mathcal{A}\rightarrow \mathcal{B}$ are isomorphic, then $F_i$ and  $G_i$ are all isomorphic.\footnote{To prove such, just induce the two natural transformation $F_0\rightsquigarrow G_0$ and  $G_0 \rightsquigarrow F_0$ inverse to each other to each $F_i$ and  $G_i$.}



%
% So the rest is about proving $R^iF$ are $\delta$- and universal, and that $\operatorname{Tor}_i(N,-)$ are $\delta$ and universal. 
%
...
\label{SEdf}
\begin{abstract}
In this section, we show that derived functors are well-defined and they are universal $\delta$-functors.   
\end{abstract}
Let $F: \mathcal{A}\rightarrow \mathcal{B}$ be a  \emph{covariant additive} functor from an abelian category $\mathcal{A}$ with enough injective to an arbitrary abelian category $\mathcal{B}$. Let $X \in \mathcal{A}$ and  $X\rightarrow I^\bullet$ be an injective resolution. Truncate the $F(X)$ in its image, we have a cochain: 
% https://q.uiver.app/#q=WzAsNCxbMCwwLCIwIl0sWzIsMCwiRihJXjApIl0sWzQsMCwiRihJXjEpIl0sWzYsMCwiXFxjZG90cyAiXSxbMCwxXSxbMSwyXSxbMiwzXV0=
\[\begin{tikzcd}
	0 && {F(I^0)} && {F(I^1)} && {\cdots }
	\arrow[from=1-1, to=1-3]
	\arrow[from=1-3, to=1-5]
...
For now we denote 
\begin{align*}
R^iF(X)\triangleq H^i(F(I^\bullet))
\end{align*}
\begin{theorem}
\textbf{($R^iF(X)$ is well-defined)} Let $\mathcal{A}$ be an abelian category with enough injective and $\mathcal{B}$ an abelian category. Let $F:\mathcal{A}\rightarrow \mathcal{B}$ be a covariant additive functor. Given two injective resolutions $X \rightarrow I^\bullet$ and $X\rightarrow Q^\bullet$ of $X \in \mathcal{A}$, for each $i$ there exists a  \emph{canonical} isomorphism  
\begin{align*}
  H^i(F(I^\bullet)) \cong  H^i(F(Q^\bullet)),\quad \text{ in }\mathcal{B} 
\end{align*}
\end{theorem}
\begin{proof}
Denote \customref{THeaulr}{ the two chain morphisms induced by  $\id_X$} as: 
\begin{align*}
\customref{THeaulr}{f^\bullet \triangleq  \id_X^\bullet: I^\bullet \rightarrow Q^\bullet}\quad \text{ and }\quad \customref{THeaulr}{g^\bullet \triangleq \id_X^\bullet : Q^\bullet \rightarrow I^\bullet}
\end{align*}
We are required to check that $F(f^i)_*:H^i(F(I^\bullet))\rightarrow H^i(F(Q^\bullet))$ and $F(g^i)_*:H^i(F(Q^\bullet))\rightarrow H^i(F(I^\bullet))$ are inverse to each other. WLOG, we only show that 
\begin{align*}
F(g^i)_*\circ F(f^i)_*  = \id_{H^i(F(I^\bullet))}
\end{align*}
By definition, we have 
\begin{align*}
  F(g^i)_*\circ F(f^i)_* = \left(F(g^i)\circ F(f^i) \right)_* = F(g^i \circ f^i) _*
\end{align*}
Now, since, as one should check, both $g^i \circ f^i$ and $\id_{I^i}$ are induced by  
% https://q.uiver.app/#q=WzAsNixbMCwwLCJYIl0sWzAsMiwiWCJdLFsyLDAsIkleMCJdLFsyLDIsIkleMCJdLFs0LDAsIkleMSJdLFs0LDIsIkleMSJdLFswLDJdLFsxLDNdLFswLDEsIlxcaWRfWCIsMV0sWzMsNV0sWzIsNF1d
\[\begin{tikzcd}
	X && {I^0} && {I^1} \\
	\\
	X && {I^0} && {I^1}
	...
	\arrow[from=3-3, to=3-5]
\end{tikzcd}\]
\customref{THeaulr}{We know they are homotopic}. Then because $F$ is additive, as one should check,  $F((g^i\circ f^i))$ and $F(\id_{I^i})=\id_{F(I^i)}$ are homotopic. \customref{THbph}{This then implies $F((g \circ f)^i)_*=\id_{H^i(F(I^\bullet))}$}. 
\end{proof}
In conclusion, we have a well-defined functor $R^iF: \mathcal{A}\rightarrow \mathcal{B}$ called the \textbf{$i$-th right derived functor} of the \emph{covariant additive} functor $F:\mathcal{A}\rightarrow \mathcal{B}$ from an abelian category with enough injective, in which the morphisms $f:X \rightarrow  Y$ in $\mathcal{A}$ are mapped to 
\begin{align*}
  R^iF(f)\triangleq F(f^i)_* : H^i(F(I^\bullet) ) \rightarrow H^i (F(Q^\bullet) )
\end{align*}
where  \customref{THeaulr}{$f_i$ is canonically induced by $f$}. \\

Usually, we talk about right derived functor $R^iF$ when  $F$ is  left-exact. This is because if $F$ is left-exact, then the sequence 
\begin{align*}
 0 \longrightarrow   F(X) \longrightarrow  F(I^0) \longrightarrow  F(I^1),\quad \text{ in }\mathcal{B} 
\end{align*}
is exact, which implies 
\begin{align*}
R^0F(X)= F(X)\quad \text{ and }\quad R^0F=F
\end{align*}
Moreover, given a short exact sequence $A \rightarrow B \rightarrow C$ in $\mathcal{A} $ with injective resolutions $A \rightarrow I^\bullet_A$ and $C \rightarrow I^\bullet _C$, by \customref{THhorseshoe}{horseshoe lemma}, we have a short exact sequence 
\begin{align*}
  0 \longrightarrow  I_A^\bullet \longrightarrow  I_A^\bullet \oplus  I_C^\bullet \longrightarrow  I_C^\bullet \longrightarrow  0,\quad \text{ in }\mathbf{cCh}(\mathcal{A}) 
\end{align*}
in which $I_B^\bullet \triangleq I_A^\bullet \oplus I_C^\bullet$ form an injective resolution of $B$. Then because \customref{THbpadf}{additive functors preserve biproducts}, we have a short exact sequence 
\begin{align*}
0 \longrightarrow  F(I_A^\bullet) \longrightarrow  F(I_B^\bullet)  \longrightarrow  F(I_C^\bullet) \longrightarrow  0, \quad \text{ in }\mathbf{cCh}(\mathcal{B})
\end{align*}
Then because \customref{THses}{short exact sequence of cochains induces a \emph{canonical} long exact sequence of cohomologies}, we have a \emph{canonical} long exact sequence: 
\begin{align*}
...
Dually, given an \emph{covariant additive} functor $F:\mathcal{A}\rightarrow \mathcal{B}$ from an abelian category \emph{with enough projective}, we have a well-defined functor $L_iF:\mathcal{A}\rightarrow \mathcal{B}$ called the \textbf{$i$-th left derived functor} of $F: \mathcal{A}\rightarrow \mathcal{B}$ 
\begin{align*}
L_iF(X)\triangleq H_i(F(P_\bullet))\quad \text{ and }\quad L_iF(f)\triangleq  F(f_i)_*
\end{align*}
whilst, if $F: \mathcal{A}\rightarrow \mathcal{B}$ is right-exact, then we have 
\begin{align*}
L_0F(X) \cong  F(X)\quad \text{ and }\quad L_0F=F
\end{align*}
and a \emph{canonical} long exact sequence: 
\begin{align*}
\cdots \rightarrow L_1F(A) \rightarrow L_1F(B) \rightarrow L_1F(C) \rightarrow F(A) \rightarrow F(B)\rightarrow F(C) \rightarrow 0
\end{align*}
\begin{theorem}
\label{THdfoiap}
\textbf{(Derived functors on injective and projective objects)} Let  $F:\mathcal{A}\rightarrow \mathcal{B}$ be a covariant additive functor between two abelian categories. If $\mathcal{A}$ has enough injective and $F$ is left-exact, then for all injective object $I \in \mathcal{A}$, we have 
\begin{align*}
  R^nF(I)=0,\quad \text{ for all }n\geq 1
\end{align*}
Dually, if $\mathcal{A} $ has enough projective and $F$ is right-exact, then for all projective object $P \in \mathcal{A}$, we have 
\begin{align*}
L_nF(P)=0,\quad \text{ for all }n\geq 1
\end{align*}
\end{theorem}
\begin{proof}
The statement for injective object follows from resoluting $I$ by 
 \begin{align*}
I \longrightarrow  I \longrightarrow  0 \longrightarrow  0 \longrightarrow \cdots  
\end{align*}
where the first arrow is an isomorphism. The statement for projective objects can be proved similarly.  
\end{proof}
\begin{theorem}
\label{THdad}
\textbf{(Derived functors are $\delta$-functors)} Let $F:\mathcal{A}\rightarrow \mathcal{B}$ be a covariant additive functor. If $\mathcal{A}$ has enough injective, then $R^iF$ forms a homological covariant $\delta$-functor. Dually, if $\mathcal{A}$ has enough projective, then  $L_iF$ forms a cohomological covariant $\delta$-functor. 
\end{theorem}
\begin{proof}
The proofs are dual to each other. Note that even though we have already shown that right derived functors induces long exact sequences from short exact sequence, we haven't shown that such inducing is natural to conclude that $R^iF$ forms a  $\delta$-functor. \href{https://github.com/ericliu515515/MathNotic/blob/main/Books/Homological%20Algebra/Romyar%20Sharifi.pdf}{The proof of naturalaity boils down to a long and routine computation, in which one, necessarily, use horseshoe lemma to construct a manageable injective resolution of $B$ for us to conduct the computation. See Theorem 3.3.5 of this copy of the text \emph{Homological algebra} by Romyar Sharifi}. 
% Note that the link may break 
\end{proof}
\begin{theorem}
\textbf{(Characterization of universal $\delta$-functor from abelian categories with enough injective or projective)} Let $F^i: \mathcal{A}\rightarrow \mathcal{B}$ be a homological $\delta$-functor from one abelian category to another. If $\mathcal{A}$ has enough injective and $F^n(I)=0$ for all injective object  $I \in \mathcal{A}$ and $n\geq 1$, then $F^i:\mathcal{A}\rightarrow \mathcal{B}$ is universal. The dual statement also holds. 
\end{theorem}
\begin{proof}
 \href{https://github.com/ericliu515515/MathNotic/blob/main/Books/Homological%20Algebra/Romyar%20Sharifi.pdf}{See Theorem 3.3.9 of this copy of the text \emph{Homological algebra} by Romyar Sharifi}. 
\end{proof}
Now, since  derived functors satisfy \customref{THdfoiap}{$R^nF(I)=0$ for all $n\geq 1$}, we see that they are indeed universal $\delta$-functors.  
\section{Tor Functor}
Let $A$ and $B$ be two rings, and $M$ be an $(A,B)$-bimodule. Then we have a right-exact functor $N \otimes_A -$ from $\mathbf{Mod}_A$ to $\mathbf{Mod}_{(A,B)}$. The  \textbf{$n$-th Tor functor} $\operatorname{Tor}_n^A(N, -)$ is then its $n$-th left derived functor. In other words, we write 
\begin{align*}
\operatorname{Tor}_n^A(N,M) \triangleq H_n (N \otimes  P_\bullet)
\end{align*}
where $P_\bullet$ is a projective resolution of $M$. 
\begin{theorem}
\textbf{(Balancing of Tor functor)} Let $A$ be a ring and  $M$ and  $N$ be two  $A$-modules. Then we have an isomorphism 
 \begin{align*}
   \operatorname{Tor}^A_n(M,N)\cong \operatorname{Tor}^A_n(N,M)
\end{align*}
for all $n\geq 0$. 
\end{theorem}
\begin{proof}
There are supposedly two methods to prove this. One, which seems more down to earth, is to consider the double complex of their projective resolutions. See \href{https://github.com/ericliu515515/MathNotic/blob/main/Books/Homological%20Algebra/Romyar%20Sharifi.pdf}{the first proof of Proposition 3.5.9 of this copy of the text \emph{Homological algebra} by Romyar Sharifi} or \href{https://github.com/ericliu515515/MathNotic/blob/main/Books/Homological%20Algebra/Weibel.pdf}{Theorem 2.7.2 of Weibel}.\\

The other supposed "solution" is to show that $\operatorname{Tor}_n(-,N)$ also forms a universal $\delta$-functor, as suggested by \href{https://github.com/ericliu515515/MathNotic/blob/main/Books/Homological%20Algebra/Romyar%20Sharifi.pdf}{the second proof of Proposition 3.5.9 of this copy of the text \emph{Homological algebra} by Romyar Sharifi} and the \href{https://mathoverflow.net/questions/126938/commutativity-of-tor?utm_source=chatgpt.com}{comment in this MO post}. However, I will make the remark that the second proof by Romyar seems skeptical to me, in that he didn't prove the naturalaity of $\operatorname{Tor}_n(-,N)$, whilst the rest seems fine.  
\end{proof}

\begin{equiv_def}
\label{EDflat}
\textbf{(Flat modules)} Let $N$ be an  $A$-module. We say $N$ is \textbf{flat} if any of the followings hold true 
\begin{enumerate}[label=(\roman*)]
  \item  $- \otimes  N$    is exact. 
  \item Restriction of $- \otimes N$ onto the category of finitely generated $A$-modules is exact. 
  \item $\operatorname{Tor}_n^A(N,M)=0$ for all $n\geq 1$ and $M\in \mathbf{Mod}_A$. 
  \item $\operatorname{Tor}_1^A(N,M)=0$ for all $M\in \mathbf{Mod}_A$.   
  \item $\operatorname{Tor}_n^A(N,M)=0$ for all $n\geq 1$ and finitely generated $M \in \mathbf{Mod}_A$
  \item $\operatorname{Tor}_1^A(N,M)=0$ for all finitely generated  $M \in \mathbf{Mod}_A$. 
\end{enumerate}
In particular, the definitions that use tor functor \customref{THdfoiap}{implies that projective modules are flat}. 
\end{equiv_def}
\begin{proof}
(i)$\implies $(iii)$\implies $(iv)$\implies $(i) and  (iii)$\implies $(v)$\implies $(vi) $\implies $(ii) are routine, and (i)$\implies $(ii) is clear, so it only remains to prove (ii)$\implies $(i). In particular, since \customref{THbpt}{tensor product is right exact}, we only have to prove that given injective homomorphism $f: M' \hookrightarrow  M$, where $M'$ and  $M$ need not be finitely generated, the homomorphism $f \otimes  1: M' \otimes  N \rightarrow M \otimes  N$ remains injective.\\     
...
\end{align*} 
\item $A$ is a flat  $A$-module. 
   \item $A[x]$ is a flat $A$-module.  
...
\end{theorem}
\begin{proof}
Note that (iii) is a corollary of (i) and (ii), since $A[x]\cong \bigoplus M_i$, where $M_i$ is the sub  $A$-module generated  $x^i$, which is isomorphic to  $A$ as  $A$-modules. The rest are proved the same way: We wish to show flatness by showing that tensor preserve injectivity, so we factors the tensored morphism into a composition a isomorphisms and a tensored morphism that is guaranteed injectivity via flatness of the objects we are given. \\

We prove the  $(\impliedby)$ direction of (i) as an example. Note that the proof of the converse is a simple observation that if one of $M_i$ isn't flat, then  clearly  $f \otimes  \id_{\bigoplus M_i}$ isn't injective. \\

Let $f:P' \hookrightarrow  P$ be an injective $A$-module homomorphism. Universal property of coproduct gives us an $A$-module homomorphisms $\bigoplus  f \otimes \id_{M_i}: \bigoplus (P' \otimes  M_i)\rightarrow \bigoplus (P \otimes  M_i) $  that maps $p' \otimes  m_i$ to $f(p')\otimes  m_i$. Because $M_i$ are flat, we know  $f \otimes  \id_{M_i}$ are injective. This clearly implies $\bigoplus f \otimes  \id_{M_i}$ is flat. The proof then follows from noting that we have a commutative diagram:  
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{P' \otimes \bigoplus M_i} && {P\otimes \bigoplus M_i} \\
	\\
	{\bigoplus P'\otimes M_i} && {\bigoplus P\otimes M_i}
	\arrow["{f\otimes \id_{\bigoplus M_i}}", from=1-1, to=1-3]
	\arrow[from=1-1, to=3-1]
	\arrow["{\bigoplus (f\otimes \id_{M_i}) }"', from=3-1, to=3-3]
	\arrow[from=3-3, to=1-3]
\end{tikzcd}\]
where the columns are the  \customref{THidt}{natural isomorphisms}: 
\begin{align*}
P' \otimes  \bigoplus M_i \cong  \bigoplus P' \otimes M_i \quad \text{ and }\quad P \otimes  \bigoplus M_i \cong  \bigoplus  P \otimes  M_i
\end{align*}
Now, (ii) use the isomorphism  $P \otimes  A \cong  P$, (iv) use the isomorphism $P \otimes  (M \otimes N) \cong  (P \otimes  M) \otimes  N$, and (v) use the isomorphisms: 
\begin{align*}
P \otimes_A  N \cong  P \otimes_A (B \otimes _B N) \cong  (P \otimes_A B) \otimes_B N 
\end{align*}
\end{proof}
\begin{theorem}
\textbf{(Homological identity of flat modules)} Let $A$ be a ring. Given an exact sequence 
  \begin{align*}
 0 \longrightarrow N' \longrightarrow N \longrightarrow N'' \longrightarrow 0 
  \end{align*}
  of $A$-modules with  $N''$ flat, we have 
  \begin{align*}
 N'\text{ is flat }\iff N\text{ is flat } 
  \end{align*}
\end{theorem}
\begin{proof}
This follows from noting that since $N''$ is flat, we have a short exact sequence: 
\begin{align*}
0=\operatorname{Tor}_2(M,N'') \longrightarrow \operatorname{Tor}_1(M,N') \longrightarrow \operatorname{Tor}_1(M,N) \longrightarrow \operatorname{Tor}_1(M,N'')=0 
\end{align*}
\end{proof}
\begin{theorem}
\textbf{(Homological identity of flat modules)} Let $A$ be a ring and $N$ an $A$-module. Then 
 \begin{align*}
N\text{ is flat }\iff \operatorname{Tor}_1(A \quotient \mathfrak{a},N)=0,\quad \text{ for all finitely generated }\mathfrak{a}\leq A
\end{align*}
\end{theorem}
\begin{proof}
Left to right follows from \customref{EDflat}{definition of flatness}. We prove from right to left. We first show that: 
\begin{align*}
\vi{\operatorname{Tor}_1(A \quotient  \mathfrak{b},N)=0,\quad \text{ for all }\mathfrak{b}\trianglelefteq A}
\end{align*}
By premise, for each finitely generated submodule  $\mathfrak{a}_i$, we have a short exact sequence 
\begin{align*}
0 \longrightarrow \mathfrak{a}_i \otimes  N \longrightarrow A \otimes  N \longrightarrow A \quotient \mathfrak{a}_i \otimes  N  \longrightarrow 0
\end{align*}
Because \customref{THdad}{$\operatorname{Tor}(N,-)$ is a $\delta$-functor}, for each inclusion $\mathfrak{a}_i \hookrightarrow \mathfrak{a}_j$ of finitely generated submodules of  $\mathfrak{b}$, we have a morphism of short exact sequence 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	0 & {\mathfrak{a}_i\otimes N} & {A\otimes N} & {A\quotient \mathfrak{a}_i\otimes N} & 0 \\
	0 & {\mathfrak{a}_j\otimes N} & {A\otimes N} & {A\quotient \mathfrak{a}_j\otimes N} & 0
	\arrow[from=1-1, to=1-2]
	\arrow[from=1-2, to=1-3]
	\arrow[from=1-2, to=2-2]
	\arrow[from=1-3, to=1-4]
	...
	\arrow[from=2-4, to=2-5]
\end{tikzcd}\]
This implies that we have a short exact sequence of directed system 
\begin{align*}
0 \rightarrow   (\mathfrak{a}_i \otimes  N) \rightarrow (A \otimes  N) \rightarrow (A \quotient \mathfrak{a}_i \times N) \rightarrow 0
\end{align*}
consisting of finitely generated submodules of $\mathfrak{b}$. Then because \customref{EDsm}{direct limit is an exact functor}, this induces a short exact sequence consisting of their direct limit 
\begin{align*}
0 \longrightarrow \directlimit(\mathfrak{a}_i \otimes  N) \longrightarrow \directlimit (A \otimes  N)\longrightarrow  \directlimit (A \quotient \mathfrak{a}_i \otimes  N) \longrightarrow 0
\end{align*}
Then because \customref{THidt}{direct limit commute with tensor product}, we have a short exact sequence 
\begin{align*}
0 \rightarrow \mathfrak{b} \otimes  N \longrightarrow A \otimes  N \longrightarrow   A \quotient \mathfrak{b} \otimes  N\longrightarrow  0 
\end{align*}
The injectivity of $\mathfrak{b}\otimes  N\rightarrow A \otimes  N$ in this short exact sequence then implies $\operatorname{Tor}_1(A\quotient \mathfrak{b}, N )=0$. $\vdone$ \\

We now prove that 
\begin{align*}
\blue{\operatorname{Tor}_1(M,N)=0,\quad \text{ for all finitely generated module $M$ }}
\end{align*}
This is proved by induction on number of generators. Let $M\triangleq (x_1,\dots ,x_n)$ and denote  $M_i \triangleq (x_1, \dots ,x_i)$ for all $0 \leq  i \leq n $. The base case $i=0$ is clear. Consider the function defined by 
\begin{align*}
A \rightarrow M_n \quotient M_{n-1},\quad a \mapsto \overline{ax_n}
\end{align*}
This gives us a short exact sequence 
\begin{align*}
0 \longrightarrow  M_{n-1} \longrightarrow M_n \longrightarrow A \quotient \operatorname{ker}(f)  \longrightarrow 0  
\end{align*}
Then by inductive hypothesis and the earlier result, we have an exact sequence  
\begin{align*}
0=\operatorname{Tor}_1(M_{n-1},N) \longrightarrow \operatorname{Tor}_1(M_n,N) \longrightarrow \operatorname{Tor}(A \quotient \operatorname{ker}(f),N) = 0
\end{align*}
which implies $\operatorname{Tor}_1(M_n,N)=0$. 


\end{proof}

\section{more constructions in general categories}
the \textbf{kernel} and \textbf{cokernel} of a morphism $f: A \rightarrow B$ is then defined by 
\begin{align*}
...
\section{Every $\mathbf{Mod}_A$ has enough injective and projective}
A \textbf{divisible abelian group} $G$ is an abelian group in which for all $g \in G$ and $n\inn$, there exists some $y \in G$ that makes $ny=g$.  
\begin{theorem}
\textbf{(Divisible abelian groups are exactly injective objects)} We have 
\begin{align*}
J\in \mathbf{Ab}\text{ is injective }\iff  J\text{ is divisible }
\end{align*}
\end{theorem}
\begin{proof}
Left to right follows from a routine proof of contradiction. \\


\end{proof}
Therefore, $\Q \quotient \Z\in \mathbf{Ab}$ is injective. 





 We say an $A$-module  $M$ is a  \textbf{direct summand} of $M''$ if there exists some module  $M'$ that makes  $M'' \cong  M \oplus  M'$. 
\begin{theorem}
\label{THmiqf}
\textbf{(Every module is a quotient of some free module)} Let $A$ be a ring and $M$ be an $A$-module. Then we have a surjective module homomorphism from the free module 
\begin{align*}
A^{(M)}\triangleq \bigoplus_{m\neq 0 \in M}A^m,\quad \text{ where }A^m \cong  A
\end{align*}
to $M$ by sending 
\begin{align*}
a \in A^m \mapsto am
...
\end{proof}
\begin{equiv_def}
\textbf{(Projective Modules)} Let $A$ be a ring. We say an $A$-module $P$ is \textbf{projective} if any of the followings hold true
\begin{enumerate}[label=(\roman*)]
  \item $P\in \mathbf{Mod}_A$ is a projective object. 
  \item $P$ is a direct summand of some free module.  
\end{enumerate}

\end{equiv_def}
\begin{proof}
  (i)$\implies $(ii): Because $\id_P: P \rightarrow P$ is surjective, by universal property, the sequence right split. \\

  (ii)$\implies $(iii): This follows from the fact that \customref{THmiqf}{$P$ is a quotient of the free module  $A^{(P)}$} and \customref{THsl}{split lemma}.\\

  (iii)$\implies $(i): Let $F\triangleq P \oplus N$ be free with basis $\set{x_i}$ and surjective homomorphism $\pi  : F \rightarrow P$. Because $g: M \rightarrow M'$ is surjective, for all $x_i$ there exists some  $z_i\in M$ such that $g(z_i)=f \circ  \pi  (z_i)$. Therefore, 
\end{proof}

Let $A$ be a ring and $M$ an  $A$-module. We say a set $E \subseteq M$ is \textbf{linearly independent} if for all finite subset $\set{e_1,\dots ,e_n}\subseteq E$, we have  
\begin{align*}
a_1e_1 + \cdots + a_ne_n=0  \implies a_1= \cdots = a_n=0
\end{align*}
A \textbf{basis} for $M$ is then a linearly independent generating set. 
\begin{equiv_def}
\textbf{(Free modules)} Let $A$ be a ring. We say an $A$-module $M$ is \textbf{free} if any of the followings hold true: 
\begin{enumerate}[label=(\roman*)]
  \item $M$ admits a basis. 
  \item For some set $S$, we have an  $A$-module isomorphism  $M\cong \bigoplus  _{i \in S}A$.  
\end{enumerate}
\end{equiv_def}
\begin{proof}
Routine. 
