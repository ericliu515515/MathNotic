...
\begin{theorem}
\label{THdls}
\textbf{(Sum of modules is a direct limits)} Let $M_i$ be a collection of submodules indexed by $I$. If for all  $i,j\in I$ there exists some $k \in I$ that makes  
\begin{align*}
M_i + M_j \leq M_k
\end{align*}
...
\begin{equiv_def}
\textbf{(Properties of functor categories whose source is small and target is abelian)} Let $\mathcal{C}$ be a small category and $\mathcal{D}$ be an abelian category. Then the category $\operatorname{Func}(\mathcal{C},\mathcal{D})$ is abelian, whose image, kernels, and cokernels can be constructed component wise. Therefore, a sequence in $\operatorname{Func}(\mathcal{C},\mathcal{D})$ is exact if and only if it is exact component wise.
\end{equiv_def}
\begin{proof}
Routine. (Categorical)  
\end{proof}
...
\item $\mathcal{A}$ admits finite coproducts.  
\end{enumerate}
...
Then $g\circ f$ is the zero in the abelian group $\Hom (A,B)$.  
  \item Let $A_1, \dots ,A_n \in \mathcal{A}$. If for each $i$ we use the universal property of coproduct to define $p_i:\coprod A_i\rightarrow A_i$ by 
\begin{align*}
...
\end{tikzcd}\]
commutes. (iii): Left to right is clear. We now prove from right to left. Because the proofs for initial and terminal of $Z$ are similar, we only prove it is terminal, which  follows from noting that for all $f:A \rightarrow Z$, we have 
\begin{align*}
f= \id _Z \circ f = 0   \in \Hom  (A,Z)
\end{align*}
(iv) follows from (iii) and the fact that \customref{THbpom}{left inverses are epimorphisms and right inverses are monomorphisms}. (v) is routine. 
\end{proof}
...
\end{align*}
Let $F: \mathcal{A}\rightarrow \mathcal{B}$ and $G:\mathcal{A}\rightarrow \mathcal{B}$ be respectively covariant and contravariant functors between abelian categories. We say $F$ and $G$ are \textbf{additive} if for all $A,B \in \mathcal{A}$, respectively, the map $\Hom _{\mathcal{A} }(A,B) \rightarrow \Hom _\mathcal{B}(F(A),F(B))$ and the map $\Hom_{\mathcal{A} }(A,B)\rightarrow \Hom _{\mathcal{B} }(G(B),G(A)) $ are group homomorphisms. Because by definition, functor send identity morphism to identity morphism, from the fact that \customref{THbpac}{$Z \in \mathcal{B} $ is a zero object $\iff \id_Z$ is zero in $\operatorname{End}(Z)$}, we see additive functor send zero  object to zero object, covariant or contravariant. 
...
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{\cdots } & {H_i(A_\bullet)} & {H_i(B_\bullet)} & {H^i(C^\bullet)} & {H_{i-1}(A_\bullet)} & \cdots
...
\end{theorem}
\begin{proof}
This is a corollary of  \customref{THsl}{snake lemma}. 
\end{proof}
...
Routine. 
\end{proof}
Let $\mathcal{C}$ be a category. We say $\mathcal{C}$ \textbf{has enough injective} if for all $X \in \mathcal{C}$ there exists a monomorphism $X \hookrightarrow I$ from $X$ to an injective object  $I \in \mathcal{C}$, and we say $\mathcal{C}$ \textbf{has enough projective} if for all $X \in \mathcal{C}$ there exists an epimorphism $P \twoheadrightarrow X$ from a projective object $P\in \mathcal{C}$ to $X$. \\

Given an abelian category $\mathcal{A}$ with enough injective, for each $X \in \mathcal{A} $, we may form an exact cochain:  
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&&&&& {\operatorname{Coker}(f_0)} && {\operatorname{Coker}(f_1)} \\
	0 && X && {I^1} && {I^2} && {\cdots }
	\arrow["{f_1}", hook, from=1-6, to=2-7]
	\arrow["{f_2}", hook, from=1-8, to=2-9]
	\arrow[from=2-1, to=2-3]
	\arrow["{d^0\triangleq f_0}"', hook, from=2-3, to=2-5]
	\arrow["{\pi_0}", two heads, from=2-5, to=1-6]
	\arrow["{d^1}"', from=2-5, to=2-7]
	\arrow["{\pi_1}", two heads, from=2-7, to=1-8]
	\arrow["{d^2}"', from=2-7, to=2-9]
\end{tikzcd}\]
...
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&&&&& {\operatorname{ker}(g_0)} && {\operatorname{ker}(g_1)} \\
	0 && X && {P_1} && {P_2} && {\cdots }
	\arrow[hook', from=1-6, to=2-5]
	\arrow[hook', from=1-8, to=2-7]
	\arrow[from=2-3, to=2-1]
	\arrow["{d_0\triangleq g_0}", two heads, from=2-5, to=2-3]
	\arrow["{g_1}"', two heads, from=2-7, to=1-6]
	\arrow["{d_1}", from=2-7, to=2-5]
	\arrow["{g_2}"', two heads, from=2-9, to=1-8]
	\arrow["{d_2}", from=2-9, to=2-7]
\end{tikzcd}\]
called \textbf{a projective resolution of $X$}. 
\begin{theorem}
\label{THeaulr}
\textbf{(Existence and uniqueness of liftings to resolutions)} Let $\mathcal{A}$ be an abelian category. Given two injective resolutions $X \rightarrow I^\bullet$ and $Y \rightarrow Q^\bullet$ in $\mathcal{A}$ and a morphism $f:X \rightarrow Y$, there exists a cochain morphism $f^\bullet:I^\bullet \rightarrow Q^\bullet $ that makes $f^0=f$, unique up to homotopic.\\

Dually, given two projective resolution $X \leftarrow P_\bullet $ and $Y \leftarrow Q_\bullet$ in $\mathcal{A}$ and a morphism $f: X \rightarrow Y$, there exists a chain morphism  $f_\bullet: P_\bullet \rightarrow Q_\bullet $ that makes $f_0=f$, unique up to homotopic. 
\end{theorem}
\begin{proof}
...
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	0 && X && {I^1} && {I^2} && \cdots \\
	\\
	0 && Y && {Q^1} && {Q^2} && \cdots
	\arrow[from=1-1, to=1-3]
	\arrow["{d^0}", hook, from=1-3, to=1-5]
	\arrow["f"{description}, from=1-3, to=3-3]
	\arrow["{d^1}", from=1-5, to=1-7]
	\arrow[from=1-7, to=1-9]
	\arrow[from=3-1, to=3-3]
	\arrow["{\epsilon^0}"', hook', from=3-3, to=3-5]
	\arrow["{\epsilon^1}"', from=3-5, to=3-7]
	\arrow[from=3-7, to=3-9]
\end{tikzcd}\]
Because $d^0: X \hookrightarrow I^1$ is by definition injective, we may construct $f^1$ using the universal property of $I^1$, giving us a commutative diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	0 && X && {I^1} && {I^2} && \cdots \\
	\\
	0 && Y && {Q^1} && {Q^2} && \cdots
	...
	\arrow["f"{description}, from=1-3, to=3-3]
	\arrow["{d^1}", from=1-5, to=1-7]
	\arrow["{f^1}"{description}, dashed, from=1-5, to=3-5]
	\arrow[from=1-7, to=1-9]
	\arrow[from=3-1, to=3-3]
	\arrow["{\epsilon^0}"', hook', from=3-3, to=3-5]
	\arrow["{\epsilon^1}"', from=3-5, to=3-7]
...
Then because 
\begin{align*}
\epsilon^1 \circ f^1 \circ d^0 = \epsilon ^1 \circ \epsilon ^0 \circ f = 0
\end{align*}
By universal property of $\operatorname{Coker}(d^0)$, there exist a morphism $u: \operatorname{Coker}(d^0)\rightarrow Q^1$ that makes the diagram 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	...
	\arrow[from=2-1, to=2-3]
	\arrow["{d^0}", hook, from=2-3, to=2-5]
	\arrow["f"{description}, from=2-3, to=4-3]
	\arrow["\pi", two heads, from=2-5, to=1-6]
	\arrow["{d^1}", from=2-5, to=2-7]
	\arrow["{f^1}"{description}, from=2-5, to=4-5]
	\arrow[from=2-7, to=2-9]
	\arrow[from=4-1, to=4-3]
	\arrow["{\epsilon^0}"', hook', from=4-3, to=4-5]
	\arrow["{\epsilon^1}"', from=4-5, to=4-7]
	\arrow[from=4-7, to=4-9]
\end{tikzcd}\]
commutes. The construction of $f^2$ then follows from universal property of  $I^2:$
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&&&&& {\operatorname{Coker}(d^0)} \\
	0 && X && {I^1} && {I^2} && \cdots \\
	\\
	...
	\arrow["{f^1}"{description}, from=2-5, to=4-5]
	\arrow[from=2-7, to=2-9]
	\arrow["{f^2}"{description}, dashed, from=2-7, to=4-7]
	\arrow[from=4-1, to=4-3]
	\arrow["{\epsilon^0}"', hook', from=4-3, to=4-5]
	\arrow["{\epsilon^1}"', from=4-5, to=4-7]
	\arrow[from=4-7, to=4-9]
\end{tikzcd}\]
The construction of $f^n$ for  $n\geq 3$ are similar to that of $f^2$. We now prove its uniqueness up to homotopic. Clearly, it suffices to show that given $f\triangleq 0$, the induced chain morphism  $f^\bullet$ is homotopic to $0^\bullet$. In other words, we only have to construct $s^i: I^i \rightarrow Q^{i-1}$ to satisfies 
\begin{align*}
0= s \circ d + d \circ s
\end{align*}
Clearly, $s^i\triangleq 0$ for $i\leq 1$ satisfies 
\begin{align*}
0^i=s \circ  d + d \circ  s,\quad \text{ for all }i\leq 0
\end{align*}
We now construct $s^2$. Because 
\begin{align*}
  f^1 \circ d^0 = \epsilon ^0 \circ f=0 
\end{align*}
By universal property of $\operatorname{Coker}(d^0)$ induces a commutative diagram:  
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	& X \\
	& {I^1} \\
	{\operatorname{Coker}(d^0)} \\
	& {Q^1}
	\arrow["{d^0}", hook, from=1-2, to=2-2]
	\arrow["\pi"', two heads, from=2-2, to=3-1]
	\arrow["{f^1}", curve={height=-18pt}, from=2-2, to=4-2]
	\arrow["{\tilde{f}^1}"{description}, dashed, from=3-1, to=4-2]
\end{tikzcd}\]
which by injectivity of $I^2$ induces a commutative diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	& X \\
	& {I^1} \\
	{\operatorname{Coker}(d^0)} & {I^2} \\
	& {Q^1}
	\arrow["{d^0}", hook, from=1-2, to=2-2]
	\arrow["\pi"', two heads, from=2-2, to=3-1]
	\arrow["{d^1}"', from=2-2, to=3-2]
	\arrow["{f^1}", curve={height=-18pt}, from=2-2, to=4-2]
	\arrow[hook, from=3-1, to=3-2]
	\arrow["{\tilde{f}^1}"', from=3-1, to=4-2]
	\arrow["{s^2}"{description}, dashed, from=3-2, to=4-2]
\end{tikzcd}\]
Because $s^1=0$, such  $s^2$ clearly is what we are looking for. Let  
 \begin{align*}
h^2\triangleq f^2 - \epsilon ^1 \circ s^2 \in \Hom (I^2 ,Q^2)
\end{align*}
We are then require to construct $s^3:I^3 \rightarrow Q^2$ that makes $h^2= s^3 \circ d^2$. First, because $f^1= s^2 \circ d^1$, we have  
\begin{align*}
h^2  \circ d^1 &= f^2 \circ d^1 - \epsilon ^1 \circ s^2 \circ d^1 \\
&= \epsilon ^1 \circ f^1 - \epsilon ^1 \circ  s^2 \circ d^1 = \epsilon ^1 \circ (f^1 - s^2 \circ d^1) =0 
\end{align*}
This then by universal property of $\operatorname{Coker}(d^1)$ induces a commutative diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&&& {\operatorname{Coker}(d^1)} \\
	{I^1} && {I^2} \\
	\\
	&& {Q^2}
	\arrow["{\tilde{h}^2}"{description}, curve={height=-12pt}, dashed, from=1-4, to=4-3]
	\arrow["{d^1}", from=2-1, to=2-3]
	\arrow["\pi", two heads, from=2-3, to=1-4]
	\arrow["{h^2}", from=2-3, to=4-3]
\end{tikzcd}\]
which by injectivity of $I^3$ induces a commutative diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&&& {\operatorname{Coker}(d^1)} \\
	{I^1} && {I^2} && {I^3} \\
	\\
	...
	\arrow["{\tilde{h}^2}"{description}, curve={height=-24pt}, from=1-4, to=4-3]
	\arrow["{d^1}", from=2-1, to=2-3]
	\arrow["\pi", two heads, from=2-3, to=1-4]
	\arrow["{d^2}", from=2-3, to=2-5]
	\arrow["{h^2}", from=2-3, to=4-3]
	\arrow["{s^3}"{description}, curve={height=-18pt}, dashed, from=2-5, to=4-3]
\end{tikzcd}\]
as desired. The construction of $s^n$ for  $n\geq 4$ is similar to that of $s^3$, so we win. 
\end{proof}
\begin{theorem}
...
\end{tikzcd}\]
The same statement holds for abelian categories with enough injective. 
\end{theorem}
\begin{proof}
Let $P_i \triangleq P'_i \oplus  P''_i$. We start with the diagram: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	0 && 0 \\
	\\
	{P'_0} && {A'} && 0 \\
	...
	\arrow["{\diota_0}", hook, from=3-1, to=5-1]
	\arrow[from=3-3, to=3-5]
	\arrow["f", hook, from=3-3, to=5-3]
	\arrow["{\pi_0}", two heads, from=5-1, to=7-1]
	\arrow[from=5-3, to=5-5]
	\arrow["g", two heads, from=5-3, to=7-3]
	\arrow["{\epsilon''}"', two heads, from=7-1, to=7-3]
	\arrow[from=7-1, to=9-1]
	\arrow[from=7-3, to=7-5]
	\arrow[from=7-3, to=9-3]
\end{tikzcd}\]
Because $g$ is surjective, projectivity of $P_0''$ gives us a commutative diagram:  
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	0 && 0 \\
	\\
	{P'_0} && {A'} && 0 \\
	...
	\arrow[from=5-3, to=5-5]
	\arrow["g", two heads, from=5-3, to=7-3]
	\arrow["t"{description}, dashed, from=7-1, to=5-3]
	\arrow["{\epsilon''}"', two heads, from=7-1, to=7-3]
	\arrow[from=7-1, to=9-1]
	\arrow[from=7-3, to=7-5]
	\arrow[from=7-3, to=9-3]
\end{tikzcd}\]
Because $P_0=P'_0 \oplus  P_0$, universal property of coproduct gives us a morphism  which by definitions forms a commutative diagram
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	0 && 0 \\
	\\
	{P'_0} && {A'} && 0 \\
	...
	\arrow[from=3-3, to=3-5]
	\arrow["f", hook, from=3-3, to=5-3]
	\arrow["{\epsilon\triangleq t+ f \circ\epsilon'}", dashed, from=5-1, to=5-3]
	\arrow["{\pi_0}", two heads, from=5-1, to=7-1]
	\arrow[from=5-3, to=5-5]
	\arrow["g", two heads, from=5-3, to=7-3]
	\arrow["t"{description}, from=7-1, to=5-3]
	\arrow["{\epsilon''}"', two heads, from=7-1, to=7-3]
	\arrow[from=7-1, to=9-1]
	\arrow[from=7-3, to=7-5]
	\arrow[from=7-3, to=9-3]
\end{tikzcd}\]
Note that $\epsilon = t \circ \pi _0$ since by definition we have $t \circ \pi_0 \circ \diota = t = \epsilon  j\circ \diota    $, where $\diota: P_0'' \hookrightarrow  P_0 $ is the canonical morphism for coproduct, \customref{THbpac}{which is a monomorphism}. 
\end{proof}
\section{Derived Functors}
Let $F: \mathcal{A}\rightarrow \mathcal{B}$ be a  \emph{covariant additive} functor from an abelian category $\mathcal{A}$ with \emph{enough injective} to an arbitrary abelian category $\mathcal{B}$. Let $X \in \mathcal{A}$ and  $X\rightarrow I^\bullet$ be an injective resolution. Even though its image: 
% https://q.uiver.app/#q=WzAsNCxbMCwwLCJGKFgpIl0sWzIsMCwiRihJXjEpIl0sWzQsMCwiRihJXjIpIl0sWzYsMCwiXFxjZG90cyAiXSxbMCwxXSxbMSwyXSxbMiwzXV0=
\[\begin{tikzcd}
	{F(X)} && {F(I^1)} && {F(I^2)} && {\cdots }
	\arrow[from=1-1, to=1-3]
	\arrow[from=1-3, to=1-5]
	\arrow[from=1-5, to=1-7]
\end{tikzcd}\]
may no longer be exact, we can compute its cohomologies  $H^i(F(X)\rightarrow F(I^\bullet))$, which for now we denote 
\begin{align*}
R^iF(X)\triangleq H^i(F(X)\rightarrow F(I^\bullet))
\end{align*}
\begin{theorem}
\textbf{($R^iF(X)$ is well-defined)} Let $\mathcal{A}$ be an abelian category with enough injective, and let $F:\mathcal{A}\rightarrow \mathcal{B}$ be a covariant additive functor. Given two injective resolutions $X \rightarrow I^\bullet$ and $X\rightarrow Q^\bullet$ of $X \in \mathcal{A}$, for each $i$ there exists a  \emph{canonical} isomorphism  
\begin{align*}
  H^i(F(X)\rightarrow F(I^\bullet)) \cong  H^i(F(X)\rightarrow F(Q^\bullet)),\quad \text{ in }\mathcal{B} 
\end{align*}
\end{theorem}
\begin{proof}
Denote \customref{THeaulr}{ the two chain morphisms lifted by  $\id_X$} as: 
\begin{align*}
...
\end{align*}
Now, since, as one should check, both $g^i \circ f^i$ and $\id_{I^i}$ are lifts of  
% https://q.uiver.app/#q=WzAsNixbMCwwLCJYIl0sWzAsMiwiWCJdLFsyLDAsIkleMSJdLFsyLDIsIkleMSJdLFs0LDAsIkleMiJdLFs0LDIsIkleMiJdLFswLDJdLFsxLDNdLFswLDEsIlxcaWRfWCIsMV0sWzIsNF0sWzMsNV1d
\[\begin{tikzcd}
	X && {I^1} && {I^2} \\
	\\
	X && {I^1} && {I^2}
...
\end{proof}
In conclusion, we have a well-defined functor $R^iF: \mathcal{A}\rightarrow \mathcal{B}$ called the \textbf{$i$-th right derived functor} of the \emph{covariant additive} functor $F:\mathcal{A}\rightarrow \mathcal{B}$ from an abelian category \emph{with enough injective}, in which the morphisms $f:X \rightarrow  Y$ in $\mathcal{A}$ are mapped to 
\begin{align*}
  R^iF(f)\triangleq F(f^i)_* : H^i(F(X)\rightarrow F(I^\bullet) ) \rightarrow H^i (F(Y)\rightarrow F(Q^\bullet) )
\end{align*}
where  \customref{THeaulr}{$f_i$ is canonically lifted by $f$}. Moreover, if $F:\mathcal{A}\rightarrow \mathcal{B}$ is left-exact,  then \customref{EDlare}{injectivity of $X\longhookrightarrow  I^0 $ implies the injectivity of $F(X)\longhookrightarrow   F(I^0)$}, which  implies 
\begin{align*}
R^0F(X)\cong  F(X)\quad \text{ and }\quad R^0F=F
\end{align*}
...
\begin{align*}
L_iF(X)\triangleq H_i(F(P^\bullet)\rightarrow F(X))\quad \text{ and }\quad L_iF(f)\triangleq  F(f_i)_*
\end{align*}
...
\begin{enumerate}[label=(\roman*)]
  \item $P$ satisfies the  \textbf{universal property of projective modules}: For all module homomorphism $f:P \rightarrow M'$ and surjective module homomorphism  $g:M \twoheadrightarrow M''$, there exists a unique module homomorphism $h:P \rightarrow M$ that makes the diagram 
% https://q.uiver.app/#q=WzAsMyxbMiwwLCJQIl0sWzAsMiwiTSJdLFsyLDIsIk0nIl0sWzEsMiwiZyIsMix7InN0eWxlIjp7ImhlYWQiOnsibmFtZSI6ImVwaSJ9fX1dLFswLDIsImYiXSxbMCwxLCJoIiwyLHsic3R5bGUiOnsiYm9keSI6eyJuYW1lIjoiZGFzaGVkIn19fV1d
\[\begin{tikzcd}
	&& P \\
	\\
	M && {M'}
...
\item Every exact sequence 
  \begin{align*}
  0\longrightarrow N \longhookrightarrow M \longtwoheadrightarrow P \longrightarrow 0
  \end{align*}
  splits. 
  \item $P$ is a direct summand of some free module. 
  \item The covariant functor $\Hom (P,-):\mathbf{Mod}_A \rightarrow \mathbf{Mod}_A$ is exact. 
\end{enumerate}
\end{equiv_def}
\begin{proof}
...
Routine. 
\end{proof}
\section{$\operatorname{Tor}$ Functor}
\begin{equiv_def}
...
\end{theorem}
\begin{proof}
  (i): Let $f:N' \hookrightarrow  N$ be an injective $A$-module homomorphism. The proof from left to right is relatively simple. Just note that if one of $M_i$ isn't flat, then clearly  $f \otimes  \id_{\bigoplus M_i}$ isn't injective. We now prove from right to left. By universal property, there exists some $A$-module homomorphisms $\bigoplus  f \otimes \id_{M_i}: \bigoplus (N' \otimes  M_i)\rightarrow \bigoplus (N \otimes  M_i) $  that maps $n' \otimes  m_i$ to $f(n')\otimes  m_i$. Clearly, injectivity of $f \otimes  \id_{M_i}$ implies injectivity of $\bigoplus f \otimes  \id _{M_i}$. This then implies the injectivity of $f \otimes  \id_{\bigoplus  M_i}$ using the fact that it factors thorough  $\bigoplus  f \otimes  \id_{M_i}$ :
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{N' \otimes \bigoplus M_i} && {N\otimes \bigoplus M_i} \\
	\\
	{\bigoplus N'\otimes M_i} && {\bigoplus N\otimes M_i}
...
(ii): Let  $f:N' \hookrightarrow N$ be an injective $A$-module homomorphism. The injectivity of  $f \otimes 1 : N' \otimes  A \rightarrow N \otimes  A$ then follows from the fact that it factors through $f$ 
% https://q.uiver.app/#q=WzAsNCxbMCwwLCJOJyBcXHRpbWVzIEEiXSxbMCwyLCJOJyJdLFsyLDAsIk5cXG90aW1lcyBBIl0sWzIsMiwiTiJdLFswLDIsImYgXFxvdGltZXMgMSJdLFswLDFdLFszLDJdLFsxLDMsImYiLDJdXQ==
\[\begin{tikzcd}
	{N' \otimes A} && {N\otimes A} \\
	\\
	{N'} && N
	\arrow["{f \otimes 1}", from=1-1, to=1-3]
	\arrow[from=1-1, to=3-1]
	\arrow["f"', from=3-1, to=3-3]
	\arrow[from=3-3, to=1-3]
\end{tikzcd}\]
...
(iii): This is a corollary of (i) and (ii), since  $A[x]\cong \bigoplus _{i \geq 0} M_i$, where $M_i$ is the sub  $A$-module generated by  $x^i$, which is isomorphic to  $A$ as $A$-modules. \\

(iv): Let $f: N'\rightarrow N''$ be an injective $A$-module homomorphism. The injectivity of  $f \otimes  \id_{M \otimes  N}$ follows from the fact that if factor through the injective homomorphism $(f\otimes  \id_M)\otimes  \id _N$ : 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{N'\otimes (M\otimes N)} && {N''\otimes (M\otimes N)} \\
	\\
	{(N'\otimes M)\otimes N} && {(N''\otimes M)\otimes N}
	\arrow["{f\otimes \id_{M\otimes N}}", from=1-1, to=1-3]
	\arrow[from=1-1, to=3-1]
	\arrow["{(f\otimes \id_M)\otimes \id_N}"', from=3-1, to=3-3]
	\arrow[from=3-3, to=1-3]
\end{tikzcd}\]
where the columns are the \customref{THidt}{natural isomorphisms}.\\

(v): Let $f: M' \hookrightarrow  M$ be an injective $A$-module homomorphism. Clearly, $f \otimes  \id _B : M' \otimes_A B \rightarrow M \otimes _A B$ is an $B$-module homomorphism. Therefore, we have a $B$-module homomorphism $(f \otimes  \id _B) \otimes  \id _N :(M' \otimes_A B)\otimes _B N \rightarrow (M\otimes _A B)\otimes  _B N$, which is injective by premise. The injectivity of $f\otimes  \id _N: M' \otimes_A N \rightarrow M\otimes _A N$ then follows from the fact that it, as a set map, factors through: 
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{M'\otimes_A N} && {M\otimes_A N} \\
	\\
	{M'\otimes_A (B\otimes _B N)} && {M\otimes_A(B\otimes _B N)} \\
	\\
	{(M'\otimes _A B)\otimes_B N} && {(M\otimes_A B)\otimes _B N}
	\arrow["{f\otimes \id_{N}}", from=1-1, to=1-3]
	\arrow[from=1-1, to=3-1]
	\arrow["{f\otimes \id_{B\otimes_B N}}", from=3-1, to=3-3]
	\arrow[from=3-1, to=5-1]
	\arrow[from=3-3, to=1-3]
	\arrow["{(f\otimes \id_B)\otimes \id_N}"', from=5-1, to=5-3]
