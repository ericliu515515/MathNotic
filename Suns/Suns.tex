...
At this point, we should point out that \textbf{finitely generated module} and \textbf{finitely generated algebra} are two different notions: 
...
\begin{align*}
\mathfrak{a}M \triangleq \set{am \in M: a \in \mathfrak{a}\text{ and }m \in M}
\end{align*}
\end{equiv_def}
\begin{proof}
Routine. 
...
\end{theorem}
\begin{proof}
Clearly we have $\mathfrak{a}\leq \mathfrak{a}_1 \times \cdots \times \mathfrak{a}_n$
\end{proof}
...
\begin{theorem}
\label{THpp}
\textbf{(Basic properties of prime ideals)} Let $A$ be a ring. Clearly contraction of prime ideals are prime, and we have:  
 \begin{enumerate}[label=(\roman*)]
...
\begin{equiv_def}
\label{EDmf}
  \textbf{(Maximal ideals and fields)} A proper ideal $\mathfrak{m}\subset A$ is said to be \textbf{maximal} if it is maximal among the proper ideals of $A$. We say a \emph{nonzero} ring $F$ is a  \textbf{field} if and of the followings hold true: 
\begin{enumerate}[label=(\roman*)]
...
\begin{theorem}
\textbf{(Second isomorphism theorem for rings)} Let $B\subseteq A$ be a subring, and $\mathfrak{a}\trianglelefteq  A$. Then
\begin{enumerate}[label=(\roman*)]
...
\begin{theorem}
\label{THtitr}
\textbf{(Third isomorphism theorem and correspondence theorem for rings)} Let $A$ be a ring,  $\mathfrak{a}\trianglelefteq   A$, and $\pi : A \twoheadrightarrow  A \quotient \mathfrak{a}$ be the canonical ring homomorphism. For $\mathfrak{a}\leq \mathfrak{b}\trianglelefteq  A$, the subset $\pi  (\mathfrak{b})\trianglelefteq  A \quotient \mathfrak{a}$ forms an ideal, which we denote by $\mathfrak{b}\quotient \mathfrak{a}$, that makes the natural map $(A\quotient \mathfrak{a})\quotient (\mathfrak{b}\quotient \mathfrak{a})\cong A\quotient \mathfrak{b}$ a ring isomorphism.\\

The \textbf{spectrum} $\operatorname{Spec}(A)$ is the set of prime ideals in $A$, and the \textbf{maximal spectrum}  $\operatorname{Max}(A)$ is the set of maximal ideals in $A$.   The third isomorphism theorem induces a bijection between the collection of ideals of $A$ that contains  $\mathfrak{a}$ and ideals of $A \quotient \mathfrak{a}$, a bijection between the collection of prime ideals of $A$ that contains  $\mathfrak{a}$ and $\operatorname{Spec}(A \quotient \mathfrak{a})$, and a bijection between the collection of maximal ideals of $A$ that contains  $\mathfrak{a}$ and $\operatorname{Max}(A \quotient \mathfrak{a})$. \\

Also, given arbitrary $\mathfrak{c}\trianglelefteq  A$, we always have 
\begin{align*}
\pi  (\mathfrak{c})= \frac{\mathfrak{c}+\mathfrak{a}}{\mathfrak{a}}
\end{align*}
\end{theorem}
\begin{proof}
Routine. 
...
\end{proof}
\begin{Example}{}{}
$A\triangleq \C[x]$ makes $\operatorname{Nil}(A)=\operatorname{Jac}(A)$ but not $\operatorname{Spec}(A)=\operatorname{Max}(A)$. 
\begin{align*}
\operatorname{Spec}
\end{align*}
\end{Example}
...
Since $(6)\trianglelefteq  \Z$ is the intersection of prime ideals $(2)$ and $(3)$, we know $(6)\trianglelefteq \Z$ is radical, but since $2\cdot 3 \in (6)$, we see $(6)\trianglelefteq $ isn't prime.  
\end{Example}
...
\end{enumerate}
Let $\mathfrak{a}_1,\dots , \mathfrak{a}_n \trianglelefteq A$, and consider the natural ring homomorphism $A\overset{\pfi }{\longrightarrow} \prod A \quotient \mathfrak{a}_i$. Clearly, $\pfi $ is injective if and only if $\bigcap \mathfrak{a}_i=0$. We also have: 
\begin{align}
\label{EQpf}
  \pfi\text{ is surjective }\iff  \mathfrak{a}_i\text{ are pairwise comaximal }
\end{align}
which implies \textbf{Chinese remainder theorem for integers}. Moreover, we have:   
\begin{align}
\label{EQa1}
...
\end{proof}
Be careful. Submodules of finitely generated modules need not be finitely generated, with the exception that the base ring is Noetherian 
\begin{Example}{Submodules of finitely generated modules need not be finitely generated over a non-Noetherian ring}{}
Consider 
\begin{align*}
  (X_1,X_2, \dots ) \trianglelefteq \R[X_1,X_2,\dots ]
\end{align*}
Because  $\R[X_1,X_2,\dots ]=(1)$, we know $\R[X_1,X_2, \dots]$ is finitely generated. To see $(X_1,X_2, \dots )$ isn't finitely generated, assume for a contradiction that  $(X_1,X_2,\dots,)=(f_1,\dots ,f_m)$. Let $X_k$ be a variable that doesn't occur in any of $f_i$. Then since $X_k \in (f_1,\dots ,f_m)$, we may write $X_k= g_1f_1+\cdots +g_m f_m$ for some $\set{g_i \in \R[X_1,X_2,\dots ]: 1\leq i\leq m}$. Therefore, $X_k$ must occur in some  $g_if_i$, which is only possible if $X_k$ occurs in  $g_i$ and $f_i$ has nonzero constant term. This then cause a contradiction to $f_i \in (X_1,X_2,\dots )$. 
\end{Example}
Nakayama Lemma and a bunch. 
...
\end{proof}
\section{Domains}
Let $D$ be an integral domain.  $a \in D$ is said to be \textbf{irreducible} if $a \not \in D^{\times}$ and whenever $a=bc$, $b$ or $c\in D^{\times}$.\\

A \textbf{Bézout domain} is an integral domain in which the sum of two principal ideals is also a principal ideal. 
\begin{equiv_def}
\textbf{(GCD domain)}  An  integral domain $D$ is called an  \textbf{GCD domain} if any of the followings hold true: 
\begin{enumerate}[label=(\roman*)]
  \item Every two $a,b \in D$ has a greatest common divisor,  
  \item Every two $a,b \in D$ has a least common multiple. 
\end{enumerate}
...
\begin{equiv_def}
\textbf{(UFD)} An integral domain $D$ is called an \textbf{unique factorization domain} if any of the followings hold true: 
\begin{enumerate}[label=(\roman*)]
  \item $D$ is a Noetherian GCD domain.  
\end{enumerate}
\end{equiv_def}

An nonzero non-unit  $p \in A$ is a \textbf{prime element} if whenever $p$ divides the product of two elements in $A$,  $p$ divides at least one of them. 

Let $a\neq 0 \in A$. Clearly, $(a)\subseteq A$ is prime if and only if $a$ is prime.\\
...
\end{align*}
\end{proof}
Let $k$ be an algebraically closed field. An \textbf{affine algebraic variety} $X\subseteq k^n$ is simply the solution set  to some subset of $k[x_1,\dots ,x_n]$. The \textbf{ideal $I(X)$ of the variety} $X$ is just the set of polynomial functions $f \in k[x_1,\dots ,x_n]$ that vanishes identically on $X$. The \textbf{coordinate function} is simply the image of $x_i\in k[x_1,\dots ,x_n]$  onto the  \textbf{coordinate ring} $P(X)\triangleq k[x_1,\dots ,x_n]\quotient I(X) $. 

For all $x \in X$, because the vanishing set 
\begin{align*}
\mathfrak{m}_x \triangleq \set{f \in P(X)}
\end{align*}
...
\end{theorem}
\begin{proof}
(i) is a consequence of $(x+1)^2=x+1$. (ii) follows from (i). (iii) is proved by induction on number of generators. The base case of single generator is trivial. Let $\mathfrak{a}\triangleq (x_1,\dots ,x_n,y)\trianglelefteq A$. Inductive hypothesis stated that  $(x_1,\dots ,x_n)=(x)$ for some $x\in A$. Let $z \triangleq x+y -xy$. Because $x=xz$ and  $y=yz$,  we have
\begin{align*}
  \mathfrak{a}=(x,y)=(z)
\end{align*}
as desired. 
