...
\begin{equiv_def}
  \textbf{(Sum, product, and quotient for ideals and modules)} Let $A$ be a ring, and $M$ an  $A$-module. The \textbf{sum $\sum M_i$ of} \emph{possibly infinite} \textbf{submodules} $M_i \leq M $ is the set of finite combinations of elements of $M_i$. The \textbf{sum of} \emph{possibly infinite} \textbf{ideals $\mathfrak{a}_i\trianglelefteq  A$} is defined as their sum as submodules. Notably, finite sum of principal ideals satisfies: 
\begin{align*}
  (x) + (y) = (x+y),\quad \text{ for all }x,y \in A  
\end{align*}
  A collection of module homomorphisms  $j_i:M_i\rightarrow \bigoplus M_i$ satisfies the \textbf{universal property for external direct sum} if for every collection $f_i:M_i \rightarrow  M$ of module homomorphism, there exists a unique module homomorphism $f:\bigoplus M_i \rightarrow M$ that makes the diagram: 
% https://q.uiver.app/#q=WzAsMyxbMCwwLCJNX2kiXSxbMiwwLCJhIl0sWzIsMiwiTSJdLFswLDEsImpfaSJdLFswLDIsImZfaSIsMl0sWzEsMiwiZiIsMCx7InN0eWxlIjp7ImJvZHkiOnsibmFtZSI6ImRhc2hlZCJ9fX1dXQ==
\[\begin{tikzcd}
	{M_i} && \bigoplus M_i \\
	\\
	&& M
	\arrow["{j_i}", from=1-1, to=1-3]
...
\end{align*}
In such case, we say  $\sum M_i$ form an  \textbf{internal direct sum}. Let $N,P \leq M$ be two submodules. Their \textbf{quotient} is the ideal of $A$ defined by: 
\begin{align*}
...
\end{align*}
We say $M$ is a  \textbf{faithful module} if $\operatorname{Ann}(M)=0$. Interestingly, if $\operatorname{Ann}(M)\leq \mathfrak{a}$, then $M$ have a natural  $A \quotient \mathfrak{a}$-module structure: 
\[\begin{tikzcd}
...
\end{tikzcd}\]
  ...
  The \textbf{product} of a \emph{finite} set of ideals $\mathfrak{a}_i$ is defined to be the set of all finite sums of products of elements: 
\begin{align*}
...
\begin{theorem}
\textbf{(Ideals of direct product of rings)} Let $\mathfrak{a}\trianglelefteq A_1 \times \cdots \times A_n$. Then 
\begin{align*}
...
Let $M,N$ be two  $A$-modules. The hom-space $\operatorname{Hom}(M,N)$ has a natural $A$-module structure, and  $\operatorname{End}(M)$ has a natural $A$-algebra structure with $1\triangleq \id_M$. Let $P$ be an $A$-module. Clearly, we have a \emph{covariant} functor $\Hom (P,-):\mathbf{Mod}_A \rightarrow \mathbf{Mod}_A$ defined by: 
\begin{align*}
...
\end{align*}
\end{Example}
From the universal property, we see that for all $A$-module  $P$, we have a \emph{covariant} functor $T_P:\mathbf{Mod}_A \longrightarrow \mathbf{Mod}_A$ defined by 
\begin{align*}
T_P(M)\triangleq M \otimes  P\quad \text{ and }\quad  T_P(f) \triangleq f \otimes  \id_P
\end{align*}
where $f \otimes  \id _P : M \otimes  P \rightarrow N \otimes  P$ is induced by the bilinear map 
\begin{align*}
...
\end{align*}
called the \textbf{tensor-hom adjunction}. Concretely, the isomorphism maps $f:M \otimes  N\rightarrow P$ to $f': M \rightarrow \Hom (N,P)$ 
\begin{align*}
...
\item For all $A$-module  $N$, the covariant functor  $T_N$ is right-exact, that is, given any exact sequence: 
\begin{align*}
M' \overset{u}{\longrightarrow } M \overset{v}{\longtwoheadrightarrow  } M'' \longrightarrow 0 
\end{align*}
of $A$-modules, the sequence 
\begin{align*}
M'' \otimes  N \overset{T_N(u)}{\longrightarrow } M \otimes  N \overset{T_N(v)}{\longtwoheadrightarrow  }  M'' \otimes  N \longrightarrow 0
\end{align*}
is exact. 
\end{enumerate}
\end{theorem}
...
\end{align*}
of $\Z$-modules, where $f(x)\triangleq 2x$. Because 
\begin{align*}
  (f \otimes  \id_{\Z_2}) (x \otimes  y) = 2x \otimes  y = x \otimes  2y = 0  
\end{align*}
We know the $\Z$-modules sequence: 
\begin{align*}
0\longrightarrow 0\longrightarrow  \Z \otimes  \Z_2 \overset{f \otimes  \id _{\Z_2}}{\longrightarrow } \Z \otimes  \Z_2
\end{align*}
isn't exact. 
\end{Example}
\begin{theorem}
\textbf{(Identities for tensor products)} 
\begin{enumerate}[label=(\roman*)]
 \item  We have a natural isomorphism
\begin{align*}
A \otimes  M \cong  M,\quad a \otimes  m \mapsto am
\end{align*}
 \item Let $\mathfrak{a}\trianglelefteq A$. Then we have an isomorphism 
\begin{align*}
  (A \quotient \mathfrak{a})\otimes  M \cong  M \quotient \mathfrak{a}M 
\end{align*}
  \item For \emph{possibly infinite} collection $M_i (i \in I)$ of $A$-module, we have a natural isomorphism 
    \begin{align*}
...
\end{theorem}
\begin{proof}
(i): Define  $g:M\rightarrow A\otimes  M$ by $g(m)\triangleq 1 \otimes  m$. We are required to show $f$ and $g$ are inverse to each other. The direction $f \circ g=\id _M$ is clear. Because $A \otimes  M$ is generated by $a \otimes  m$, to show the other direction $g \circ f = \id_{A \otimes  M}$, we only have to show it fixes  $a \otimes  m$: 
\begin{align*}
g \circ  f (a \otimes  m)= g(am)= 1 \otimes  am= a \otimes  m
\end{align*}
(ii): Because \customref{THbpt}{tensor product is right exact}, tensoring the short exact sequence  
\begin{align*}
0 \longrightarrow \mathfrak{a} \overset{\diota }{\longhookrightarrow } A \overset{\pi }{\longtwoheadrightarrow } A \quotient \mathfrak{a} \longrightarrow 0
\end{align*}
with $M$, we get the exact sequence: 
\begin{align*}
\mathfrak{a}\otimes  M \overset{T_M(\diota ) }{\longrightarrow } A \otimes  M \overset{T_M(\pi )}{\longtwoheadrightarrow } (A\quotient \mathfrak{a}) \otimes  M \longrightarrow 0
\end{align*}
This implies that 
 \begin{align*}
   (A \quotient \mathfrak{a}) \otimes  M \cong (A \otimes  M) \quotient \operatorname{Im}(T_M(\diota )) 
\end{align*}
The proof then follows from checking that the isomorphism $A \otimes  M \cong M$ maps $\operatorname{Im}(T_M(\diota ))$ to $\mathfrak{a}M$.  
\end{proof}
...
\end{align*}
which by universal property induces an $A$-module endomorphism of  $B \otimes  _A M$ 
\begin{align*}
b' \otimes  m \mapsto bb' \otimes  m
\end{align*}
Therefore, we may give $M_B \triangleq B \otimes_A M$ an $B$-module structure called   \textbf{extension of scalar}:  
 \begin{align*}
B \longrightarrow \operatorname{End}_{A}(M_B)
\end{align*}
\begin{theorem}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item For each two objects  $A,B \in \mathcal{C}  $, a \emph{class} $\operatorname{Hom}(A,B)$ of \textbf{morphism}. 
...
\end{enumerate}
In a category $\mathcal{C}$, two morphisms $f: A \rightarrow B,g:B\rightarrow A$ are said to be \textbf{inverse to each other} if 
\begin{align*}
g \circ f = \id_A\quad \text{ and }\quad f\circ g = \id _B
\end{align*}
A morphism $f:A \rightarrow B$ is then said to be an \textbf{isomorphism} if it admits an inverse, and in such case, we say $A$ and  $B$ are  \textbf{isomorphic}. A morphism $f: A \rightarrow B$ is said to be a \textbf{monomorphism} if for all $g,h: C \rightarrow A$, we have 
\begin{align*}
f \circ g = f \circ h \implies g=h
\end{align*}
A morphism $f: A \rightarrow B$ is said to be an \textbf{epimorphism} if for all $g,h:B \rightarrow C$, we have 
\begin{align*}
g \circ f = h \circ  f \implies  g=h
\end{align*}
\begin{theorem}
\textbf{(Right inverses are monomorphisms and left inverses are epimorphism)} Let $f:A \rightarrow B$ and $g: B \rightarrow A$ satisfies $g \circ f= \id _A$. Then $f$ is a monomorphism, and $g$ is an epimorphism. 
\end{theorem}
\begin{proof}
Routine. 
\end{proof}
Let $C \in \mathcal{C}$. A \textbf{subobject of $C$} is then an object $A$ together with a monomorphism  $\diota: A \hookrightarrow  C $, and a \textbf{quotient of $C$} is an object $B$ with an epimorphism $\pi  : C \twoheadrightarrow B $.\\

Let $\mathcal{C},\mathcal{D}$ be two categories. A \textbf{covariant functor} $F: \mathcal{C}\rightarrow \mathcal{D}$ is a map of objects $F: \operatorname{ob}(\mathcal{C})\rightarrow \operatorname{ob}(\mathcal{D})$ together with a map of morphisms: 
\begin{align*}
...
\end{align*}
A \textbf{contravariant functor} $F: \mathcal{C}\rightarrow \mathcal{D}  $ is a map of objects $F: \operatorname{ob}(\mathcal{C} )\rightarrow \operatorname{ob}(\mathcal{D} )$ together with a map of morphisms: 
\begin{align*}
F: \operatorname{Hom}_\mathcal{C}(A,B) \rightarrow \operatorname{Hom}_\mathcal{D}(F(B),F(A))  
\end{align*}
such that 
\begin{align*}
F(\id _A)=\id _{F(A)}\quad \text{ and }\quad F(g \circ h) = F(h)\circ F(g)
\end{align*}
Let $F:\mathcal{I}\rightarrow \mathcal{C}$ be a covariant functor. A \textbf{cone to $F$}  is an object $N\in \mathcal{C}$ together with a family of morphisms $\psi_X:N \rightarrow F(X)$ indexed by $\mathcal{I}$ such that for all $f: X \rightarrow Y$, the diagram 
...
\end{tikzcd}\]
commutes. A cone $(\lim F,\pfi )$ to $F$ is said to satisfy the \textbf{universal property of limit} if for all cones $(N,\psi )$ to $F$ there exists a unique morphism $u: N \rightarrow \lim F$ such that $\psi_X= \pfi _X \circ u$ for all $X \in \mathcal{I} $. Therefore, given a limit $(\lim F, \psi)$, we have a commutative diagram: 
...
\end{tikzcd}\]
Let $\mathcal{I}$ be a category that has only identity morphism, and let $F: \mathcal{I}\rightarrow \mathcal{C}$ a covariant functor. Denoting images $F$ by  $A_i$, the  limit $(\lim F,\pfi )$, denoted $\prod A_i \triangleq \lim F$, is then called the \textbf{product} of $A_i$, with $\pfi_i : \prod A_i \rightarrow A_i$ the \textbf{projection map}. Clearly, in the case of modules and sets, the products are indeed direct products and Cartesian products. \\

Let $F:\mathcal{I}\rightarrow \mathcal{C}$ be a contravariant functor. A \textbf{co-cone to} $F$  is an object $N \in \mathcal{C}$ together with a family of morphisms $\psi_X:F(X)\rightarrow N$ indexed by $\mathcal{I}$ such that for all $f: X \rightarrow Y$, the diagram 
...
\[\begin{tikzcd}
	...
	\arrow["{F(f)}"', from=1-3, to=1-1]
	\arrow["{\psi_Y}", from=1-3, to=3-2]
\end{tikzcd}\]
commute. A co-cone $(\colim F,\pfi )$ to $F$ is said to satisfy the \textbf{universal property of colimit} if for all co-cones $(N,\psi )$ to $F$ there exists a unique morphism $u: \colim F \rightarrow N$ such that $\psi_X= u \circ \pfi _S$ for all $X \in \mathcal{I} $. Therefore, given a colimit $(\colim F, \psi)$, we have a commutative diagram: 
...
\end{tikzcd}\]
Denoting images $F$ by  $A_i$, the colimit $(\colim F,\pfi )$, denoted $\coprod A_i \triangleq \lim F$, is then called the \textbf{coproduct} of $A_i$, with $\pfi_i : A_i \rightarrow \coprod A_i$ the \textbf{inclusion map}. Clearly, in the case of modules and sets, the coproducts are indeed direct sums and disjoint union. \\
...
In additive category, kernel is the equalizer $\operatorname{ker}f = \operatorname{eq}(f,0)$ while the cokernel is the coequalizer $\operatorname{coker}(f)=\operatorname{coeq}(f,0)$.\\

image, coimage. There case in modules and set. \\


The induced coimage -> image is isomorphism means it is strict. 

\section{Abelian Category and Its Lemmas}
