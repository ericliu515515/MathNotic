...
\end{enumerate}
Clearly, given an additive category $\mathcal{A}$ and a small category $\mathcal{C}$, the functor category $\operatorname{Func}(\mathcal{C},\mathcal{A})$ form an additive category the obvious way.  
\begin{theorem}
\label{THbpac}
...
\begin{theorem}
\label{THkar}
  \textbf{(Kernels are monomorphisms and cokernels are epimorphisms)} Let $f:A \rightarrow B$ be a morphism in an additive category $\mathcal{A}$ that has both $\diota: \operatorname{ker}(f)\hookrightarrow A $ and $\pi  : B \twoheadrightarrow \operatorname{Coker}(f)$. Then $\diota$ is indeed a monomorphism, and $\pi $ is indeed an epimorphism.  
\end{theorem}
\begin{proof}
Since the proof for two statements are similar, we only prove that $\diota : \operatorname{ker}(f)\rightarrow A$ is a monomorphism. Let $g,h: C \rightarrow \operatorname{ker}(f)$ satisfies $\diota \circ g= \diota \circ h$. This implies 
\begin{align*}
\diota \circ (g-h)= \diota \circ g - \diota \circ h =0   
\end{align*}
which implies 
\begin{align*}
f \circ \diota \circ (g-h)=0  
\end{align*}
Therefore, by universal property of kernel, there exists a unique morphism $u:C \rightarrow \operatorname{ker}(f)$ that makes $\diota \circ u = \diota \circ (g-h)=0 $. Clearly both $u\triangleq 0$ and $u \triangleq g-h$ suffices, so $g-h=0$, as desired. 
\end{proof}
...
Also, note that clearly we may construct kernels and cokernels in $\operatorname{Func}(\mathcal{C},\mathcal{A})$ component wise. \\  
...
\begin{align*}
A \mapsto  \Hom (N,A)\quad \text{ and }\quad F(f)(g)\triangleq f \circ g
\end{align*}
and an additive \emph{contravariant} functor $\Hom  (-,N):\mathcal{A}\rightarrow \mathbf{Ab}$ defined by 
\begin{align*}
A \mapsto \Hom  (A,N)\quad \text{ and }\quad G(f)(g)\triangleq g \circ f
\end{align*}
...
\end{theorem}
\begin{proof}
Since the two proofs are similar, we only prove for the contravariant functor $G$. The direction $(\implies )$ is to say that $G$ is left exact. We first prove this direction. The fact that functors preserve chain complex is clear, so we only have to prove 
\begin{align*}
