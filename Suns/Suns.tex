...
\end{proof}
\begin{corollary}
\textbf{(Geometric consequences of Nakayama lemmas)} Let $A$ be a ring and  $\pfi : A^n \rightarrow A^m$ be a module homomorphism. If $\pfi $ is injective, then  $n \leq m$. Conversely, if $\pfi$ is surjective, then $n\geq m$.  
\end{corollary}
\begin{proof}
...
\end{theorem}
\begin{proof}
(i), (iii), (iv), (vi), and (vii) are routine. (ii) is essentially a corollary of (i). In short, because \customref{THbpt}{tensor product is right exact}, tensoring the exact sequence  
\begin{align*}
