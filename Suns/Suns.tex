...
\end{align*}
on the power set of $G$. Clearly, orbit of a subgroup under left multiplication is its left coset space, while orbit of a subgroup under conjugation are isomorphic groups that have the same index: If $K= gHg^{-1}$, then 
\begin{align*}
xH \mapsto  xg^{-1}K
\end{align*}
forms a well-defined bijection between left coset spaces of $H$ and  $K$. 
\begin{Example}{Isomorphic subgroups don't need to have the same index}{}
Isomorphic subgroups can have different indices. Consider subgroups of  $\Z$.  
\end{Example}
\begin{theorem}
...
\begin{theorem}
\textbf{(Property of finite group)} Let $G$ be a finite group and $p$ the smallest prime that divides $o(G)$. Then any subgroup $H \leq G$ of index $p$ is normal in $G$. 
\end{theorem}
\begin{proof}
...
\end{proof}
\begin{Example}{Normal subgroups of finite $p$-groups intersect with centers}{}
Let $1< N \trianglelefteq  P$, where $P$ is a finite $p$-group. Let $P$ acts on $N$ by conjugation. By \customref{THcl}{our counting lemma for $p$-group}, we have 
\begin{align*}
0 \equiv o(N) \equiv \abso{\set{n \in N: gng^{-1}=n\text{ for all } g \in P}} \pmod{p} 
\end{align*}
Noting that the latter is the set $N \cap Z(P)$, we now see $N \cap Z(P)>1$. 
\end{Example}
\begin{theorem}
\label{THsSt}
...
\end{proof}
Second Sylow theorem may be the most important one out of the three, as it suggests that there is only one conjugacy class within $\operatorname{Syl}_p(G)$,  i.e., it self, which, more importantly, shows that if a sylow $p$-group is unique, then it must be normal.  
\begin{theorem}
\label{THtSt}
...
\end{proof}
\begin{corollary}
\textbf{(Application of Sylow theorems)} Let $p,q,r$ be three distinct prime. 
\begin{enumerate}[label=(\roman*)]
  \item Groups of order $pq$ must be non-simple. Says, $p<q$. If  $p   $
\end{enumerate}
\end{corollary}
\begin{Example}{Groups of order $pq$}{}
  Let $o(G)=pq$ with $p>q$ being prime. Because \customref{THfSt}{$n_p \equiv 1 \pmod{p}$} and \customref{THtSt}{$n_p \mid q$}, we see $n_p=1$, which implies the non-simplicity of $G$.  \\

Suppose $q \nmid p-1$. Then from \customref{THfSt}{$n_q \equiv 1 \pmod{q}$} and \customref{THsSt}{$n_q \mid p$}, we see $n_q=1$, which by \customref{EDidpfg}{recognition theorem} implies $G\cong \Z_p \times \Z_q$. 
\end{Example}
\begin{Example}{Groups of order $p^2q$ must have a normal Sylow subgroup unless $p=2$ and  $q=3$}{} 
Let $o(G)=p^2q$. If $p>q$, then clearly $n_p$ can only be  $1$. If $p<q$, then we must have $n_q \in \set{1,p^2}$. Assume moreover that $n_q= p^2$. Then from \customref{THfSt}{$n_q \equiv 1 \pmod{q}$}, we see $q=p+1$, which can only happens if $q=3$ and  $p=2$.  
\end{Example}
\begin{Example}{Groups of order $p^am$ with  $p\nmid m$}{}
Let $o(G)=p^am$ with $a \inn$ and $p\nmid m$. If $G$ is simple, then $o(G) \mid  n_p!$
\end{Example}
\begin{Example}{Simple group of order $60$ can only be $A_5$}{}
Let $o(G)=60$ and $G$ be simple. Simplicity of $G$ implies $n_2 \in \set{3,5,15} $. 
\end{Example}
\begin{Example}{}{}
Let $o(G)=pqr$ with $p<q<r$ prime. We have  $1 \in \set{n_p,n_q,n_r}$. 
\end{Example}
\begin{Example}{No group of order $132$ is simple}{} 
\end{Example}
