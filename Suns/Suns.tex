...
\textbf{(Homomorphism of direct systems over a fixed directed set)}

\textbf{(Exact sequence of direct systems over a fixed directed set induce exact sequence of the direct limits)} 

\textbf{(Direct limit of ring is $0$ only if some  $A_i$ is  $0$)}

\textbf{(Nilradical of direct limit is a direct limit of nilradicals)} 

\textbf{(Direct limit of integral domains is integral domain)} 
...
\end{align*}
In particular, for all $M_0 \leq M$ and $N_0 \leq N$, given $x\in M_0$ and $y \in M_0$ such that $x \otimes  y =0 \in M_0 \otimes  N$, the module homomorphism from $M_0 \otimes  N_0$ to $M \otimes  N$ tell us that $x \otimes  y$ must also be $0$ in  $M \otimes  N$. The converse however is not true: That is, given $x \in M_0$ and $y \in M_0$, 
\begin{align*}
...
\end{align*}
\item  Let $M_i$ be a directed system of $A$-modules. We have a natural isomorphism 
  \begin{align*}
 \left(\directlimit M_i \right) \otimes  M  \cong  \directlimit \left( M_i \otimes  M \right),\quad \phi_i(x_i)\otimes  m \mapsto \psi_i (x_i \otimes  m)
  \end{align*}
where $\phi$ denote the canonical morphism of $M_i$, while the canonical morphism $\psi$ of $M_i \otimes  M$ are defined by 
\begin{align*}
\psi_{ij}\triangleq \phi_{ij}\otimes  1
\end{align*}
  \item For \emph{possibly infinite} collection $M_i$ of $A$-module, we have a natural isomorphism 
   ...
   N \otimes  \bigoplus M_i \cong  \bigoplus (N \otimes  M_i)
    \end{align*}
  \item  Let $M_i$ be a directed system of  $A$-modules over  directed set $\mathcal{P}$ and $N_i$ be a directed system of  $A$-modules over directed set  $\mathcal{Q}$. Clearly, we may make $\mathcal{P}\times \mathcal{Q}$ a directed set by 
    \begin{align*}
      (p,q) \leq  (p',q')\overset{\triangle}{\iff } p \leq  p' \quad \text{ and }\quad q\leq q'
    \end{align*}
Then the natural directed system of $M_i \otimes  N_j$ over $\mathcal{P}\times \mathcal{Q}$ has a natural isomorphism 
\begin{align*}
 \directlimit_{\mathcal{P}\times \mathcal{Q}}\left( M_i \otimes  N_j \right)  \cong  \directlimit_{\mathcal{P} } \left( M_i \right) \otimes  \directlimit_{\mathcal{Q} } \left(N_j   \right)
\end{align*}
\end{enumerate}
\end{theorem}
\begin{proof}
(i) follows from checking that the map $a \otimes  m \mapsto am$ and $m \mapsto 1 \otimes  m$ are inverse to each other. (ii) is essentially a corollary of (i). In short, because \customref{THbpt}{tensor product is right exact}, tensoring the exact sequence  
\begin{align*}
...
\end{align*}
The proof for (ii) then follows from checking that the isomorphism $A \otimes  M \cong M$ maps $\operatorname{Im}(\diota \otimes  1 )$ to $\mathfrak{a}M$. The proof for (iii) is simply checking that we have a well-defined homomorphism 
\begin{align*}
  \directlimit\left(M_i \otimes  M \right) \rightarrow \left( \directlimit M_i \right) \otimes  M,\quad \psi_i (x)\mapsto  \phi_i \otimes  1 (x)
\end{align*}
and it together with the proposed homomorphism are inverse to each other.  \\

(v) in nature is a statement for all colimits, rather than just directed system of modules. See \href{https://stacks.math.columbia.edu/tag/002M}{Stack Project Lemma 4.14.10}
\end{proof}
\begin{theorem}
\textbf{(Existence of finitely generated submodules that makes an expression of zero remain zero)} Let  $A$ be a ring and  $M$ and  $N$ two  $A$-modules. Let $\sum x_i \otimes y_i =0 \in M \otimes  N$. Then there exist two finitely generated submodules $M' \leq M,N' \leq N$ that respectively contains $x_i,y_i$ and makes
\begin{align*}
\sum  x_i \otimes  y_i= 0 \in M' \otimes  N'
\end{align*}
\end{theorem}
\begin{proof}
Let $M_i$ and $N_i$ be the directed system of finitely generated submodules of $M$ and $N$. Then   
\begin{align*}
...
\end{align*}
Let $M_0 \leq M$ and $N_0\leq N$ be the submodules generated by $x_i$ and  $y_i$. By definition ?, the canonical morphism $M_0 \otimes  N_0 \rightarrow M \otimes  N$ maps $\sum  x_i \otimes  y_i$ to $0$. The proof then follows from \customref{THpdl}{this property of direct limit}. 
\end{proof}
\textbf{(Tensor product of infinite algebras)}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item For all injective homomorphism $f:M' \rightarrow M$ between finitely generated modules, the homomorphism $f \otimes  1:M' \otimes N \rightarrow M \otimes  N $ is injective. 
\end{enumerate}
\end{equiv_def}
\begin{proof}
(i)$\implies $(ii) is clear, so we only have to prove (ii)$\implies $(i). Let $f:M'\rightarrow M$ be an injective homomorphism.  
\end{proof}
Let $f:M' \rightarrow M$ be injective and let $u=\sum x_i' \otimes y_i \in \operatorname{ker}(f \otimes 1 )$, so that $\sum f(x_i')\otimes  y_i=0$ in $M \otimes  N$. Let $M_0'$ be the submodule of  $M'$ generated by the  $x_i'$ and let  $u_0$ denote  $\sum x_i'\otimes  y_i$ as an element of $M'_0 \otimes  N$. By (2.13) there exists a finitely generated submodule $M_0$ of  $M$ containing  $f(M_0')$ and such that $\sum  f(x_i')\otimes  y_i=0$ as an element of $M_0 \otimes  N$. If $f_0:M'_0 \rightarrow M_0$ is the restriction of $f$, this means that  $(f_0 \otimes  1)(u_0)=0$. Since $M_0$ and  $M_0'$ are finitely generated,  $f_0 \otimes  1$ is injective and therefore $u_0=0$, hence  $u=0$.   \\

\begin{align*}
M \otimes  N \cong  (\directlimit M') \otimes  (\directlimit N') \cong \directlimit [M' \otimes  \directlimit N'] \cong  \directlimit [\directlimit (M'\otimes  N')] 
\end{align*}
...
\end{align*}
Clearly, if morphism $f: A \rightarrow B$ is a right inverse of some $g:B \rightarrow A$, then $f$ must be a monomorphism, and if $f$ is a left inverse of some $h:B \rightarrow A$, then $f$ must be an epimorphism. Let $B \in \mathcal{C}$. A \textbf{subobject of $B$} is an object $A$ together with a monomorphism  $\diota: A \hookrightarrow  B $, and a \textbf{quotient of $B$} is an object $C$ with an epimorphism $\pi  : B \twoheadrightarrow C $.
...
\begin{theorem}
\label{THpdl}
\textbf{(Basic properties of direct limit)}  Let $A$ be a ring, $I$ a directed set, and $\set{M_i, \phi_{ij}}$ a directed system of $A$-modules. Then 
\begin{enumerate}[label=(\roman*)]
...
Let $\set{A_i}$ be a collection of objects in $\mathcal{C}$. Clearly, we can make $\set{A_i}$ a category $\mathcal{J}$ by requiring it to have only the  identity morphisms, and doing such, we have an obvious covariant functor $F:\mathcal{J}\rightarrow \mathcal{C}$, whose limit is called the  \textbf{product of $\set{A_i}$} denoted by $\prod A_i$, with canonical morphism $p_i : \prod A_i \rightarrow A_i$ called the \textbf{projection map}.\\

The colimit of $F: \mathcal{J}\rightarrow \mathcal{C}$ is called the \textbf{coproduct of $\set{A_i}$} denoted by $\coprod A_i$, whose canonical morphism $A_i \rightarrow \coprod A_i$ are often called the \textbf{coprojection map}. 
\begin{theorem}
\textbf{(Existence of products in familiar categories)} Product and coproduct always exists in $\mathbf{Set}$. They are respectively Cartesian product and disjoint union. Let $A$ be a ring. Product and coproduct always exists $\mathbf{Mod}_A$. They are direct product and direct sum.  
\end{theorem}
\begin{proof}
