...
\end{Example}
\begin{theorem}
\textbf{(Group property of totient function)} Let $\pfi $ be the Euler totient function. For all $a,n \inn$, we have $n \mid \pfi (a^n-1)$.
\end{theorem}
\begin{proof}
...
\end{theorem}
\begin{proof}
The set $X$ of $p$-tuples  $(x_1,\dots ,x_p)$ that satisfies $x_1\cdots x_p=e$ clearly has cardinality $o(G)^{p-1}$. Consider the group action $\Z_p \rightarrow \operatorname{Bij}(X)$ defined by 
\begin{align*}
...
\begin{enumerate}[label=(\roman*)]
\item If there exists a unique subgroup $H \leq G$ of a fixed index, then $H$ is is characteristic. 
...
\end{Example}
Note that even though normal subgroups need not be preserved by automorphisms, normality is: Given $N \trianglelefteq G$ and $f\in \operatorname{Aut}(G)$, we also have $f(N)\trianglelefteq G$. 
\begin{theorem}
\label{THfit}
...
\end{proof}
\begin{equiv_def}
\textbf{(Core)} Let $H\leq G$, and let $\phi : G \rightarrow \operatorname{Bij}(G \quotient H)$ be the left multiplicative action on the left coset space of $H$. We have 
\begin{align*}
\operatorname{ker} \phi = \bigcap_{g \in G} H^g, \quad \text{ where }H^g \triangleq gHg^{-1}
\end{align*}
and they are the largest subgroup of $H$ normal in  $G$, called the  \textbf{core} of $H$. 
\end{equiv_def}
\begin{proof}
Routine. 
\end{proof}
...
\begin{theorem}
\textbf{(Properties of core)} Let $G$ be finite. Then 
\begin{enumerate}[label=(\roman*)]
  \item $[G:\operatorname{Core}(H)]$ divides $ [G:H]!$, for all $H\leq G$ 
  \item Any proper subgroup of $G$ of smallest possible index is normal. 
\end{enumerate}
\end{theorem}
\begin{proof}
  (i) is a consequence of \customref{THfit}{first isomorphism theorem}, since we have an injective group homomorphism $G\quotient \operatorname{Core}(H)\longhookrightarrow \operatorname{Bij}(G \quotient H)$. (ii) follows from one, since we would get $\operatorname{Core}(H)=H$.   
\end{proof}
\begin{equiv_def}
...
\end{align*}
is the smallest normal subgroup of $G$ that contains $S$. This is called the \textbf{normal closure} of $S$ in  $G$.  
\end{equiv_def}
\begin{proof}
...
2a \equiv 0 \pmod{n}
\end{align*}
Clearly we have $D_1 \cong  \Z_2$ and $D_2 \cong  \Z_2 \times \Z_2$. Therefore, we from now on suppose $n\geq 3$. \\

Let $n$ be odd.  Then clearly  $a=0$. Since  $rs\neq sr$ in general, we see that we also must have $b=0$. We have shown that $D_n$ has trivial center for odd $n\geq 3$. \\
...
\begin{equiv_def}
\label{EDpoc}
\textbf{(Property of center)} Let $S\subseteq G$. We have 
\begin{align*}
S \subseteq Z(G) \iff  C_G(S)=G
\end{align*}
\end{equiv_def}
\begin{proof}
Routine.
\end{proof}
Clearly, regardless of $G$, we always have a natural surjective group homomorphism  $G \longtwoheadrightarrow \operatorname{Inn}(G)$, such group homomorphism is injective (and thus an isomorphism) if and only if $G$ is centerless. \\


Because every element of the center form a single conjugacy classes, by \customref{THost}{orbit-stabilizer theorem}, we have the \textbf{class equation}:
\begin{align*}
\label{THce}
...
\begin{theorem}
\label{THpcs}
\textbf{(Property of commutator subgroup)} Given normal subgroup $N \trianglelefteq G$, if $G' \cap N=1$, then $N \leq Z(G)$. 
\end{theorem}
\begin{proof}
...
\end{proof}
\begin{Example}{Commutator subgroup of $\operatorname{GL}_2(\R)$ is $\operatorname{SL}_2(\R)$}{}
Clearly we have $ (\operatorname{GL}_2(\R))' \leq \operatorname{SL}_2(\R)$. The opposite relation requires computation. We offer a fast way. Write $[g,h]\triangleq ghg^{-1}h^{-1}$. Compute that 
\begin{align*}
\begin{pmatrix} 
  1 & x \\
  0 & 1 
...
\end{pmatrix} \begin{pmatrix} 
  a & 0\\
  0 & \frac{1}{a}
\end{pmatrix} \in \left(\operatorname{GL}_2(\R) \right)' 
\end{align*}
Compute that 
\begin{align*}
\begin{pmatrix} 
...
\end{Example}
\begin{theorem}
\textbf{(Schur's upper bound for order of commutator subgroup)} Let $C\subseteq G$ be the set of commutators.  If $[G:Z(G)]\triangleq n$, then
\begin{align*}
 \abso{C}\leq n^2 \quad \text{ and }\quad o\left(G^{(1)} \right) \leq n^{2n^3}
\end{align*}
\end{theorem}
\begin{proof}
To prove $\abso{C}\leq n^2$, we show that 
\begin{align*}
\vi{\begin{cases}
  aZ(G)=cZ(G) \\
  bZ(G)=dZ(G)
...
\end{proof}
\begin{theorem}
\textbf{(Schur's upper bound for index of center)} Let $G\triangleq \langle g_1, \dots , g_m\rangle $ be finitely generated. If $o(G^{(1)})\triangleq n$, then 
\begin{align*}
[G:Z(G)] \leq n^m
\end{align*}
\end{theorem}
...
\begin{equiv_def}
\textbf{(Abelian)} We call a group $G$ \textbf{abelian} if any of the followings hold true 
\begin{enumerate}[label=(\roman*)]
  \item  $Z(G)=G$
  \item $G'=1$ 
...
Routine. 
\end{proof}
Interestingly, our equivalence definition for Abelian immediately gives a corollary on automorphism groups: 
\begin{corollary}
...
\end{theorem}
\begin{proof}
See Chapter 10, Section 1, Corollary 1 of the book Presentations of Groups (2nd ed.) by D.L. Johnson. The proof is not difficult, but without his approach, can be rather lengthy and tedious.    
\end{proof}
...
\end{proof}
Suppose $N\trianglelefteq  G$ and $H \leq G$ satisfies the conditions in the \customref{EDrip}{recognition theorem for inner semidirect product}. Defining $\phi : H \rightarrow \operatorname{Aut}(N)$ by $\phi_h(n)\triangleq hnh^{-1}$, we see that the natural map $N \rtimes_{\phi} H \rightarrow G$ indeed forms a well-defined group isomorphism. Note that if $H$ is also normal in $G$, then the action  $\phi_h(n)=hnh^{-1}$ become trivial, since we would have $[H,N]\subseteq H \cap N= 1$. This agrees with the \customref{EDidpfg}{recognition theorem for direct product}. 
\begin{Example}{Inner semidirect product}{}
View $\Z_2$ as a subgroup of  $S_n$ by sending  $1\mapsto  (1,2)$. We see that we have a right split sequence:  
\begin{align*}
1 \longrightarrow A_n \longrightarrow S_n \overset{\operatorname{sgn}}{\longrightarrow } C_2 \longrightarrow 1
\end{align*} 
Therefore, we may write 
\begin{align*}
S_n \cong  A_n \rtimes_{\phi} C_2,\quad \text{ with }\phi_1 (g) = (1,2)g (1,2)^{-1}
\end{align*}
Let $\F$ be a field, and view $\F^{\times}$ as a subgroup of $\operatorname{GL}_n(\F)$ by sending $c$ to $\operatorname{diag}(c,1,\dots ,1)$. We see that we have a right split sequence: 
\begin{align*}
...
\begin{align*}
\left(\prod_{i \in I} G_i \right)' \leq \prod_{i\in I} (G_i')
\end{align*}
...
\end{Example}
\begin{Example}{The requirement in definitions of internal direct products for groups}{}
Let $G=\Z \times \Z$. Clearly $N_1 \triangleq \Z \times 1,N_2 \triangleq 1 \times \Z,N_3 \triangleq \set{(x,x)\in G: x \inz}$ satisfies 
\begin{align*}
...
\end{proof}
\customref{THfgag}{Structure theorem for finitely generated abelian group} in fact also gives a structure theorem for automorphism group of finite abelian group. See \href{https://arxiv.org/pdf/math/0605185}{this paper}. A particular case is that $\operatorname{Aut}(C_p^n)\cong \operatorname{GL}_n(\F_p)$. 
\begin{corollary}
\label{COfao}
...
\end{proof}
The slickest application of Sylow theorem is without a doubt \customref{THWt}{Wilson's theorem}. 
\begin{theorem}
\textbf{($a^n \equiv 1 \pmod{p}$ has at most $n$ solution for all  $n$)}
\end{theorem}
\begin{proof}
Because $\Z_p^{\times}\cong  C_{p-1}$ is cyclic, we may write $a= x^c$ with $C_{p-1}=\langle x\rangle $, and the proof reduce to finding number of solution to 
\begin{align*}
  p \mid  nc
\end{align*}
We then see that the solution set is 
\begin{align*}
c \in \set{\frac{p}{\operatorname{gcd}(p,n)}, \dots ,\frac{p \left(\operatorname{gcd}(p,n)-1 \right)}{\operatorname{gcd}(p,n)} }
\end{align*}
\end{proof}
\begin{theorem}
\label{THWt}
...
\end{align*}
Because \customref{THzpc}{$\Z_p^{\times} \cong  C_{p-1}$ is cyclic}, we know the inversion $x\mapsto x^{-1}$ forms a bijection on its non-trivial elements. Therefore, we have 
\begin{align*}
...
\end{align*}
if $\operatorname{Stab}_{A_n}(x)\leq A_n$, then the conjugacy class of $x$ in $A_n$ is the half of its conjugacy class in $S_n$. On the other hand, if $\operatorname{Stab}_{A_n}(x)\not \leq A_n$, then since $A_n\operatorname{Stab}_{A_n}(x)=S_n$, by \customref{THof}{order formula}, we know $[A_n:\operatorname{Stab}_{A_n}(x)]=[S_n:\operatorname{Stab}_{S_n}(x)]$, which implies that the conjugacy class of $x$ in $A_n$ is the same as  in  $S_n$. 
\end{Example}
\begin{theorem}
\label{THgos}
\textbf{(generator of $S_n$)} Let $n\geq 2$. Then $S_n$ is generated by  transpositions or $\set{(1,2),(1,\dots ,n)}$ or $\set{(1,k) \in S_n : 2 \leq k \leq n}$
\end{theorem}
\begin{proof}
...
\begin{align*}
  (i,j)= (1,i)(1,j)(1,i)^{-1}
\end{align*}
We only have to show $(1,i)$ can be generated by $\set{(1,2),(1,\dots ,n)}$, which is proved via two inductions, with one of them being 
\begin{align*}
...
\begin{enumerate}[label=(\roman*)]
  \item $A_3$ is simple. 
  \item $A_4$ is not simple. It contains a characteristic $2$-Sylow subgroup, i.e., the set of product of two disjoint transposition.  
...
\end{proof}
\section{Analysis of some finite group}
...
\begin{corollary}
\label{COaSo}
\textbf{(Analysis of finite group of fixed prime structure)} Let $p,q,r$ be three distinct prime. 
\begin{enumerate}[label=(\roman*)]
...
\end{align*}
A \textbf{composition series} is a maximal normal series. 
\begin{Example}{Subgroups in normal series need not all be normal}{}
Consider 
\begin{align*}
\langle s\rangle  \trianglelefteq \langle s,r^2\rangle  \trianglelefteq D_4 \triangleq \langle r,s \mid r^4=s^2=e,srs^{-1}=r^{-1} \rangle 
\end{align*}
Clearly $\langle s\rangle $ is not normal in $D_4$.   
\end{Example}
\begin{equiv_def}
...
\begin{align*}
\cdots \trianglelefteq  G^{(2)} \trianglelefteq G^{(1)}\trianglelefteq G^{(0)} \triangleq  G,\quad \text{ where }G^{(k+1)}\triangleq \left(G^{(k)} \right)'
\end{align*}
reach to $1$. 
\end{enumerate}
\end{equiv_def}
...
\begin{theorem}
\label{THpp}
\textbf{(Properties of $p$-groups)}
\begin{enumerate}[label=(\roman*)]
  \item $p$-groups are nilpotent, and thus solvable. 
\end{enumerate}
\end{theorem}
Let $G$ be a finite $p$-group. Since \customref{THfp}{$p$-groups has nontrivial centers}, we see that the upper central series of $G$ must reach to $G$. Therefore $p$-groups are nilpotent.   
\begin{equiv_def}
\label{EDfng}
...
\end{equiv_def}
\begin{proof}
(i)$\implies $(ii): This is done via induction on $o(G)$. Let $H<G$. If $Z(G)$ isn't contained by $H$, then since $H \cdot Z(G)$ normalize $H$, we see $H$ satisfies normalizer condition. Let $Z(G)\leq H$. If $G$ is abelian, then the proof is trivial, so suppose $G$ isn't abelian, and therefore  $o\left(G \quotient Z(G) \right)< o(G)$. Then by inductive hypothesis, $\frac{H}{Z(G)}\leq  \frac{G}{Z(G)}$  satisfies normalizer condition. The proof then follows from pullback. \\
...
(iv)$\implies $(i): This follows from the fact that \customref{THpp}{$p$-groups and nilpotent} and that nilpotency is closed under taking finite direct product. 
\end{proof}
...
\end{align*}
We know $S_3$ is solvable. However, it clearly has $3$ Sylow $2$-subgroups, so we know $S_3$ isn't nilpotent.  
\end{Example}
\section{$p$-group}
...
\begin{align*}
\operatorname{PSL}_n(\F) \triangleq \operatorname{SL}_n(\F) \quotient \set{c I_n \in \operatorname{SL}_n(\F): c^n =1}
\end{align*}
\begin{theorem}
\label{THpslg}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item $\operatorname{SL}_n(\F)$ has center $\set{cI_n \in \operatorname{SL}_n(\F):c^n=1}$, thus equal to the kernel of its action on $\P^{n-1}$.
\end{enumerate}
\end{theorem}
\begin{proof}
...
\begin{enumerate}[label=(\roman*)] 
  \item $I_n + \ld  E_{ij}$ is conjugate to $I_n+ E_{12}$ in $\operatorname{SL}_n(\F)$, for all $\ld \in \F^{\times}$. 
  \item $\set{I_n + \ld E_{ij}}$ generates $\operatorname{SL}_n(\F)$. 
\end{enumerate}
...
A subgroup $H\leq G$ of a finite group $G$ is called a \textbf{Hall subgroup} if its order is coprime to its index. 
\begin{theorem}
...
\end{theorem}
\begin{theorem}
\textbf{(Schreier theorem)} Outer automorphism group of finite simple groups are solvable. 
\end{theorem}
\begin{theorem}
...
\end{question}
\begin{proof}
Write $G$ in its  \customref{THfgag}{primary decomposition form}. We are required to show that its $p$-torsion subgroup has at most one component. This follows from noting that for $d,e\geq 1$, the group $\Z_{p^d}\times \Z_{p^e}$ has $p^2$ number of solution to the equation $x^n=e$.  
\end{proof}
\begin{question}{}{}
...
\end{proof}
\begin{question}{}{}
Let $a \in G$ lies in a conjugacy class of finite cardinality and has finite order. Show that the normal closure of $a$ is of finite order. 
\end{question}
\begin{proof}
Recall that 
\begin{align*}
\operatorname{ncl}_G(\set{a})= \set{g_1a^{\epsilon_1}g^{-1}_1\cdots g_na^{\epsilon _n}g^{-1}_n: n\inn \cup  \set{0}\text{ and }\epsilon _i \in \set{\pm 1}}
\end{align*}
