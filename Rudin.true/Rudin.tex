...
In this section, we will introduce Cauchy-Schwarz inequality and the idea of norm and inner product.\\

When we use the word \textit{norm}, or the word \textir{norm function}, we mean a function that maps vectors in a complex or real vector space to non-negative real numbers.\\

Because we haven't define complex numbers, the following definition should be interpreted as a "preview".   
\end{minipage}}
\begin{definition}
\label{1.5.2}
...
When we use the word \textit{Euclidean $n$-Space}, we mean $\R^n$ as a vector space equipped with a function $\gen{\cdot,\cdot}:\R^n\times\R^n\rightarrow \R$ called inner product defined by
\begin{equation}
...
$2$-norm is also called Euclidean Norm or  $L_2$ norm. Notice that we also use the notation $\vecta{x}\cdot\vecta{y}$ in place of $\gen{\vecta{x},\vecta{y}}$, and when $n=1$, we can use \myref{Definition}{1.5.2} to verify $\norm{\vecta{x}}=\sqrt{\abso{x_1}^2} =\abso{x_1}$, so in Euclidean space, no matter of how many dimension, we also use the notation $\abso{\vecta{x}}$ in place of $\norm{\vecta{x}}$. Lastly, we can use \myref{Definition}{1.5.2}, to verify $\norm{\vecta{x}}=\gen{\vecta{x},\vecta{x}}$ in Euclidean Space. 
\end{definition}
\fbox{\begin{minipage}{39em}
From now, we will just use $\R^n$ to denote Euclidean  $n$-space. 
\end{minipage}}
\begin{theorem}
\label{1.5.5}
...
\end{align}
$f(t)$ is nonnegative quadratic since $f(t)$ is a sum of squares. Then $D(f)\leq 0$; that is $4(\vecta{x}\cdot\vecta{y})^2-4\abso{\vecta{x}}^2\abso{\vecta{y}}^2\leq 0$, which implies $(\vecta{x}\cdot\vecta{y})^2\leq \abso{\vecta{x}}^2\abso{\vecta{y}}^2$, and further implies $\abso{\vecta{x}\cdot \vecta{y}}\leq \abso{\vecta{x}}\abso{\vecta{y}}$     \\

If $\abso{\vecta{x}}\abso{\vecta{y}}=\abso{\vecta{x}\cdot\vecta{y}}$, Then $D(f)=0$, that is, there exists a unique $t'\inr$ such that $f(t')=0$ 
\end{proof}
\section{Complex Numbers}

\begin{definition}
\label{1.6.1}
\textbf{(Definition of absolute value)} Let $x\inC$.\\
\begin{equation}
\abso{a+bi}=\sqrt{a^2+b^2} 
\end{equation}
\end{definition}
\begin{definition}
\label{1.6.2}
\textbf{(Definition of inner product)} Let $V$ be a vector space over $\F$ where  $\F$ is either  $\R$ or  $\C$. An inner product $\gen{\cdot,\cdot}:V\times V\rightarrow \F $ is a function that satisfy 
\begin{equation}
\forall \vecta{x,y}\inF,\gen{\vecta{x},\vecta{y}}=\overline{\gen{\vecta{y},\vecta{x}}} \text{ ( Conjugate symmetry) }
\end{equation}
\begin{equation}
...
\end{equation}
\begin{equation}
\forall \vecta{x}\inV, \gen{\vecta{x},\vecta{x}}>0  
\end{equation}
\end{definition}
\begin{definition}
\label{1.6.3}
\textbf{(Definition of norm function)} Let $V$ be a vector space over  $\F\subseteq\C$. A norm, or norm function, $p:V\rightarrow \R$ is a function that satisfy 
\begin{equation}
  \forall \vecta{x,y}\inV, p(\vecta{x}+\vecta{y})\geq p(\vecta{x})+p(\vecta{y})  \text{( Subadditivity/Triangle inequality)}
\end{equation}
\begin{equation}
...
\begin{theorem}
\label{1.6.4}
\textbf{(Cauchy-Schwarz inequality)} Let $V$ be a complex inner product space, and let  $\vecta{v,w}\inV$. We have  
 \begin{equation}
\norm{\vecta{v}} \norm{\vecta{w}}\geq \abso{\gen{\vecta{v},\vecta{w}}}
\end{equation}
and the equality hold when $\vecta{v}=\ld\vecta{w}$
\end{theorem}
\begin{definition}
\label{1.6.5}
\textbf{(Definition of $p$-norm)} Let $p\geq 1\inR$, and let $\vecta{x}=(x_1,\dots ,x_n)$, where $x_1,\dots ,x_n$ can be real or complex.\\
\begin{equation}
\norm{\vecta{x}}_p:=(\sum_{i=1}^{n}\abso{x_i}^p)^{\frac{1}{p}} 
\end{equation}
