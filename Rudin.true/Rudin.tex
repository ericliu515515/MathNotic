...
\begin{theorem}
\label{1.6.4}
Euclidean norm satisfy the three axioms.
\end{theorem}
\begin{proof}
We started from the last sentence of \myref{Definition}{1.5.5}, for which we have $\abso{\vecta{x}+\vecta{y}}=((\vecta{x}+\vecta{y})\cdot(\vecta{x}+\vecta{y}))^{\frac{1}{2}}$ 
\end{proof}
\begin{axiom}
\label{1.6.5}
\textbf{(Axioms of inner product)} Let $V$ be a vector space over $\F$, where $\F$ is either $\R$ or $\C$. An inner product $\gen{\cdot,\cdot}:V\times V\rightarrow \F $ is a function that satisfy the following three axioms: 
\begin{equation}
...
\end{equation}
\begin{equation}
\forall \vecta{x}\inV, \gen{\vecta{x},\vecta{x}}\geq 0 \text{ ( Positive Semi-definitness) }  
\end{equation}
