...
\end{proof}
\fbox{\begin{minipage}{39em}
We now give the formal definition of neighborhood while a topology is given, and translate \myref{Theorem}{3.12.5} into language without neighborhood function. 
\end{minipage}}
\begin{definition}
\label{2.1.6}
...
\end{corollary}
\begin{proof}
Let $\mathcal{O}$ be a countable base for $X$. For each $O\in \mathcal{O}$, we can collect a point $x_O\in O$, and have $A:=\set{x_O:O\in \mathcal{O}}$. Because $\mathcal{O}$ is countable, we know $A$ is countable, and by  \myref{Theorem}{3.12.4}, we know $A$ is dense.
\end{proof}
\fbox{\begin{minipage}{39em}
...
Notice \myref{Theorem}{3.7.6} will prove that our definition of open do satisfy the axioms of open sets. In other words, every metric induce a topology. 
\end{minipage}}
\begin{theorem}
\label{3.5.5}
...
\end{proof}
\fbox{\begin{minipage}{39em}
    Notice \myref{Theorem}{3.12.3} is written in language of general topology. Now, we prove that our definition of closed and limit point in metric space is compatible to those in general topology. Then we can apply the result we have in general topology into metric space.
\end{minipage}}
\begin{theorem}
\label{3.6.4}
...
\end{corollary}
\fbox{\begin{minipage}{39em}
Notice \myref{Corollary}{3.7.6} is proved using language of general topology in the first section of this chapter. I personally haven't tried to prove \myref{Corollary}{3.7.6} using limit point definition, but I think this won't be difficult.\\
...
\end{theorem}
\begin{proof}
For $E^\circ \subseteq F^\circ $, notice an open ball contained by $E$ is also contained by $F$. For  $\overline{E}\subseteq \overline{F}$, notice $E'\subseteq F'$, by \myref{Theorem}{3.10.2}
\end{proof}
\begin{theorem}
\label{3.9.4}
...
\end{corollary}
\fbox{\begin{minipage}{39em}
Also, notice \myref{Theorem}{3.10.1} have been proved in language of general topology, and here we give a simple proof in language of metric.\\
...
\end{theorem}
\begin{proof}
Because by \myref{Corollary}{3.12.5}, $X$ is separable if  $X$ has a countable base, we only have to prove separable implies existence of countable base. Let $X$ be separable. Then we have a countable dense subset $E$ of $X$. We now prove
\vi{
 \begin{equation}
...
\end{theorem}
\begin{proof}
  Let $\mathcal{G}$ be an open cover for $K$. By \myref{Theorem}{3.12.3}, we know $K$ is separable. Then by  \myref{Theorem}{3.12.1}, we know $K$ has a countable base. Then by \myref{Theorem}{2.3.13}, we know $\mathcal{G}$ contain a countable sub-cover
  \begin{equation}
  \set{G_i}_{i\inn}
  \end{equation}
...
In summary 
\begin{gather}
\text{(2)$\implies $(1) by \myref{Corollary}{3.12.5}}\\
\text{(1)$\implies $(2) by \myref{Theorem}{3.12.1}}\\
\text{(a)$\implies $(1) and (2) by \myref{Theorem}{3.12.3}}\\
\text{(2)$\implies $(i) by \myref{Theorem}{2.3.13}}\\
\text{(a)$\implies $(b) by \myref{Theorem}{3.12.4}, which use  (i)}\\
...
\end{equation}
Denote this sequence $\set{I_n}_{n=1}^\infty$. By \myref{Lemma}{3.12.4}, we know 
\begin{equation}
...
Now, using \myref{Lemma}{3.12.3}, it is easy to verify when  $m>\log_2 \frac{\delta}{r}$, the k-cell $I_m$ is small enough to be contained by $B_r(\vecta{x})\tCaC$.
\end{proof}
\begin{corollary}
\label{3.12.5}
...
\end{corollary}
\begin{proof}
  We have proved from right to left in \myref{Theorem}{3.12.3} and  \myref{Theorem}{3.12.5}. To prove from left to right, just observe $K$ is bounded implies that $K$ is contained by some k-cell. Then because closed subset of compact set is compact, we can deduce $K$ is compact.
\end{proof}
...
\end{equation}
Then by \myref{Corollary}{4.2.6}, we know there exists a unique $p$ such that
 \begin{equation}
...
Pick a k-cell that contain $\set{p_n}$. We know k-cell is compact. By \myref{Theorem}{4.2.8}, we see $\set{p_n}$ must converge to some point in that k-cell.
\end{proof}
\begin{corollary}
\label{4.2.10}
...
\begin{definition}
\label{4.3.1}
\textbf{(Definition of Sub-sequential Limit)} We say 
\begin{gather}
 x \text{ is a sub-sequential limit of $\set{x_n}$}\\
\liff \text{ there exists a sub-sequence $\set{x_{f(n)}}$ such that $\lim_{n\to\infty} x_{f(n)}=x$ }  
\end{gather}
\end{definition}
\fbox{\begin{minipage}{39em}
Above is specification. We first give definition of lim-sup.
\end{minipage}}
\begin{definition}
\label{4.3.2}
\textbf{(Definition of Limit Superior)} For real sequence $\set{x_n}$, we define
\begin{equation}
\liminf_{n\to\infty} x_n=\lim_{n\to\infty}\inf \set{x_k:k>n}
\end{equation}
And
\begin{equation}
\limsup_{n\to\infty} x_n=\lim_{n\to\infty} \sup \set{x_k:k>n}
\end{equation}
Notice that the sequence $\set{\inf \set{x_k:k>n}}_{n\inn}$ monotonically increase, and the sequence $\set{\sup \set{x_k:k>n}}_{n\inn}$ monotonically decrease.
\end{definition}
\begin{theorem}
\label{4.3.3}
\textbf{(Main Property of Limit Superior)} 
\begin{equation}
\Lambda  >\limsup_{n\to\infty} x_n \implies  \text{ There exists only finite terms greater than $\Lambda $}
\end{equation}
\begin{equation}
\text{ There exists infinite terms greater than $\Lambda $ }\implies \Lambda \leq \limsup_{n\to\infty} x_n
\end{equation}
\end{theorem}
\begin{proof}
We first prove
\begin{equation}
  \vi{\Lambda  >\limsup_{n\to\infty} x_n \implies  \text{ There exists only finite terms greater than $\Lambda $}}
\end{equation}
Because $\Lambda >\limsup_{n\to\infty} x_n$, we know there exists $N$ such that 
\begin{equation}
\Lambda >\sup \set{x_n:n>N}
\end{equation}
In other words,
\begin{equation}
\exists N,\forall n>N, x_n\leq \sup \set{x_n:n>N}<\Lambda 
\end{equation}
Then we know the amount of terms greater than $\Lambda $ is at most  $N\vdone$.\\

We now prove
\begin{equation}
\blue{\text{ There exists infinite terms greater than $\Lambda $ }\implies \Lambda \leq \limsup_{n\to\infty} x_n}
\end{equation}
...
\begin{theorem}
\label{4.4.1}
\textbf{(Sequence Arithmetic)} Let $\set{x_n},\set{y_n}$ be two real sequences that respectively converge to $x$ and  $y$. We have
\begin{gather}
...
\end{gather}
\begin{proof}
Fix $\epsilon $. We know there exists $N_{(x,\frac{\epsilon}{2})}$ and $N_{(y,\frac{\epsilon}{2})}$ such that
\begin{equation}
n>N_{(x,\frac{\epsilon}{2})}\implies \abso{x_n-x}<\frac{\epsilon}{2}\text{ and }n>N_{(y,\frac{\epsilon}{2})}\implies \abso{y_n-y}<\frac{\epsilon}{2}
\end{equation}
Then we have
\begin{equation}
n>\max \set{N_{(x,\frac{\epsilon}{2})},N_{(y,\frac{\epsilon}{2})}}\implies \abso{x_n+y_n-(x+y)}\leq \abso{x_n-x}+\abso{y_n-y}<\epsilon 
\end{equation}
We know there exists $N_{(x,\frac{\epsilon}{\abso{c}})}$ such that
\begin{equation}
n>N_{(x,\frac{\epsilon}{\abso{c}})}\implies \abso{x_n-x}<\frac{\epsilon}{\abso{c}}\implies \abso{cx_n-cx}=\abso{c}\abso{x_n-x}<\epsilon 
\end{equation}
We know there exists $N_\epsilon $ such that
\begin{equation}
n>N_{(x,\epsilon )}\implies \abso{c+x_n-c+x}=\abso{x_n-x}<\epsilon 
\end{equation}
Notice that
\begin{align}
  \abso{x_ny_n-xy}&=\abso{(x_n-x)(y_n-y)+x(y_n-y)+y(x_n-x)}\\
  &\leq \abso{x_n-x}\abso{y_n-y}+\abso{x}\abso{y_n-y}+\abso{y}\abso{x_n-x}
\end{align}
We know there exists  $N_{(x,\sqrt{\frac{\epsilon }{3}} )}$ and $N_{(y,\sqrt{\frac{\epsilon}{3}} )}$ and $N_{(x,\frac{\epsilon}{\abso{y}})}$and $N_{(y,\frac{\epsilon}{\abso{x}})}$ such that if 
\begin{equation}
n>\max \set{N_{(x,\sqrt{\frac{\epsilon}{3}} )},N_{(y,\sqrt{\frac{\epsilon}{3}} )},N_{(x,\frac{\epsilon }{3\abso{y}})},N_{(y,\frac{\epsilon}{3\abso{x}})}}
\end{equation}
Then we have
\begin{align}
  \abso{x_ny_n-xy}&\leq \abso{x_n-x}\abso{y_n-y}+\abso{x}\abso{y_n-y}+\abso{y}\abso{x_n-x}\\
  &\leq \sqrt{\frac{\epsilon}{3}}\sqrt{\frac{\epsilon}{3}}+\abso{x}\frac{\epsilon }{3\abso{x}}+\abso{y}\frac{\epsilon}{3\abso{y}}=\epsilon 
\end{align}
Notice that
\begin{equation}
\abso{\frac{1}{x_n}-\frac{1}{x}}=\abso{\frac{x-x_n}{xx_n}}=\frac{1}{\abso{x}}\frac{1}{\abso{x_n}}\abso{x-x_n}
\end{equation}
We know there exists $N_{(x,\frac{\abso{x}}{2})}$ such that  
\begin{equation}
n>N_{(x,\frac{\abso{x}}{2})}\implies \abso{x}-\abso{x_n}\leq \abso{x-x_n}<\frac{\abso{x}}{2}\implies \frac{\abso{x}}{2}<\abso{x_n}\implies \frac{1}{\abso{x}}\frac{1}{\abso{x_n}}<\frac{2}{\abso{x^2}}
\end{equation}
We also know there exists $N_{(x,\frac{\abso{x^2}\epsilon }{2})}$ such that
\begin{equation}
n>\max \set{N_{(x,\frac{\abso{x}}{2})},N_{(x,\frac{\abso{x^2}\epsilon }{2})}}\implies \abso{\frac{1}{x_n}-\frac{1}{x}}=\frac{1}{\abso{x}}\frac{1}{\abso{x_n}}\abso{x-x_n}<\frac{2}{\abso{x^2}}\frac{\abso{x^2}\epsilon }{2}=\epsilon 
\end{equation}
\end{proof}
\end{theorem}
\begin{theorem}
\label{4.4.2}
\textbf{(Special Sequence)} Let $p>0$. We have
\begin{gather}
...
Fix $\epsilon $. We have
\begin{equation}
\lim_{n\to\infty}\frac{1}{n^p}=\lim_{n\to\infty}(\frac{1}{n})^p=(\lim_{n\to\infty} \frac{1}{n})^p
\end{equation}
We only have to prove
\begin{equation}
\lim_{n\to\infty}\frac{1}{n}=0
\end{equation}
Let $N=\frac{1}{\epsilon }$ and we have
\begin{equation}
n>\frac{1}{\epsilon }\implies \abso{\frac{1}{n}-0}=\frac{1}{n}<\epsilon 
\end{equation}
\end{proof}
\begin{theorem}
\label{4.4.3}
...
Notice that  
\begin{equation}
\lim_{n\to\infty}u_n=0\implies \lim_{n\to\infty}\sqrt[n]{p} -1=0\implies \lim_{n\to\infty}\sqrt[n]{p}=1 
\end{equation}
...
Also, notice 
\begin{equation}
  1+\binom{n}{1}u_n+\binom{n}{2}u_n^2+\cdots +\binom{n}{n}u_n^n =(u_n+1)^n=(\sqrt[n]{p}-1+1)^n=p
\end{equation}
We first prove
\begin{equation}
  \vi{p>1\implies \lim_{n\to\infty}u_n=0}
\end{equation}
We have
\begin{equation}
p>1\implies \sqrt[n]{p}>1\implies u_n>0 \implies \binom{n}{2}u_n^2+\cdots +\binom{n}{n}u_n^n>0
\end{equation}
Then because
\begin{equation}
1+nu_n+\binom{n}{2}u_n^2+\cdots +\binom{n}{n}u_n^n=p
\end{equation}
We have
\begin{equation}
1+nu_n<p
...
Then by Squeeze Theorem, we have 
 \begin{equation}
0\leq \lim_{n\to\infty}u_n\leq \lim_{n\to\infty}\frac{p-1}{n}=0\vdone
\end{equation}
Notice that if $0<p<1$, then by what we have just proved, we know
\begin{equation}
...
Notice
\begin{equation}
n=(e_n+1)^n=1+\binom{n}{1}e_n+\binom{n}{2}e_n^2+\cdots +\binom{n}{n}e_n^n
\end{equation}
So we have
\begin{equation}
\frac{n(n-1)}{2}e_n^2=\binom{n}{2}e_n^2< 1+\binom{n}{1}e_n+\binom{n}{2}e_n^2+\cdots +\binom{n}{n}e_n^n=n
\end{equation}
Then we have
\begin{equation}
e_n^2< \frac{2}{n-1}
...
So we have
\begin{equation}
0<e_n<\sqrt{\frac{2}{n-1}} 
\end{equation}
By Squeeze Theorem, we have
\begin{equation}
0\leq \lim_{n\to\infty}e_n\leq \lim_{n\to\infty}\sqrt{\frac{2}{n-1}}=\sqrt{2}\lim_{n\to\infty}\frac{1}{(n-1)^{\frac{1}{2}}}=0
\end{equation}
\end{proof}
\begin{theorem}
\label{4.4.5}
\textbf{(Special Sequence)}
Let $\alpha \inr$ and $q>1$
\begin{equation}
...
So we have 
\begin{equation}
q^n=(1+p)^n=1+np+\binom{n}{2}p^2+\cdots + \binom{n}{n}p^n
\end{equation}
Let $k\inn$ satisfy
\begin{equation}
k>\alpha^+
\end{equation}
Then for $n$ greater than $k$, because $q>1\implies p>0$, we have
\begin{equation}
q^n=1+np+\binom{n}{2}p^2+\cdots +\binom{n}{n}p^n>\binom{n}{k}p^k
\end{equation}
If we require $n$ to be greater than  $2k$, we see
\begin{equation}
n>2k\implies n>2k-2\implies \frac{n}{2}>k-1\implies n-k+1>\frac{n}{2}
\end{equation}
Then we have
\begin{equation}
\binom{n}{k}p^k=\frac{n(n-1)\cdots(n-k+1)}{k!}p^k=n(n-1)\cdots (n-k+1) \frac{p^k}{k!}>(\frac{n}{2})^k \frac{p^k}{k!}
\end{equation}
Notice that we have 
\begin{equation}
q^n>\binom{n}{k}p^k>(\frac{n}{2})^k \frac{p^k}{k!}
\end{equation}
Then we have
\begin{equation}
\frac{n^\alpha }{q^n}<\frac{n^\alpha }{n^k}\frac{2^k}{p^k}k!=n^{\alpha -k}(\frac{2}{p})^{k}k!
\end{equation}
Because $k>\alpha^+$, we know $u=k-\alpha >0$. Then we have
\begin{equation}
0\leq \lim_{n\to\infty}\frac{n^\alpha }{q^n}\leq (\frac{2}{p})^kk!\lim_{n\to\infty}n^{-u}=0
\end{equation}
\end{proof}
\begin{theorem}
\label{4.4.6}
\textbf{(Special Sequence)} Let $\abso{x}<1$. We have
\begin{equation}
\lim_{n\to\infty}x^n=0
\end{equation}
\end{theorem}
\begin{proof}
\myref{Theorem}{4.4.5} tell us $\forall \alpha \inr,\forall q>1$ we have
\begin{equation}
\lim_{n\to\infty}\frac{n^{\alpha }}{q^n}=0
\end{equation}
If we let $\alpha =0$, we have
\begin{equation}
\lim_{n\to\infty}\frac{1}{q^n}=0
\end{equation}
Notice
\begin{equation}
\abso{x}<1\implies \abso{\frac{1}{x}}>1
\end{equation}
If $x$ is positive, we immediately have
\begin{equation}
  \lim_{n\to\infty} x^n= \lim_{n\to\infty} \frac{1}{(\frac{1}{x})^n}=0
\end{equation}
\end{proof}
\section{Comparison Test,  Root Test and Ratio Test}
\begin{theorem}
\label{4.5.1}
\textbf{(Geometric Series)} Let $x$ be non-negative
\begin{equation}
\sum_{n=0}^\infty x^n\text{ converge }\iff x<1
\end{equation}
\end{theorem}
\begin{proof}
Let 
...
Notice
\begin{equation}
  (1-x)s_n=\sum_{k=1}^n x^k-\sum_{k=1}^n x^{k+1}=x-x^{n+1}
\end{equation}
This give us
\begin{equation}
s_n=\frac{x-x^{n+1}}{1-x}=\frac{x}{1-x}(1-x^{n+1})
\end{equation}
Then we have
\begin{equation}
x<1\implies \lim_{n\to\infty}x^{n+1}=0\implies \lim_{n\to\infty}s_n=\frac{x}{1-x}
\end{equation}
And have
\begin{equation}
x>1\implies \lim_{n\to\infty}x^{n+1}=\infty\implies \lim_{n\to\infty}s_n= \frac{x}{1-x}(-\infty)
\end{equation}
\end{proof}
\begin{definition}
\label{4.5.2}
...
We say $\set{x_n}$ monotonically increase if 
\begin{equation}
\forall n, x_{n+1}\geq x_n
\end{equation}
We say $\set{x_n}$ monotonically decrease if 
\begin{equation}
\forall n, x_{n+1}\leq x_n
\end{equation}
\end{definition}
\begin{theorem}
\label{4.5.3}
...
We now prove
\begin{equation}
\blue{\set{x_n}\text{ converge }\implies \set{x_n}\text{ is bounded }}
\end{equation}
Let $\epsilon \inr$. We know there exists $N$ such that
 \begin{equation}
\forall n>N, x_n<x+\epsilon 
\end{equation}
Then we see $\set{x_n}$ is bounded above by $\max \set{x_1,\dots, x_{N},x+\epsilon }$. $\bdone$\\

Let $\set{x_n}$ monotonically decrease. We know $\set{-x_n}$ increase. If $\set{x_n}$ is bounded, then $\set{-x_n}$ is bounded, which implies $\set{-x_n}$ converges and $\set{x_n}$ converges.\\
...
\begin{gather}
\sum c_n\text{ converge and }\exists N, \forall n>N, \abso{a_n}\leq c_n\implies \sum a_n\text{ converge }
\end{gather}
\end{theorem}
\begin{proof}
By Cauchy criterion, we know 
\begin{equation}
\forall \epsilon ,\exists N_0, \forall m>u>N_0, \sum_{k=u}^m c_k<\epsilon 
\end{equation}
Fix $\epsilon $, let $N_1=\max \set{N_0,N}$. We have
\begin{equation}
\forall m>u>N_1, \sum_{k=u}^m a_k<\sum_{k=u}^m \abso{a_k}\leq \sum_{k=u}^m c_k<\epsilon 
\end{equation}
And we are done, because we have just prove the series $\sum a_n$ satisfy Cauchy Criterion.
\end{proof}
\begin{corollary}
\label{4.5.5}
\textbf{(Comparison Test)} 
\begin{equation}
\text{ $\sum d_n$ diverge and }\exists N, \forall n>N, a_n\geq d_n\geq 0\implies \sum a_n\text{ diverge }
\end{equation}
\end{corollary}
\begin{proof}
If $\sum a_n$ converge, then $\sum d_n$ converge by \myref{Theorem}{4.5.4}
\end{proof}
\begin{theorem}
\label{4.5.6}
...
Then 
\begin{enumerate}[label=(\alph*)]
  \item $\alpha <1\implies \sum a_n$ converge\\
...
Let 
\begin{equation}
\limsup_{n\to\infty} \sqrt[n]{\abso{a_n}} =\alpha <\beta <1
\end{equation}
We know there exists $N$ such that
 \begin{equation}
\forall n>N, \sqrt[n]{\abso{a_n}}<\beta <1 
\end{equation}
Then we have
\begin{equation}
\forall n>N, a_n\leq \abso{a_n}<\beta ^n
\end{equation}
Because $\beta<1$, we know 
\begin{equation}
\sum_{n=1}^\infty \beta^n=\frac{\beta }{1-\beta }
\end{equation}
...
\begin{equation}
  \lim_{n\to\infty}\sqrt[f(n)]{\abso{a_{f(n)}}}=\alpha>1 
\end{equation}
Then we see there exists $N_{\alpha -1}$ such that
\begin{equation}
  \forall n>N_{\alpha -1}, \sqrt[f(n)]{ \abso{a_{f(n)}}}>1\text{ and }\abso{a_{f(n)}}>1
\end{equation}
Notice that $\sum_{n=1}^\infty a_n$ converge implies $\lim_{n\to\infty}a_n=0\tCaC$
\end{proof}
\begin{theorem}
\label{4.5.7}
...
Let 
\begin{equation}
\limsup_{n\to\infty} \abso{\frac{a_{n+1}}{a_n}}<1
\end{equation}
Then we can pick $\beta $ such that
\begin{equation}
\limsup_{n\to\infty} \abso{\frac{a_{n+1}}{a_n}}<\beta <1
\end{equation}
We know there exists $N$ that satisfy
 \begin{equation}
\forall n\geq N, \abso{\frac{a_{n+1}}{a_n}}<\beta <1
\end{equation}
This give us
\begin{equation}
\forall n>N, \abso{a_n}<\beta^{n-N}\abso{a_N}
\end{equation}
In other words,
\begin{equation}
\forall k, \abso{a_{N+k}}<\beta^{k}\abso{a_N}
\end{equation}
Then because
\begin{equation}
\sum_{k=1}^\infty \beta^{k} \abso{a_N}=\frac{\abso{a_N}\beta }{1-\beta }
\end{equation}
...
\end{equation}}
\As{$\sum a_n$ converge}. We have
\begin{equation}
\lim_{n\to\infty}a_n=0
\end{equation}
By 
\begin{equation}
\liminf_{n\to\infty} \abso{\frac{a_{n+1}}{a_n}}>1
\end{equation}
We know there exists $N$ such that
 \begin{equation}
   \forall n\geq N, \abso{\frac{a_{n+1}}{a_n}}>1
\end{equation}
This tell us
\begin{equation}
\forall n\geq N, \abso{a_n}>\abso{a_N}
\end{equation}
Let $\epsilon=\abso{a_N}$, clearly 
\begin{equation}
\forall N_1, \exists n> \max \set{N_1,N}, \abso{a_n}>\abso{a_N}=\epsilon \tCaC
\end{equation}
\end{proof}
\begin{theorem}
\label{4.5.8}
...
\end{question}
\begin{proof}
Because $\Q$ is countable and dense in  $\R$, so in \myref{Theorem}{3.10.1}, we have proved the set
\begin{equation}
...
\end{equation}
So we know no set in $M$ has a limit point. Let $K\subseteq M$ be compact. By \myref{Theorem}{3.7.4}, we know $K$ is limit point compact. \As{$K$ is infinite}. Then $K$ has a limit point  $\tCaC$.
\end{proof}
\begin{question}{}{}
...
Then by \myref{Theorem}{3.7.4}, we know there exists a limit point $p\in B$ for $\set{b_n}_n\inn$.\\
...
Then because we are in $\R^n$ and $\set{a_n}_{n\inn}$ is closed and bounded, we know $\set{a_n}_{n\inn}$ is compact, then by \myref{Theorem}{3.7.4}, we know $\set{a_n}_{n\inn}$ has a limit point in itself.\\
...
\end{question}
\begin{proof}
See the proof in \myref{Theorem}{3.7.4}, there is a summary where you can find the numbering of rigorous proof for each step. Some proof for corollary is omitted because they are immediate consequences of theorem before.\\
...
We now prove $A_2$ is connected.\\

For (b), see next Theorem.
