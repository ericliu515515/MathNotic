...
\begin{theorem}
\label{2.3.2}
\textbf{(Norm Induced Metric)} Every norm induce a metric by 
\begin{equation}
d(\vecta{x},\vecta{y})=\norm{\vecta{x}-\vecta{y}}
\end{equation}
\end{theorem}
\fbox{\begin{minipage}{39em}
In fact, there are other way to induce a metric from norm: $d(\vecta{x},\vecta{y})=\norm{\vecta{x}}+\norm{\vecta{y}}$. The two norms don't even have to be the same. Although there are lots of type of metric, the one being study the most is of course what we induce in \myref{Theorem}{2.3.2}.\\
...
Notice \myref{Theorem}{2.3.8} will prove that our definition of open do satisfy the axioms of open sets. In other words, every metric induce a topology. 
\end{minipage}}
\begin{theorem}
\label{2.3.7}
...
\end{proof}
\fbox{\begin{minipage}{39em}
    Notice \myref{Theorem}{2.4.3} is written in language of general topology. Now, we prove that our definition of closed and limit point in metric space is compatible to those in general topology. Then we can apply the result we have in general topology into metric space.
\end{minipage}}
\begin{theorem}
\label{2.4.4}
...
\end{corollary}
\fbox{\begin{minipage}{39em}
Notice \myref{Corollary}{2.4.6} is proved using language of general topology in the first section of this chapter. I personally haven't tried to prove \myref{Corollary}{2.4.6} using limit point definition, but I think this won't be difficult.\\
...
\end{theorem}
\begin{proof}
For $E^\circ \subseteq F^\circ $, notice an open ball contained by $E$ is also contained by $F$. For  $\overline{E}\subseteq \overline{F}$, notice $E'\subseteq F'$, by \myref{Theorem}{2.4.2}
\end{proof}
\begin{theorem}
\label{2.5.4}
...
\end{corollary}
\fbox{\begin{minipage}{39em}
Also, notice \myref{Theorem}{2.5.3} have been proved in language of general topology, and here we give a simple proof in language of metric.\\
...
\end{theorem}
\begin{proof}
  Let $\mathcal{G}$ be an open cover for $K$. By \myref{Theorem}{2.7.3}, we know $K$ is separable. Then by  \myref{Theorem}{2.7.1}, we know $K$ has a countable base. Then by \myref{Theorem}{2.6.13}, we know $\mathcal{G}$ contain a countable sub-cover
  \begin{equation}
  \set{G_i}_{i\inn}
  \end{equation}
...
\end{theorem}
\begin{proof}
In \myref{Theorem}{2.6.5}, we prove that if $K$ has a count base then $K$ is separable by collecting one point from each basic open set, and in \myref{Theorem}{2.7.1}, we construct a countable base by collecting all open balls of rational radius around each point in the countable dense subset.\\

In \myref{Theorem}{2.7.3}, we prove if $K$ is limit point compact then  $K$ is separable. We proved such by covering $K$ with open ball of radius $\frac{1}{n}$ for each $n\inn$, and argue the set of centers $A$ is countable by proving to cover $K$ always require only finite amount of point (which is easy to see if one know $K$ is bounded, but our method here is to show if require infinite centers, then the set of infinite centers has no limit point, which is also easy to see if one use geometric intuition), and argue that each point $p$ that isn't in $A$ is a limit point of $A$ by arguing for each distance $r$, there exists an  $a\in A$ closed enough to be closer to $p$ than $r$.\\

One may observe, at the end of the proof for \myref{Theorem}{2.7.3}, we also construct a countable base for $K$.\\

In  \myref{Theorem}{2.6.13}, we prove if  $K$ has a countable base then every open cover for  $K$ has a countable sub-cover by filtering the open cover with countable base.\\
...
\end{equation}
Denote this sequence $\set{I_n}_{n=1}^\infty$. By \myref{Lemma}{2.9.4}, we know 
\begin{equation}
...
Now, using \myref{Lemma}{2.9.3}, it is easy to verify when  $m>\log_2 \frac{\delta}{r}$, the k-cell $I_m$ is small enough to be contained by $B_r(\vecta{x})\tCaC$.
\end{proof}
\begin{corollary}
\label{2.9.5}
...
\end{corollary}
\begin{proof}
  We have proved from right to left in \myref{Theorem}{2.8.3} and  \myref{Theorem}{2.8.5}. To prove from left to right, just observe $K$ is bounded implies that $K$ is contained by some k-cell. Then because closed subset of compact set is compact, we can deduce $K$ is compact.
\end{proof}
\chapter{Basic Topology, Part 2} 
\chapter{Numerical Sequence and Series}
\section{Convergence}
\begin{definition}
\label{3.1.1}
\textbf{(Definition of Sequence of Point)} Let $X$ be a topological space. In our definition, sequence of points is a function from $\N$ to $X$. We denote a sequence by
\begin{equation}
\set{a_n}_{n\inn}\text{ or }\set{a_n}_{n=1}^\infty\text{ or }\set{a_n}
\end{equation}
For the third notation, only $\set{a_n}\text{ or }\set{a_i}$ stands for a sequence, others, like $\set{a_m}$, stand for singleton.
\end{definition}
\begin{definition}
\label{3.1.2}
\textbf{(Definition of Convergence in General Topology)} Let $\set{a_n}$ be a sequence of points in $X$. If we say $\set{a_n}$ converge to $a$ 
\begin{equation}
\lim_{n\to\infty} a_n=a
\end{equation}
We mean for each open neighborhood $U$ around  $a$, there exists great enough $N_U\inn$  such that for each $n\inn$ 
\begin{equation}
n\geq N_U\iff a_n\in U
\end{equation}
\end{definition}
\begin{theorem}
\textbf{(Equivalent Statements for Convergence)} Let $U$ be an arbitrary open neighborhood around  $a$. The following are equivalent 
\begin{gather}
\lim_{n\to\infty}a_n=a\\
\exists N_U\inn, \forall n\inn, n\geq N_U\implies a_n\in U\\
\exists N_U\inn, \set{a_n:n\geq N_U}\subseteq U\text{ and }\set{a_n:n<N_U}\subseteq U^c\\
\end{gather}
\end{theorem}
\begin{proof}
From the first one to the second are trivial. From the second one to the first, just pick the smallest $N_U$. The first one and the third one are equivalent, the only "difference" is that the third one is written with set. 
\end{proof}
\fbox{\begin{minipage}{39em}
...
\lim_{n\to\infty}a_n=a
\end{equation}
If for each $\epsilon\inr^+$, there exists an great enough natural $N_\epsilon $ such that for each $n\inn$
\begin{equation}
 n\geq N_\epsilon \iff d(a_n,a)<\epsilon 
\end{equation}
\end{definition}
\begin{theorem}
\textbf{(Equivalent Statements for Convergence)} Let $\epsilon $ be an arbitrary real. The following are equivalent
\begin{gather}
\lim_{n\to\infty}a_n=a\\
\exists N_\epsilon\inn, \forall n\inn, n\geq N_\epsilon \implies  d(a_n,a)<\epsilon \\
\exists N_\epsilon\inn, \forall n\inn, n\geq N_\epsilon \implies a_n\in B_\epsilon (a)\\
\exists N_\epsilon \inn, \set{a_n:n\geq N_\epsilon }\subseteq B_\epsilon (a)\text{ and }\set{a_n:n>N_\epsilon }\subseteq B_\epsilon (a)^c
\end{gather}
\end{theorem}
\begin{proof}
From the second to first, pick the smallest $N_\epsilon $
\end{proof}
\begin{theorem}
\label{3.1.4}
\textbf{(Compatibility of Definitions of Convergence)} Let $X$ be a metric space. A sequence  $\set{a_n}$ converge to $a$ if and only if for each open neighborhood  $U$ around $a$, there exists a great enough natural $N_U$ such that for each $n\inn$
\begin{equation}
n\geq N_U\iff a_n\in U 
\end{equation}
\end{theorem}
\begin{proof}
From right to left, just observe $B_\epsilon (a)$ is open. From left to right, let $B_\epsilon(a) $ be an open ball in $U$, we see 
\begin{equation}
N_\epsilon \in M:=\set{N\inn: \forall n\inn, n\geq N\implies a_n\in U}
\end{equation}
Then $N_U=\min M$.
\end{proof}
\fbox{\begin{minipage}{39em}
...
\end{theorem}
\begin{proof}
\As{$a'\neq a$}. Let $r=\frac{d(a,a')}{2}$. We see there exists $N_r$ and $N'_r$ such that 
\begin{equation}
n\geq N_r\iff  a_n\in B_r(a)\text{ and }n\geq N'_r\iff  a_n\in B_r(a')
\end{equation}
Observe that $B_r(a)$ and $B_r(a')$ are disjoint and all $a_n$ such that  $n\geq \max \set{N_r,N'_r}$ is in both open balls.\CaC
\end{proof}
...
\end{question}
\begin{proof}
Because $\Q$ is countable and dense in  $\R$, so in \myref{Theorem}{2.9.4}, we have proved the set
\begin{equation}
...
\end{equation}
So we know no set in $M$ has a limit point. Let $K\subseteq M$ be compact. By \myref{Theorem}{2.7.5}, we know $K$ is limit point compact. \As{$K$ is infinite}. Then $K$ has a limit point  $\tCaC$.
\end{proof}
\begin{question}{}{}
...
We first prove \vi{the triangle inequality}
\begin{equation}
  \vi{d(X,Z)\leq d(X,Y)+d(Y,Z)}
\end{equation}

\As{$d(X,Z)>d(X,Y)+d(Y,Z)$}. We have
\begin{equation}
d(X,Z)-d(X,Y)>d(Y,Z)=\inf \set{d(y,z):y\in Y,z\in Z}
\end{equation}
Then we know there exists $y'\in Y, z'\in Z$ such that $d(y',z')<d(X,Z)-d(X,Y)$. Then we have
\begin{equation}
\inf \set{d(x,y):x\in X,y\in Y}=d(X,Y)<d(X,Z)-d(y',z')
\end{equation}
Then we know there exists $x'\in X, y'' \in Y$ such that 
\begin{equation}
d(x',y'')<d(X,Z)-d(y',z')
\end{equation}
in other words
...
Notice that 
\begin{equation}
d(x',z')\leq \min \set{d(x',y'')+d(y'',z'),d(x',y')+d(y',z')}\leq d(x',y'')+d(y',z')\tCaC\vdone
\end{equation}
...
Notice   
\begin{equation}
d(A,B)=\inf \bigcup \set{\set{d(a,b):a\in A}:b \in B}
\end{equation}
and notice
\begin{equation}
d(A,B)=\inf \set{\inf \set{d(a,b):a\inA}:b\in B}=\inf \set{d(A,\set{b}):b\in B}
\end{equation}

We first prove if $d(A,B)=0$, then $A$ and  $B$ intersect.\\
...
If $B$ is infinite, then  $0=\inf \set{d(A,\set{b}):b\in B}$. If there exists $b$ such that $d(A,\set{b})=0$, we can use the same argument above to finish the proof. We only have to consider when $\forall b\in B, d(A,\set{b})>0$. For each $n$, we then know there exists  $b_n$ such that  $d(A,\set{b_n})<\frac{1}{n}$.\\


Construct an infinite sequence $\set{b_n}_{n\inn}$ by picking $b_n$ such that $d(A,\set{b_n})<\frac{1}{n}$. We can see our method for picking $b_n$ will give us an infinite sequence as every $b$  satisfy  $d(A,\set{b})>0$. By \myref{Theorem}{2.7.5}, we know $B$ is limit point compact, so we know $\set{b_n}_{n\inn}$ has a limit point in $B$.\\
...
\end{equation}
Then we see if $n>\frac{2}{r}$, then $b_n$ is not contained by the open ball  $B_{\frac{r}{2}}(p)$. In other words, the open ball $B_{\frac{r}{2}}(p)$ that center a limit point $p$ contain at most $\lfloor \frac{2}{r}\rfloor$ amount of point in $\set{b_n}_{n\inn}$ \CaC.\\
...
Then by \myref{Theorem}{2.7.5}, we know there exists a limit point $p\in B$ for $\set{b_n}_n\inn$.\\

We know $d(A,\set{p})>u$ by our assumption.\\

Let $m=d(A,\set{p})>u$. Then from
\begin{equation}
\forall b\in B,m=d(A,\set{p})<d(A,\set{b})+d(b,p)
\end{equation}
we have
\begin{equation}
...
Then by \myref{Corollary}{2.9.5}, we know $\set{a_n}_{n\inn}$ is compact, then by \myref{Theorem}{2.7.5}, we know $\set{a_n}_{n\inn}$ has a limit point in itself.\\
...
\end{question}
\begin{proof}
See the proof in \myref{Theorem}{2.7.5}, there is a summary where you can find the numbering of rigorous proof for each step. Some proof for corollary is omitted because they are immediate consequences of theorem before.\\
