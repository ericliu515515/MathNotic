...
\end{proof}
\fbox{\begin{minipage}{39em}
We now give the formal definition of neighborhood while a topology is given, and translate \myref{Theorem}{0.1.5} into language without neighborhood function. 
\end{minipage}}
\begin{definition}
\label{2.1.6}
...
\end{corollary}
\fbox{\begin{minipage}{39em}
Lastly, we give a theorem involving separable and countably compact.
\end{minipage}}
\begin{theorem}
\label{2.6.13}
...
\As{$\set{\vecta{x}_i:i\inn}$ has some limit points in $\R^k$}. Let $p$ be a limit point of $\set{\vecta{x}_i:i \inn}$. Arbitrarily pick $r\inr^+$. By \myref{Theorem}{2.9.5}, we know $B_r(p)$ should contain infinite points of $\set{\vecta{x}_i}$.\\
...
By \myref{Lemma}{2.9.5}, we can pick $\vecta{x}\in \bigcap \set{I_i}$. Because $\set{G_\alpha }$ is an open cover of $I$, we know there exists $G_\beta \in \set{G_\alpha }$ such that $\vecta{x}\in G_\beta $. Because $G_\beta $ is open, we know there exists $r\inr^+$ such that
\begin{equation}
...
\end{question}
\begin{proof}
Because $\Q$ is countable and dense in  $\R$, so in \myref{Theorem}{2.9.5}, we have proved the set
\begin{equation}
...
\end{equation}
So we know no set in $M$ has a limit point. Let $K\subseteq M$ be compact. \As{$K$ is infinite}. Then by \myref{Theorem}{2.9.5}, we know $K$ has a limit point $\tCaC$  
\end{proof}
\begin{question}{}{}
...
\end{question}
\begin{proof}
If a set $K$ is compact, then by \myref{Theorem}{2.9.5}, we know $K$ is limit point compact. In the proof for \myref{Theorem}{2.9.5}, we have proved the limit point compact $K$ is separable.
\end{proof}
\end{document}
