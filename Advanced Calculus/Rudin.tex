...
\end{proof}
\section{General Topology: Closure and Interior}
\fbox{\begin{minipage}{39em}
This section won't go into any detailed general topology. It merely work as a comparison of the next section. 
\end{minipage}}
\begin{definition}
\label{2.2.1}
...
\end{proof}
\fbox{\begin{minipage}{39em}
Now, we present some example of open sets in different metric spaces. Give clear notice that some open set 
\end{minipage}}
\begin{theorem}
\label{2.3.9}
...
\end{proof}
\begin{theorem}
\textbf{(General Sub-Metric Space)}
An open set is still open in sub-metric space, and if a set is not open in sub-metric space, it is not open in the metric space.
\end{theorem}
\begin{proof}
For open implies open in sub-metric, just pick the same open ball. If $E$ is not open in a sub-metric space, then $E$ contain a point $p$ such that every open ball centering $p$ is not contained in $E$. In the original metric space, ever open ball centering $p$ is then not contained in $E$ 
\end{proof}
\begin{theorem}
...
Notice by \myref{Theorem}{0.3.5}, we can prove every point in $\R$ is an limit point of $[0,1]\cap \Q$ by selecting a rational number of smaller error than any desired real number.\\
...
\end{corollary}
\begin{proof}
For closed, there is a proof other than just using the above corollary. One can check out \myref{Theorem}{2.3.9}. For open, just verify for $B_{\min \set{d(x,y):y \in O}}(x)$. 
\end{proof}
\begin{theorem}
\label{2.4.14}
...
By \myref{Lemma}{2.5.8}, we can pick $\vecta{x}\in \bigcap \set{I_i}$. Because $\set{G_\alpha }$ is an open cover of $I$, we know there exists $G_\beta \in \set{G_\alpha }$ such that $\vecta{x}\in G_\beta $. Because $G_\beta $ is open, we know there exists $r\inr^+$ such that
\begin{equation}
...
Now, by \myref{Theorem}{2.5.13}, we know $K$ has a countable base (In Theorem 2.8.13, we prove that if a metric space is separable, then it has a countable base by constructing the base with open ball. To see $K$ does have a countable base in the scope of $X$, just realize the base constructed with open balls applies to our situation).\\
...
\end{equation}
Because $p$ is an limit point of $\set{x_j}$, by \myref{Theorem}{2.4.11}, we know $B_r(p)$ should contain infinite points of $\set{x_j}$.\\
...
\end{equation}
\CaC to \myref{Theorem}{2.4.11} $\odone \bdone$
\end{proof}
\fbox{\begin{minipage}{39em}
...
\As{$\set{\vecta{x}_i:i\inn}$ has some limit points in $\R^k$}. Let $p$ be a limit point of $\set{\vecta{x}_i:i \inn}$. Arbitrarily pick $r\inr^+$. By \myref{Theorem}{2.4.11}, we know $B_r(p)$ should contain infinite points of $\set{\vecta{x}_i}$.\\
...
\end{question}
\begin{proof}
Because $\Q$ is countable and dense in  $\R$, so in \myref{Theorem}{2.5.13}, we have proved the set
\begin{equation}
...
\end{equation}
So we know no set in $M$ has a limit point. Let $K\subseteq M$ be compact. \As{$K$ is infinite}. Then by \myref{Theorem}{2.5.14}, we know $K$ has a limit point $\tCaC$  
\end{proof}
\begin{question}{}{}
...
\end{question}
\begin{proof}
If a set $K$ is compact, then by \myref{Theorem}{2.5.14}, we know $K$ is limit point compact. In the proof for \myref{Theorem}{2.5.14}, we have proved the limit point compact $K$ is separable.
\end{proof}
\end{document}
