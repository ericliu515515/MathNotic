...
\fbox{\begin{minipage}{39em}
The section is dedicated to show case how we can define a topology in different angles. That being said, in practice the one being used is to define via open set.
\end{minipage}}
\begin{definition}
\label{2.5.1}
...
\end{definition}
\fbox{\begin{minipage}{39em}
The above is the standard way to define topology. We now show cases some property of closed sets. Notice that the corresponding properties of open set are axioms.
\end{minipage}}
\begin{theorem}
\label{2.5.2}
...
Before we prove $g^{-1}$ is the inverse of $g$, we first prove  \blue{$g^{-1}$ do map neighborhood function to topology}.\\
...
Lastly, we prove \vi{$g^{-1}$ is the inverse of $g$}.\\
...
Prove \red{$\mathcal{N}(x)\subseteq g(g^{-1}(\mathcal{N}))(x)$}
\end{proof}
\fbox{\begin{minipage}{39em}
...
\end{proof}
\fbox{\begin{minipage}{39em}
We now give some notion to let us talk about convergence.
\end{minipage}}
\begin{definition}
\label{2.6.6}
...
\end{equation}
Notice that $p$ does not have to belong to $E$ so that  $p$ is a limit point of  $E$. A point $p$ is an isolated point of $E$ if $p$ belongs to $E$, but  $p$ is not a limit point.
\end{definition}
\fbox{\begin{minipage}{39em}
An example of limit point not in $E$
\end{minipage}}
\begin{theorem}
\label{2.6.8}
...
\begin{theorem}
\label{2.6.9}
\textbf{(Closure Contain Exactly All Limit Points of $E$ and Points of $E$)} Let $S(E)$ be the limit point of $E$. We have
\begin{equation}
\overline{E}= S(E)\cup  E
\end{equation}
\end{theorem}
\begin{proof}
Let $p$ be in the set on RHS and not in $E$. \As{$p\not\in \overline{E}$}. Then we have $p \in (\overline{E})^c \in \mathfrak{O}$, so we have $\exists u\neq p, u \in (\overline{E})^c \cap E=\varnothing \tCaC$. Let $p \in \overline{E}\setminus E$. \As{$\exists O\in\mathfrak{O},p \in O,O\cap E=\varnothing$}. Then we see $\overline{E}\subset O^c\in \mathfrak{F}$, since $p \in \overline{E}$ and $p \in O\implies p\not\in O^c\tCaC$
\end{proof}
\begin{corollary}
\label{2.6.10}
\begin{equation}
...
\end{lemma}
\begin{proof}
If $p \in E$, then we can see $p \in M\cap E$. If $p \in \overline{E}\setminus E$, then $p\in S(E)$, so $\forall O:p\in O,O\cap E\neq \varnothing$. Then $M\in \mathcal{N}(p)\implies \exists O, p \in O \subseteq M\implies \varnothing \subset O\cap E\subseteq M\cap E$. We have proved $\overline{E}\subseteq \set{p: \forall M\in \mathcal{N}(p), M\cap E\neq \varnothing}$. We now prove the other side. Let $p\in 
\set{p: \forall M\in \mathcal{N}(p), M\cap E\neq \varnothing}$. Either $p\in E$ or $p\not\in E$. If $p\in E$, then $p \in \overline{E}$. If $p\not\in E$, observe $p \in O\in \mathfrak{O}\implies p \in O \in \mathcal{N}(p)\implies O\cap E\neq \varnothing$. Because $p\not\in E$, we can deduce $p \in S(E)$ 

\end{proof}
\begin{theorem}
\label{2.6.12}
\textbf{(Property of Limit Points)} Let $S(E)$ be the set of limit points of $E$, we have  
\begin{equation}
S(E)=\set{p:p \in \overline{E\setminus \set{p}}}
\end{equation}
\end{theorem}
\begin{proof}
Let $p \in S(E)$. \As{$p\not\in \overline{E\setminus \set{p}}$}. We have $p \in (\overline{E\setminus \set{p}})^c\in \mathfrak{O}$, so we have $\exists u\neq p, u \in (\overline{E\setminus \set{p}})^c \cap E$. Observe $u\in E\implies u=p\text{ or }u \in E\setminus \set{p}\implies u\in E\setminus \set{p}\subseteq \overline{E\setminus \set{p}}\tCaC$. Let $p \in \overline{E\setminus \set{p}}$. We have $\forall M\in \mathcal{N}(p),M\cap E=M\cap (E\setminus \set{p})\neq \varnothing$. Then we have $\forall O: p \in O, O\cap E\neq \varnothing$, so we have $p\in S(E)$
\end{proof}
\fbox{\begin{minipage}{39em}
It is quite difficult to describe the last axiom of neighborhood function without using symbol: every neighborhood around a point contain a smaller neighborhood such that the big neighborhood is a neighborhood of every point in the smaller neighborhood.
\end{minipage}}
\begin{definition}
\label{2.6.13}
...
For each $N\in\mathcal{N}(x)$, we let $B_r(x)$ be an open ball contained by $N$. We now prove \vi{$B_{\frac{r}{2}}(x)$ is an neighborhood of $x$ such that for all $z \in B_{\frac{r}{2}}(x)$ we have $N\in \mathcal{N}(z)$}. Observe
\begin{equation}
...
WOLG, let $\sup A\geq \sup B$. \As{$\sup A>\sup (A\cup B)$}. Let $x$ be a number between  $\sup A\text{ and }\sup (A\cup B)$ ($x$ exists since it can be  $\frac{\sup A+\sup (A\cup B)}{2}$). Because $\sup A>x$, by definition, we know there exists $a' \in A$ such that $a'>x$. Notice that $x> \sup (A\cup B)$ implies $x$ is an upper bound of $A \cup B$, so we know $\forall a \in A\subseteq A\cup B, a\leq x\tCaC$
\end{proof}
\begin{proof}
...
\end{equation}
Let $\set{f(x_i)}_{i=1}^\infty$ be a sequence where $\forall i, x_i \in D$ and $j>i\implies \abso{x_j-x'}<\abso{x_i-x'}$ and $\lim_{i \to \infty}x_i=x'$. Such sequence exists because $D$ is dense in $\R$. We now show 
\begin{equation}
\lim_{i\to\infty}f(x_i)=f(x')
\end{equation}
Notice that $f$ is continuous meaning
\begin{equation}
