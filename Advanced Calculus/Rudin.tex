...
\end{question}
\begin{proof}
Because $\Q$ is countable and dense in  $\R$, so in \myref{Theorem}{2.8.13}, we have proved the set
\begin{equation}
...
\end{equation}
So we know no set in $M$ has a limit point. Let $K\subseteq M$ be compact. \As{$K$ is infinite}. Then by \myref{Theorem}{2.8.14}, we know $K$ has a limit point $\tCaC$  
\end{proof}
\begin{question}{}{}
...
\end{question}
\begin{proof}
If a set $K$ is compact, then by \myref{Theorem}{2.8.14}, we know $K$ is limit point compact. In the proof for \myref{Theorem}{2.8.14}, we have proved the limit point compact $K$ is separable. 
\end{proof}
\end{document}
