...
\section{HW3}
\begin{question}{}{}
Let $f$ be a simple function, taking its distinct values on disjoint sets $E_1,\dot , E_N$. Show that $f$ is measurable if and only if $E_1,\dots ,E_N$ are measurable. 
\end{question}
\begin{proof}
...
\end{theorem}
\begin{proof}
  We prove by induction on the sum $k$ of the amount of bricks consisting $A$ and the amount of bricks consisting $B$. The base case $k=2$ have been proved by \myref{Theorem}{Brunn-Minkowski Inequality for Bricks}. Suppose the proposition hold true when $k\leq r$. Let $k=r+1$. Because the bricks consisting of $A$ are non-overlapping, by a translation  (and renaming axis if necessary), we can suppose the following proposition.
\begin{center}
...
\end{align}
Define $B^-\triangleq B\cap \set{\textbf{x}\inr^d:\textbf{x}_1\leq 0}$. With reason similar to that of \customref{pp1}{Proposition 1}, we know $B^+$ and  $B^-$ are both union of collection of non-overlapping bricks, with each collection containing bricks no more than  $B$. Therefore, with \customref{pp1}{Proposition 1}, we can deduce that the sum of the amount of bricks consisting $A^+$ (resp. $A^-$) and the amount bricks consisting $B^+$ (resp. $B^-$) is at least one fewer than $r+1$. Then because the proposition hold true for $k\leq r$, we now have
\begin{align*}
...
\end{align*}
This together with \myref{Theorem}{BMIb} then give us the desired inequality  
\begin{align*}
\abso{A+B}^{\frac{1}{d}}&=\Bigg( \lim_{N\to \infty} \abso{\Big(\bigcup_{n=1}^N K_{n,a} \Big) + \Big( \bigcup_{n=1}^N K_{n,b}  \Big)}  \Bigg)^{\frac{1}{d}} \\
&=\lim_{N\to \infty} \abso{\Big(\bigcup_{n=1}^N K_{n,a} \Big) + \Big( \bigcup_{n=1}^N K_{n,b}  \Big)}^{\frac{1}{d}}\\
&\geq \lim_{N\to \infty} \abso{\bigcup_{n=1}^N K_{n,a}}^{\frac{1}{d}}+ \abso{\bigcup_{n=1}^N K_{n,b}}^{\frac{1}{d}}\\
&=\Big(\lim_{N\to \infty} \abso{\bigcup_{n=1}^N K_{n,a}} \Big)^{\frac{1}{d}}+ \Big(\lim_{N\to \infty} \abso{\bigcup_{n=1}^N K_{n,b}} \Big)^{\frac{1}{d}}\\
...
\end{align*}
Note that the inequality hold because $\textbf{z}\in A \implies \textbf{x}\neq \textbf{z} $. Similar argument shows that $B_\epsilon \searrow B$. It now follows from  
\end{proof}
...
\end{question}
\begin{proof}
The if part follows from the last question by taking some small enough $r$ such that  $B_r(x)\subseteq W$. We now prove the only if part. Fix arbitrary $x \in U$. Because $f$ is locally constant at $x$, we know there exists some  $B_r(x)$ such that $f$ is constant on $B_r(x)$. Therefore, we can let $c\inr$ satisfy 
\begin{align*}
...
\begin{mdframed}
  Consider the following \textbf{Dirichlet problem}
\begin{align*}
\begin{cases}
  v_t-kv_{xx}=0 \text{ for }0<x<\infty, 0<t<\infty\\
  \lim_{t\to 0}v(x,t)=\phi (x)\text{ for }0<x<\infty \\
  v(0,t)=0\text{ for }0<t<\infty 
\end{cases}
\end{align*}
where $\phi:\R^+\rightarrow \R$. Define $\phi_{\operatorname{odd}}:\R\rightarrow \R$ by 
\begin{align*}
...
\end{align*}
In other words, we have deduced that $u$ is an odd function in $x$. It then follows that  $u(0,t)=-u(-0,t)=0$. Then we see that the restriction $v\triangleq u|_{(\R^+)^2}$ form a solution of the Dirichlet problem. In particular, we can express $v$ in a form without usage of $\phi_{\operatorname{odd}}$ if we consider 
\begin{align*}
u(x,t)&=\int_{-\infty}^{\infty}S(x-y,t)\phi_{\operatorname{odd}}(y)dy\\
...
Now, consider the following \textbf{Neumann problem}
\begin{align*}
\begin{cases}
  w_t-kw_{xx}=0\text{ for }0<x<\infty,0<t<\infty\\
  \lim_{t\to 0}w(x,t)=\phi (x)\text{ for }0\leq x<\infty\\
  w_x(0,t)=0\text{ for }0<t<\infty
\end{cases}
\end{align*}
where $\phi:\R^+\rightarrow \R$. Define $\phi_{\operatorname{even}}:\R\rightarrow \R$ by   
\begin{align*}
\phi_{\operatorname{even}}(x)\triangleq \begin{cases}
  \phi (x)& \text{ if $x\geq 0$ }\\
  \phi (-x)& \text{ if $x\leq 0$ }
\end{cases}
\end{align*}
...
&u_t=ku_{xx}\text{ on the half line }0<x<\infty\\
&u(x,0)=\phi (x)\\
&u_x(0,t)-cu(0,t)=0\text{ where }c\text{ is some non-zero constant }
\end{align*}
...
then the function 
\begin{align*}
u(x,t)\triangleq \int_{\R} S(x-y,t)f(y)dy
\end{align*}
suffice both initial condition and boundary condition. To see that $u$ satisfy the boundary condition, observe that $u_x-hu$ is a solution to the diffusion equation with initial condition 
 \begin{align*}
...
\begin{align*}
&\text{ DE: }v_{tt}-c^2v_{xx}=0\text{ for }0<x<\infty,-\infty <t<\infty \\
&\text{ IC: }v(x,0)=\phi (x),v_t(x,0)=\psi (x)\text{ for }0<x<\infty \\
&\text{ BC: }v(0,t)=0\text{ for }-\infty < t< \infty
\end{align*}
...
Consider also the Neumann problem 
\begin{align*}
&\text{ DE: }v_{tt}-c^2v_{xx}=0\text{ for }0<x<\infty,-\infty <t<\infty \\
&\text{ IC: }v(x,0)=\phi (x),v_t(x,0)=\psi (x)\text{ for }0<x<\infty \\
&\text{ BC: }v_x(0,t)=0\text{ for }-\infty < t< \infty
\end{align*}
\end{mdframed}
\chapter{PDE HW}
\section{PDE HW 1}
