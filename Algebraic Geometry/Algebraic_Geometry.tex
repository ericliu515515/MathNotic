...
Let $Y$ be an abstract variety. We say a scheme morphism $f:X\rightarrow Y$ is a \textbf{resolution of singularities} if $X$ is a nonsingular abstract variety with  $f$ being proper birational. \\

Let $Y$ be a normal abstract variety and  $f:X\rightarrow Y$ a resolution of singularity. A prime divisor $D$ of $X$ is \textbf{exceptional}  

\section{Log canonical threshold}
Fix $n\geq 1$. We call $w \in (\Z_0^+)^n$ \textbf{weight vectors}. Given a monomial $c x_1^{\alpha _1}\cdots x_n ^{\alpha _n}\in \C[x_1,\dots ,x_n]$, we define its \textbf{degree respect to $w$} by: 
\begin{align*}
\operatorname{deg}_w (x_1^{\alpha_1 }\cdots x_n ^{\alpha _n})\triangleq \sum_{i=1}^n  w_i \alpha_i
\end{align*}
Let $f$ be a holomorphic $\C$-valued function from some neighborhood of  $\textbf{0}\inc^n$. On some neighborhood of $\textbf{0}$, we may write: 
\begin{align*}
f(\textbf{x})= \sum_{\alpha } c_\alpha \textbf{x}^{\alpha },\quad \text{ where $\alpha $ are multi-indices.}
\end{align*}
Its \textbf{degree respect to $w$} is then defined by 
\begin{align*}
\operatorname{deg}_w(f)\triangleq \min \set{\operatorname{deg}_w(x^{\alpha }):c_\alpha \neq 0}
\end{align*}
