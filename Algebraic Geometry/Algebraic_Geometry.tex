...
\end{proof}
One should always be careful with the word \textbf{unit} and the word \textbf{zero-divisor}. In nonzero rings, the set of units is always disjoint with the set of zero-divisors. Given rings $A \subseteq B$, it is possible that $a \in A$ forms a non-zero-divisor (or unit) in $A$ but not in  $B$. Under our initial requirement that rings are commutative unital, for a nonzero ring $A$ to be an \textbf{integral domain}, we only need all nonzero elements to be non-zero-divisors, and for $A$ to be a  \textbf{field}, we only need all nonzero elements to be units.
\begin{equiv_def}
\label{edF}
...
\end{align*}
\begin{equiv_def}
\textbf{(Sum of ideals)} Let $A$ be a ring and  $(\mathfrak{a}_i)$ a family of ideals in $A$. The sum ideal $\sum \mathfrak{a}_i$ is generated by $\bigcup \mathfrak{a}_i$. 
\end{equiv_def}
\begin{proof}
Unroll the definitions. 
\end{proof}
...
\end{equiv_def}
\begin{proof}
  (i) $=$ (ii): Unroll the definition.  (ii) $=$ (iii): Consider the \customref{ctfr}{correspondence theorem} and \customref{EDfN}{$\operatorname{Nil}(A \quotient \mathfrak{a})= \bigcap \operatorname{Spec} A \quotient \mathfrak{a}$}. 
\end{proof}
Let $\mathfrak{a}$ be an ideal of the ring $A$. A \textbf{minimal prime ideal $\mathfrak{p}$ over} $\mathfrak{a}$ is a prime ideal minimal among all prime ideals that contain $\mathfrak{a}$. Because of the \customref{Roi}{third definition of radical}, $\mathfrak{a}$ and $\sqrt{\mathfrak{a}}$ share same set of minimal prime ideal. It should be noted that there is a "less is more" philosophy in our wording and notations for product, quotient and radical of ideals. For any ideal $\mathfrak{a},\mathfrak{b}$, we have 
 \begin{align*}
...
\begin{theorem}
\textbf{(Third isomorphism theorem for modules)} Let $A$ be some ring and  $T \subseteq S \subseteq M$ be three $A$-modules. If we identify $S\quotient T$ as $A$-submodule of $M\quotient T$ the obvious way, then we have an obvious $A$-module isomorphism: 
\begin{align*}
   \frac{M\quotient T}{S\quotient T}\cong  \frac{M}{S}
\end{align*}
\end{theorem}
\begin{proof}
Unroll the definitions. 
...
\end{proof}
There are multiple equivalent definitions for the term \textbf{algebra $B$ over ring $A$}. The easiest one is to say we have a ring homomorphism $f:A\rightarrow B$, which induces the scalar product: $a(b)\triangleq f(a)b$. Let $B,C$ be two  $A$-algebra with scalar defining map $f,g$. By an \textbf{$A$-algebra homomorphism}, we mean some ring homomorphism $h:B\rightarrow C$ that is also a $A$-module homomorphism, or equivalently that the ring homomorphism $h$ satisfies  $h\circ f=g$.      \\
...
Let $A$ be a ring, $\mathfrak{C}$ a subcategory of $A$-modules, and $G$ an abelian group. A function  $\ld : \mathfrak{C}\rightarrow G$ is said to be \textbf{additive} if for each short exact sequence $0\rightarrow M'\rightarrow M\rightarrow M''\rightarrow 0$ in $\mathfrak{C}$, we have $\ld (M)=\ld (M')+\ld (M'')$. 
\begin{theorem}
\label{Eod}
...
\end{proof}
\chapter{Dimension and Completion}
\section{Completion}
...
Given an affine quasi variety $X\subseteq \A^n$, we use notation $\Gamma (X)$ to denote its \textbf{coordinate ring} $k[x_1,\dots ,x_n]\quotient I(X)$, which is clearly isomorphic to the ring of function $f:X\rightarrow k$ such that $f=F|_X$ for some $F \in k[x_1,\dots ,x_n]$\footnote{We call these functions \textbf{polynomial functions}.}. Because of the \customref{Pic}{prime-irreducibility correspondence in affine setting}, the coordinate of affine quasi variety is an integral domain. Later we will see more usage of the realization of  $\Gamma (X)$ as ring of polynomial functions in the proof of \myref{Theorem}{Asog}. 
\begin{theorem}
\label{Asog}
...
We now show $\phi^*=\theta$. Let $f \in K(Y)$ and $x \in \phi_U^{-1}(\operatorname{Dom}f)$.  Because by irreducibility, $\phi_U^{-1}(\operatorname{Dom}f)$ is nonempty open, to show $\theta (f)=\phi^*(f)$, we only have to show $\theta (f)(x)=\phi^*(f)(x)$, which is done by verifying the computation:
\begin{align*}
...
\begin{equiv_def}
  \textbf{(Singularities of affine quasi variety)} Given quasi affine variety $X \subseteq \A^n$, $\textbf{a} \in X$, and some set $\set{F_1,\dots ,F_t}\subseteq k[x_1,\dots ,x_n]$ of generators of $I(X)$, the followings are equivalent: 
  \begin{enumerate}[label=(\roman*)]
...
\end{equiv_def}
\begin{proof}
Define $k$-linear map $\theta: k[x_1,\dots ,x_n]\rightarrow k^n$ by 
\begin{align*}
\theta (G)\triangleq \left(\frac{\partial G}{\partial x_1}\Bigg|_\textbf{a},\dots , \frac{\partial G}{\partial x_n}\Bigg|_\textbf{a} \right)
\end{align*}
Let $\mathfrak{a}\triangleq \langle x_1-a_1,\dots,x_n-a_n\rangle\subseteq k[x_1,\dots ,x_n] $ be the maximal ideal correspond to $\textbf{a}$. Unrolling the definitions shows that $\theta $ vanishes on $\mathfrak{a}^2$ and $\theta (x_i-a_i)$ forms a basis for $k^n$. Therefore,  $\theta$ induces a $k$-linear isomorphism  $\theta' : \mathfrak{a}\quotient \mathfrak{a}^2 \rightarrow k^n$. Again, unrolling the definitions shows that dimension of $\theta (I(X))$ equals to the rank of the  Jacobian matrix. Note that $(\theta')^{-1}(\theta (I(X)))= (I(X)+\mathfrak{a}^2)\quotient \mathfrak{a}^2$. \\

\myref{Theorem}{Asog} stated that $\mathcal{O}_p \cong (k[x_1,\dots ,x_n]\quotient I(X))_{(\mathfrak{a}+ k[x_1,\dots ,x_n])\quotient I(X)}$
\end{proof}
...
\end{align*}
holds. The \textbf{direct limit} of the system, denoted $A\triangleq \varinjlim A_i$, is a target $(A,\phi_i)$ that satisfies the following \textbf{universal property}: For any target $(B,\psi_i)$, there exists a unique morphism $u:A\rightarrow B$ such that 
\begin{align*}
u\circ \phi_i= \psi_i,\quad \text{ for all }i\in I.
\end{align*}
...
\end{tikzcd}\]
Just like other categories, a presheaf isomorphism is again a morphism that admits a two-sided inverse. We from now on denote the category of $\mathfrak{C}$-valued presheaf on $X$ by  $\textbf{PSh}_{\mathfrak{C}}(X)$. \\


Let $\mathfrak{C}$ be a good category, $\mathcal{F}$ and $\mathcal{G}$ in $\textbf{PSh}_{\mathfrak{C}}(X)$, and $\phi\in \operatorname{Hom}_{\textbf{PSh}_{\mathfrak{C}}(X)}(\mathcal{F},\mathcal{G})$. Because we require the diagram to commute, we can define \textbf{kernel} of $\phi$ to be the presheaf $U \mapsto \operatorname{Ker}(\phi (U))$ with restriction map being the restriction of restriction maps of $\mathcal{F}$. Similarly, we can define \textbf{image} of $\phi$ to be the presheaf $U \mapsto \operatorname{Im}(\phi (U))$ with restriction map being the restriction of restriction maps of $\mathcal{G}$. Similarly, we can define \textbf{cokernel} of $\phi$ to be the presheaf $U\mapsto \operatorname{Coker}(\phi (U))$ of quotient of the restriction map of $\mathcal{G}$. \\
...
Given a point $p \in X$ and a presheaf $\mathcal{F}$ on  $X$ valued in a good category, we define the \textbf{stalk} $\mathcal{F}_p$ of $\mathcal{F}$ at $p$ as the direct limit of the directed system $(\mathcal{F}(U), \operatorname{Res})$ over all open neighborhoods $U \ni p$. Therefore, an element of $\mathcal{F}_p$ is an equivalence class represented by a pair $(U, s)$ with $s \in \mathcal{F}(U)$, and two such pairs $(U, s)$ and $(V, t)$ represent the same element if there exists a  neighborhood $W \subseteq U \cap V$ of $p$ such that $s|_W = t|_W$.\\

In particular, recall that when $X$ is a quasi-variety, we denote by $\mathscr{O}(U)$ the ring of regular functions on an open set $U \subseteq X$, and by $\mathscr{O}_p$ the local ring of $X$ at a point $p \in X$. One can verify that $\mathscr{O}$ defines a presheaf on $X$, and that its stalk at $p$ coincides with the local ring $\mathscr{O}_p$—which is precisely why we adopt this notation in the first place. \\
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{(Identity Law)} If $s,t \in \mathcal{F}(U)$ satisfies $s|_{V_i}=t |_{V_i}$ for all $i$, then  $s=t$.  
  \item \textbf{(Gluing Law)} If $\set{s_i \in V_i: i \in I}$ satisfies $s_i|_{V_i \cap V_j}=s_j|_{V_i\cap V_j}$, then there exists $s \in \mathcal{F}(U)$ such that $s|_{V_i}=s_i$ for all $i$. 
\end{enumerate}
For each presheaf $\mathcal{F}$ on $X$, the \textbf{sheafification}  is the unique (up to a isomorphism, the isomorphism is unique if one require it make the diagram commute) sheaf $\mathcal{F}^+$ on $X$ together with a morphism $\theta :\mathcal{F}\rightarrow \mathcal{F}^+$ that     . 



Given a sheaf $\mathcal{F}$ on $X$, a subspace $Z \subseteq X$, and a continuous map  $f:X\rightarrow Y$, and a sheaf $\mathcal{G}$ on $Y$, the \textbf{direct image sheaf} $f_*\mathcal{F}$ is a sheaf on $Y$ defined by $U\mapsto \mathcal{F}(f^{-1}(U))$ with the restriction map being that of $\mathcal{F}$, the \textbf{inverse image sheaf} $f^{-1}\mathcal{G}$ is the sheafification of the presheaf $U \mapsto \lim_{ V \supseteq f(U) }\mathcal{G}(V)$, and the \textbf{restriction sheaf} of $\mathcal{F}$ onto $Z$ denoted by $\mathcal{F}|_Z$ is simply the inverse image sheaf of  $\mathcal{F}$ under the inclusion map $Z \hookrightarrow X$.     
\section{Scheme}
Given some ring $A$ and ideal $\mathfrak{a} \subseteq A$, if we denote $V(\mathfrak{a})$ the set of prime ideals that contains $\mathfrak{a}$, then the collection  $\set{\operatorname{Spec}A-V(\mathfrak{a})}$ where $\mathfrak{a}$ runs over all ideals of  $A$ forms a topology on  $\operatorname{Spec}A$ called the \textbf{Zariski Topology}\footnote{$V(\mathfrak{a})\cup V(\mathfrak{b})=V(\mathfrak{a}\mathfrak{b})$, and $\bigcap V(\mathfrak{a}_i)=V (\sum \mathfrak{a}_i)$.}. Defining $\mathscr{O}(U)$ to be the ring of functions that maps $\mathfrak{p} \in U$ to some element of $A_\mathfrak{p}$ for each $U \subseteq \operatorname{Spec}A$, we get the \textbf{associated sheaf of ring} $\mathscr{O}$ on $\operatorname{Spec}A$. For each $f \in A$, we define $D(f)$ to be the open complement of $V(\langle f\rangle )$. 
\begin{theorem}
\textbf{(Basic property of spectrum)} Let $A$ be a ring. We have:
\begin{enumerate}[label=(\roman*)]
  \item For all $\mathfrak{p}\in \operatorname{Spec}A$, the stalk $\mathscr{O}_\mathfrak{p}$ is isomorphic to the local ring $A_\mathfrak{p}$. 
  \item For all $f \in A$, the ring $\mathscr{O}(D(f))$ is isomorphic to $A_f$. Note that this implies the ring of global sections over $A$ is isomorphic to $A$.  
\end{enumerate}
\end{theorem}
\begin{proof}

\end{proof}
A \textbf{ringed space} is a pair $(X,\mathscr{O})$ consisting of a topological space $X$ and a sheaf of rings  $\mathscr{O}$ on $X$ called the \textbf{structure sheaf}. A \textbf{ringed space morphism} from $X$ to $Y$ is a pair $(f,f^\sharp)$ of a continuous map $f:X\rightarrow Y$ and a sheaf morphism $f^{\sharp}$ from the structure sheaf of $Y$ to the direct image of structure sheaf of  $X$ under  $f$. A ringed space is called a \textbf{locally ringed space} if the stalk of its structure sheaf at each point is a local ring. A \textbf{locally ringed space morphism} from $X$ to  $Y$ is a ringed space morphism $(f,f^{\sharp})$ such that the ring homomorphism from the stalk of the structure sheaf of  $Y$ to the stalk of the direct image sheaf induced by $f^{\sharp}$ are moreover local ring homomorphisms.  \\

An \textbf{affine scheme} is a locally ringed space isomorphic as a locally ringed space to the spectrum of some ring. A \textbf{scheme} is a locally ringed space that has an open cover consisting of open sets when equipped with the restricted sheaf is an affine scheme. A \textbf{scheme morphism} is then just a locally ringed space morphism.
\section{$\mathcal{O}$-modules}
Given a ringed space $(X,\mathcal{O})$, if we say a sheaf $\mathcal{F}$ of abelian groups on $X$ is a $\mathcal{O}$\textbf{-module}, we are assigning each abelian group $\mathcal{F}(U)$ a $\mathcal{O}(U)$-module structure compatible not only with the addition of $\mathcal{F}(U)$ but also the restriction maps in the sense that: 
\begin{align*}
  (fs)|_V= (f|_V)(s|_V),\quad \text{ for each }f\in \mathcal{O}(U),s \in \mathcal{F}(U)\text{ and }V \subseteq U
\end{align*}
A \textbf{$\mathcal{O}$-module morphism} $\phi:\mathcal{F}\rightarrow \mathcal{G}$ is then just a sheaf morphism such that all $\phi (U)$ are also $\mathcal{O}(U)$-module homomorphisms. 
