...
\end{proof}
Let $X$ be a topological space and $Z\subseteq X$ an nonempty irreducible closed subset. A \textbf{generic point} for $Z$ is a point  $P \in Z$ such that $Z= \overline{\set{P}}$.\\ 
...
\begin{enumerate}[label=(\roman*)]
  \item $(g_*\mathcal{O}_Y)|_U \triangleq \mathcal{O}_Y|_{g^{-1}(U)} $. 
  \item $((f\circ g)_* \mathcal{O}_X) |_U = \mathcal{O}_X |_{ f^{-1}(g^{-1}(U))} $. 
\end{enumerate}
...
\end{align*}
This makes locally ringed spaces a category. An \textbf{affine scheme} is a locally ringed space isomorphic as a locally ringed space to the spectrum of some ring. A \textbf{scheme} is a locally ringed space that has an open cover consisting of open sets when equipped with the restricted sheaf is an affine scheme. A \textbf{scheme morphism} is then just a locally ringed space morphism.
\begin{theorem}
\label{Poo}
...
\begin{align*}
 \left((f,f^{\sharp}):X\rightarrow \operatorname{Spec}A   \right) \mapsto  f^\sharp (\operatorname{Spec} A): A=\mathcal{O}_{\operatorname{Spec}A}(\operatorname{Spec}A)\rightarrow \Gamma (X,\mathcal{O}_X) 
\end{align*}
...
\begin{align*}
\operatorname{Proj}S\triangleq \set{\mathfrak{p} \in \operatorname{Spec}S: \mathfrak{p}\text{ is homogeneous and doesn't contain $S_+$ }}
\end{align*}
...
\begin{theorem}
\label{THtp}
\textbf{(Topology of $\operatorname{Proj}$)} Let $S$ be a graded ring, and $\set{\mathfrak{a}_i}$ an infinite collection of homogeneous ideals in $S$. We have: 
 \begin{enumerate}[label=(\roman*)]
...
\end{proof}
Because of \myref{theorem}{THtp}, we can consider $\operatorname{Proj}S$ as a topological space where closed set take the form $V(\mathfrak{a})$, for some homogeneous ideal  $\mathfrak{a}\subseteq S$. Fix $\mathfrak{p}\in \operatorname{Proj}S$. Let $T\subseteq S$ be the multiplicative closed subset of $S$ consisting of all homogeneous elements of  $S$ not in  $\mathfrak{p}$. Let $S_{(\mathfrak{p})} \triangleq T^{-1}S$. We know \customref{Ogoh}{there exists an obvious $\Z$-grading on $S_{(\mathfrak{p})}$ defined by: 
\begin{align*}
  (S_{(\mathfrak{p})})_n \triangleq \set{f\quotient g \in S_{(\mathfrak{p})}: f \text{ homogeneous and }n=\operatorname{deg}f-\operatorname{deg}g}
\end{align*}
We define the structure sheaf on $\operatorname{Proj}S$ by making $\mathcal{O}(U)$ the ring of functions $s: U \rightarrow $
...
\end{proof}
Let $X$ be a Hartshorne scheme. A \textbf{Weil divisor} of $X$ is a finite formal sum of prime divisors of $X$. The group of Weil divisor is denoted by $\operatorname{Div}X$. Because of \myref{Theorem}{THPW}, when we define \textbf{principal divisor $\langle f\rangle $ generated by} $f\in K(X)^{\times}$ by 
\begin{align*}
\langle f\rangle \triangleq \sum_{\text{ prime divisor }Y} \nu _Y(f) \cdot Y
\end{align*}
we know $\langle f\rangle $ is a Weil divisor. We say a Weil divisor is \textbf{effective} if its every coefficient is nonnegative. We say two Weil divisors $D,D'$ are \textbf{linearly equivalent} if $D-D'$ is principal. The \textbf{Weil divisor class group} $\operatorname{Cl}X$ is then just the group of linear equivalence class of Weil divisors, a quotient group of $\operatorname{Div}X$. 
...
\end{tikzcd}\]
Chasing the diagrams, clearly our assignment $U \mapsto \mathcal{K}^-(U)$ forms a ring-valued presheaf on $X$. The \textbf{sheaf $\mathcal{K}$ of total ring of fraction} on $X$ is the sheafification of  $\mathcal{K}^-$. Now, because \customref{THshe}{injectivity of sheaf morphism can be translated to stalk and sheafification on stalk is isomorphism}, we have:  
\begin{align*}
...
\chapter{I don't fucking know}
\section{Part 1}
Let $X$ be a $n$-topological-dimensional nonsingular variety over $k$. I don't understand how to construct the \textbf{tangent $\mathcal{O}_X$-module} $\Omega_{X \quotient k}$. Its \textbf{canonical $\mathcal{O}_X$-module} is $\omega_X \triangleq \extp^n \Omega_{X\quotient k}$.
\begin{theorem}
