...
Unroll the definitions. 
\end{proof}
We say an $A$-module  is \textbf{finitely generated} if it can be spanned by some of its finite subset. Note that one should be careful with the term "generated", since when one identify a ring as a module over itself, the spanned module and the generated ideal of a fixed set can be distinct. 
\begin{theorem}
...
\end{proof}
One of the key property of local ring $A$ is that if we let $\mathfrak{m}$ be its unique maximal ideal, then the quotient $A$-module  $\mathfrak{m}\quotient \mathfrak{m}^2$ forms a $A\quotient \mathfrak{m}$-vector space, called the \textbf{cotangent space of} $A$, with the obvious assignment of scalar product.  If $A$ is local, we often write  $(A,\mathfrak{m},k)$ to mean that $\mathfrak{m}$ is the unique maximal ideal of $A$ and  $k$ the residue field  $k\triangleq A\quotient \mathfrak{m}$. A \textbf{regular local ring} $A$ satisfies  $\operatorname{dim}_k(\mathfrak{m}\quotient \mathfrak{m}^2)=\operatorname{Krudim}(A)$. 
...
y_0 \cdot \begin{bmatrix} 
  \frac{\partial F_1}{\partial x_1} (\textbf{y}) & \cdots & \frac{\partial F_1}{\partial x_n}(\textbf{y})  \\
  \vdots & \ddots & \vdots \\
  \frac{\partial F_t}{\partial x_1}(\textbf{y}) & \cdots & \frac{\partial F_t}{\partial x_n} (\textbf{y}) 
\end{bmatrix} 
\end{align}
...
\end{tikzcd}\]
commutes. Let $\mathcal{F}$ be a $\mathfrak{C}$-valued presheaf on $X$. A \textbf{subpresheaf} $\mathcal{G}$ of $\mathcal{F}$ consists of the assignment to each open set $U \subseteq X$ of a subobject $\mathcal{G}(U) \subseteq \mathcal{F}(U)$ in $\mathfrak{C}$, such that for every inclusion of open sets $U \subseteq V$, the restriction map $\operatorname{Res}^\mathcal{F}_{V,U} \colon \mathcal{F}(V) \to \mathcal{F}(U)$ satisfies: 
\begin{align}
\label{RsF}
\operatorname{Res}^\mathcal{F}_{V,U}(\mathcal{G}(V)) \subseteq \mathcal{G}(U)
\end{align}
\myref{condition}{RsF} ensures that we can define the restriction maps of $\mathcal{G}$ by restricting those of $\mathcal{F}$, thereby making $\mathcal{G}$ a well-defined presheaf. \\
...
\[\begin{tikzcd}
	...
	\arrow["{\tilde{\phi}}", from=1-3, to=3-3]
\end{tikzcd}\]
commute.  
\begin{theorem}
\textbf{(Existence of sheafification of sheaves of good category)} Let $X$ be a topological space, and  $\mathcal{F}$ a presheaf of abelian groups on $X$. The sheafification can be defined by setting  $\mathcal{F}^{+}(U)$ to be the set of functions $s$ from  $U$ to  $\bigcup_{P \in U} \mathcal{F}_P$ such that: 
\begin{enumerate}[label=(\roman*)]
...
\end{proof}
\begin{theorem}
\textbf{(Sheafification of a sheaf is itself)} Let $X$ be a topological space, and  $\mathcal{F}$ a sheaf of abelian groups on $X$. Then, the sheafification $\theta :\mathcal{F} \rightarrow \mathcal{F}^+$ is a abelian group-valued sheaf isomorphism.
\end{theorem}
\begin{proof}
...
\end{proof}
A \textbf{ringed space} is a pair $(X,\mathcal{O} )$ consisting of a topological space $X$ and a sheaf of rings  $\mathcal{O}$ on $X$ called the \textbf{structure sheaf}. A \textbf{ringed space morphism} from $X$ to $Y$ is a pair $(f,f^\sharp)$ of a continuous map $f:X\rightarrow Y$ and a sheaf morphism $f^{\sharp}$ from the structure sheaf of $Y$ to the direct image of structure sheaf of  $X$ under  $f$. \\
...
Let $X\subseteq \C^n$ be an affine variety, and let $M$ be an  $A(X)$-module. We may define a presheaf $\tilde{M}$ of abelian groups on $X$ by setting $\tilde{M}(U)$ to be the set of functions $s$ from $U$ to  $\bigcup_{P \in U}M_P$ such that: 
\begin{enumerate}[label=(\roman*)]
  \item $s(P)\in M_P$. 
  \item For all $P \in U$, there exists  $F \in \C[x_1,\dots ,x_n]$ such that $P\in X -V(F) \subseteq U$ and exists $t \in M_F $ such that $t_P = s(P)$.
\end{enumerate} 
We say a sheaf $\mathcal{F}$ of abelian groups on $X$ is  \textbf{quasi-coherent} if $X$ has an open cover $\set{U_\alpha }$ such that $U_\alpha $ is quasi-variety isomorphic to some affine variety and there exists $(A(U_\alpha ))$-module  $M_{\alpha }$  such that: 
\begin{align*}
\mathcal{F} |_{U_\alpha } = \tilde{M_\alpha} 
\end{align*}
We say $X$ is \textbf{coherent} if  $M_\alpha $ are moreover all finitely generated. 
