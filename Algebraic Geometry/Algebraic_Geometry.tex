...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{(Gluing Law)} If $\set{s_i \in V_i: i \in I}$ satisfies $s_i|_{V_i \cap V_j}=s_j|_{V_i\cap V_j}$, then there exists \textbf{gluing section} $s \in \mathcal{F}(U)$ such that $s|_{V_i}=s_i$ for all $i$. 
\end{enumerate}
\begin{definition}
...
\begin{theorem}
\label{Poo}
\textbf{(Property of open sets associated with element of the underlying ring)} Let $A$ be a ring and $f,g \in A$. Then:\footnote{If you wish, you make think of our notation $D$ stands for "the set of local rings whose denominator Doesn't contain".}
\begin{enumerate}[label=(\roman*)]
...
\end{enumerate}
\begin{theorem}
\textbf{(Basic property of affine scheme $\operatorname{Spec}A$)} Let $A$ be a ring, and $\mathcal{O}$ the structure sheaf on $\operatorname{Spec}A$. Then:
\begin{enumerate}[label=(\roman*)]
  \item For all $\mathfrak{p}\in \operatorname{Spec}A$, the stalk $\mathcal{O} _\mathfrak{p}$ is isomorphic to the local ring $A_\mathfrak{p}$. 
  \item For all $f \in A$, the ring $\mathcal{O} (D(f))$ is isomorphic to $A_f$. 
  \item $\mathcal{O}(\operatorname{Spec}A) \cong  A$ as rings. 
\end{enumerate}
\end{theorem}
\begin{proof}
...
\end{align*}
Again, this by \customref{Zto}{the property of topology of $\operatorname{Spec}A$} implies $f \in \sqrt{\sum_{i \in I} \langle h_i\rangle } $, which implies that for some $n\geq 1$, we may write $f^n$ as a finite sum: 
\begin{align*}
...
\begin{align*}
\frac{a_ih_i}{h_ih_j}= \frac{a_jh_i}{h_ih_j} \in A_{h_ih_j}. 
\end{align*}
