...
Let $a,b \in R$ with prime characteristic $p$, we have $(a+b)^p= a^p +b^p $. Moreover, by induction, $(a_1+\cdots + a_n)^{p^e}= a_1^{p^e}+ \cdots + a_n^{p^e}$. 
\begin{theorem}
\label{THFLT}
\textbf{(Fermat's Little Theorem)} $a^p \equiv a \inz$ mod $p$.     
\end{theorem}
...
\begin{theorem}
\label{THKrrr}
\textbf{(Kunz's criteria for regularity of reduced ring with prime characteristic, 1976)} Let $R$ be a reduced ring of prime characteristic $p$. The followings are equivalent:
\begin{enumerate}[label=(\roman*)]
  \item $R$ is regular. 
...
\end{align*}
we see that $r_\beta  =0 $ for all $\beta  \not\leq  (p^{e-1},\dots ,p^{e-1})$, and $r_\gamma =r_{\beta }$ for  all $\beta  \leq (p^{e}-1,\dots ,p^{e}-1)$. Because of such, for each $p$-power denominator rational number $a\quotient p^e$, it is well-defined, independent of the representation, whether there exist coefficient $r_\beta \in R$ in \myref{equation}{EQfape} that lies the maximal ideal $\mathfrak{m}\triangleq (x_1,\dots ,x_n)\subseteq R$. We define the $F$\textbf{-threshold} $FT(f)$ of $f$ to be the supremum of all nonnegative $p$-power denominator rational number $a\quotient p^{e}$ such that in \myref{equation}{EQfape}, we have $r_\beta \not \in \mathfrak{m}$ for all $\beta  $. 
\begin{equiv_def}
\label{EDFth}
\textbf{($F$-threshold)} Let $p$ be prime, $R\triangleq \F_p[x_1,\dots ,x_n]$,  $\mathfrak{m}\triangleq (x_1,\dots ,x_n)\subseteq R$, and $f \in R$. The following numbers are equivalent: 
\begin{enumerate}[label=(\roman*)]
  \item $FT(f)$.  
  \item $\sup \set{\frac{a}{p^e}:a,e \geq 0 \text{ and }f^{\frac{a}{p^e}}\in \mathfrak{m}R^{\frac{1}{p^e}}}$, where $\mathfrak{m}R^{\frac{1}{p^e}}\subseteq R^{\frac{1}{p^e}}$ is the extension ideal. 
\end{enumerate}
\end{equiv_def}
\begin{proof}
...
\begin{theorem}
\label{THLmbf}
\textbf{(Lucas' modulo binomial formula)} Let $m\geq n \inn$ and $p$ be prime. Writing $m$ and $n$ in their base $p$ extension: $m=\sum_{j=0}^r m_jp^j$ and $n=\sum_{j=0}^r n_jp^j$, we have: 
\begin{align*}
\binom{m}{n}\equiv \binom{m_0}{n_0} \binom{m_1}{n_1} \cdots \binom{m_r}{n_r}\quad (\text{mod } p)
\end{align*}
\end{theorem}
\begin{example}
Consider $x^2+y^3 \in \F_p [x,y]$. Because by \customref{THFLT}{Fermat's little theorem} and \myref{theorem}{THWp} we have 
\begin{align*}
...
\end{align*}
by \customref{THLmbf}{Lucas' modulo binomial formula}, we know that that $i=\frac{p^e-1}{2}$ satisfies \myref{condition}{EQ2ipe}. We have shown that for all prime $p$ congruent to $1$ modulo $6$, we have  $FT(f)=\frac{5}{6}$. It now follows from \myref{theorem}{THWHTY} and Dirichlet's theorem that $\operatorname{lct}(f)=\frac{5}{6}$. 
\end{example} 
