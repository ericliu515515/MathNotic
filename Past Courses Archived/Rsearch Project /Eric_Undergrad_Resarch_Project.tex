...
\end{align*}
Note that for all $\alpha \in (0,1)$, $f$ must pass through  $(\alpha , \sin (\alpha ^{-1}))$, otherwise the connected image of $f$ is separated by the two open half plane defined by $x=\alpha $, which is impossible. In other words, $f$ is surjective. We have shown $T= f([0,1])$ is compact, which is impossible. 
\end{proof}
\section{Long Line}
\label{Long Line}
\begin{mdframed}
