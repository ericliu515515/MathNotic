...
\begin{definition}
\textbf{(Definition of Alphabet)} If we say $K$ is an \textit{alphabet}, we mean $K$ is an finite ordered set of \textit{letters (or symbols and signs)}.\\

For example, we can let $K=\set{a,b}$, where $b$ is a letter of $K$.
\end{definition}
\begin{definition}
\textbf{(Definition of String)} Let $K=\set{a,b}$ be an alphabet. If we say $\alpha $ is a \textit{string (or word or symbol)} in $K$, we mean $\alpha $ is a finite sequence (or a finite tuple) of symbols of $K$.\\

For example, we can let  $\alpha =abb a b b b$\\

Also, we say the \textit{length} of $\alpha  $ is $7$, and given two words $\alpha =abbabbb,\beta =aaa$ in $K$, we define $\alpha \beta $ to be the word $abbabbbaaa$ and call $\alpha \beta $ the \textit{product (or concatenation)} of $\alpha $ and $\beta $
\end{definition}
\begin{definition}
  \textbf{(Definition of Elementary Formal System)} If we say $E$ is an \textit{elementary formal system} over alphabet $K=\set{a,b}$, we mean a collection of the following:
\begin{enumerate}[label=(\alph*)]
  \item The alphabet $K$\\
  \item Another alphabet consists of symbols called \textit{variables}\\
  \item Another alphabet consists of symbols called \textit{predicate}, where each predicate is assigned with a positive integer called its \textit{degree}. Notice that the three alphabets should be mutually disjoint.\\
  \item A symbol called \textit{implication sign} usually denoted $\longrightarrow $\\
  \item A symbol called \textit{punctuation sign} usually denoted $,$\\

By a \textit{term} of $E$, we mean a string composed of variables and symbols in $K$. For example, let $x$ and $y$ be two variables. A term can be $aybxxybba$.\\

Let $P$ be a predicate of degree $3$. By an \textit{atomic formula} , we mean a string of the form $Pt_1,t_2,t_3$ where $t_1$ and  $t_2$ and  $t_3$ are all terms.\\

By a \textit{formula (or well formed formula or w.f.f.)}, we mean either an atomic formula or a string of the form $F_1\longrightarrow F_2\longrightarrow F_3\longrightarrow F_4$ where $F_1$ and $F_2$ and $F_3$ and $F_4$ are all atomic formula. Also, we say $F_1\text{ and }F_2\text{ and }F_3\text{ and }F_4$ are \textit{atomic component} of the \textit{compound formula} $F_1\longrightarrow F_2\longrightarrow F_3\longrightarrow F_4$\\
\item A finite list of formulas, called the \textit{axioms} of $E$
\end{enumerate}
Let $x,y$ be variable,  $P_1$ be predicate of degree  $3$, $P_2$ be  predicate of degree $4$ and $X$ be formula $P_1baxy,x,ab\longrightarrow  P_2x,x,b,aa$. By an \textit{instance} of  $X$ we mean a string obtained from $X$ by substituting string in $K$ for all variables. For example, an instance of $X$ can be $P_1baabb,a,ab\longrightarrow P_2,a,bbb,b,aa$\\

A formula with no variable is called a \textit{sentence}.\\

We now give two \textit{rules of inference}.

\begin{enumerate}[label=(\arabic*)]
  \item Substitution of string in $K$\\
  \item Given an atomic formula $X_1$ and formula  $X_1\longrightarrow X_2$ where $X_2$ is a formula, we have $X_2$ 
\end{enumerate}\\

We now give an example of usage of rules of inference. Let $S$ be a predicate of degree $1$,  $a$ and  $b$ be symbols of $K$, and $x$ be a variable.\\

Given the following  $6$ axioms:
 \begin{enumerate}[label=(\arabic*)]
   \item $Sa$ \\
   \item $Sb$ \\
   \item $Sab$ \\
   \item $Sba$ \\
   \item $Sxa\longrightarrow  Sxab$\\
   \item $Sxb\longrightarrow Sxba$ \\

We can infer
    \item $Sab\longrightarrow Saba$ ($x$ in Axiom $6$  get replaced by $a$, using first rule of inference)\\
    \item $Saba$  (using second rule of inference, with Axiom $3$ and  formula $7$, which have been inferred)\\
    \item $Saba \longrightarrow Sabab$ ($x$ in Axiom $5$ get replaced by  $ab$, using first rule of inference)\\
    \item $Sabab$  (using second rule of inference, with formula $8$ and $9$, which have both been inferred)
\end{enumerate}
By a \textit{theorem (or provable string)} in $E$, we mean a string that is either an axiom or can be inferred using finite amount of times of rules of inference. So, in our example, we can say $Sabab$ is a theorem.
\end{definition}
