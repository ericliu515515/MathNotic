...
\begin{align*}
H(\boldsymbol{\xi})\triangleq \left(\sum_{i=1}^n \xi_i \cdot \textbf{T}_i (\boldsymbol{\xi}) \right)  - f\left(\textbf{T}(\boldsymbol{\xi}) , \textbf{0}\right) 
\end{align*}

Fix $i$. Compute 
 \begin{align*}
\frac{\partial H}{\partial \xi_i} \bigg|_{\boldsymbol{\xi}}&= \frac{\partial \textbf{T}}{ \partial \xi_i} \bigg|_{\boldsymbol{\xi}} + \sum_{j\neq i} \xi_j \cdot \left(\frac{\partial \textbf{T}}{\partial \xi_i \partial \xi_j} \bigg |_{\boldsymbol{\xi}}  \right) - \sum_{j=1}^n \frac{\partial f}{\partial x_j} \bigg|_{\left(\textbf{T}(\boldsymbol{\xi}), \textbf{0} \right)}  \cdot \frac{\partial T_j}{\partial \xi_i} \bigg|_{\boldsymbol{\xi}}\\
&= \frac{\partial \textbf{T}}{ \partial \xi_i} \bigg|_{\boldsymbol{\xi}} + \sum_{j\neq i} \xi_j \cdot \left(\frac{\partial \textbf{T}}{\partial \xi_i \partial \xi_j} \bigg |_{\boldsymbol{\xi}}  \right) - \sum_{j=1}^n \textbf{G}_j\left(\textbf{T}(\boldsymbol{\xi}) \right)  \cdot \frac{\partial T_j}{\partial \xi_i} \bigg|_{\boldsymbol{\xi}} \\
&=  \frac{\partial \textbf{T}}{ \partial \xi_i} \bigg|_{\boldsymbol{\xi}} + \sum_{j\neq i} \xi_j \cdot \left(\frac{\partial \textbf{T}}{\partial \xi_i \partial \xi_j} \bigg |_{\boldsymbol{\xi}}  \right) - \sum_{j=1}^n \xi_j \cdot \frac{\partial T_j}{\partial \xi_i} \bigg|_{\boldsymbol{\xi}} 
\end{align*}

\end{proof}
\begin{question}{}{}
Define for all $k\inn$ that 
\begin{align*}
f_k (\textbf{x})\triangleq k^{\frac{n}{2}}\int_{\R^n} e^{-k (\abso{\textbf{x}-\textbf{y}}^2 + \abso{\textbf{x}-\textbf{y}}^4)+ i \sum_{j=1}^n x_j^2y_j } dy
\end{align*}
Find its limit 
\end{question}
\section{Year 113}
