...
\end{corollary}
\begin{proof}
Every statement of this corollary follows from \customref{THifta}{inverse function theorem at a single point}, except that $\textbf{f}^{-1}$ is $C^1$, which would follows from proving $GL_n(\R)\rightarrow GL_n(\R),A \mapsto A^{-1}$ is continuous, since 
\begin{align*}
...
\begin{align*}
  ...
  f(x)&\triangleq  0  &\text{ if }x_1x_2x_3=0
\end{align*}
Prove that $f$ is continuous at everywhere except at $\set{\textbf{x}\inr^3: x_1x_2x_3=0 }$
\end{question}
\begin{proof}
Clearly  
\end{proof}
